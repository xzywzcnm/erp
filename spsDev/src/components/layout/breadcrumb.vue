<template>
  <!-- 面包屑 -->
  <Breadcrumb>
    <BreadcrumbItem v-for="(item, index) in BreadMenuData" :key="index" :to="item.path ? item.path.includes('/details') ? null: item.path : null">
      {{ item.name }}
    </BreadcrumbItem>
  </Breadcrumb>
</template>
...
export default {
  ...
  methods: {
    ...
    getBreadMenuData () {
      let list = this.BreadMenuAllData.filter(i => i.path === this.$route.path);
      this.BreadMenuData = list[0] && list[0].breadMenu ? list[0].breadMenu : [];
    }
  },
  watch: {
    '$route': {
