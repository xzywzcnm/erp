            ...
            <Form ref="pageParams" :model="pageParams" :label-width="labelWidth" >
              <dyt-filter>
                <Form-item label="导入类型" >
                  <dyt-select v-model="pageParams.types" :max-tag-count="1" multiple >
                    <Option
                ...
                <Form-item label="导入时间" class="pageParams-createdTime">
                  <DatePicker
                    type="date"
                    transfer
      ...
      importColumn: [
        {
          ...
          minWidth: 120,
          render: (h, params) => {
            if (params.row.type === 'supplierGoodsPriceImport') {
              return h('span', '采购报价导入');
            } else if (params.row.type === 'supplierImport') {
              return h('span', '供应商导入');
            }else if (params.row.type === 'supplierFreightCheckImport') {
              return h('span', '运费导入');
            }
          }
        }, {
