<template>
  <div class="dyt-select-demo" :class="{
    'max-tag-count-style': selectConfig['max-tag-count'] === 1 || selectConfig.nowrap
  }">
    <Select :ref="`select-${random}`" v-model="nowVal" v-bind="selectConfig" v-on="selectListeners" @on-select="onSelect"
      @on-change="onChange" @on-query-change="queryChange">
      ...
      <slot :list="list">
        <Option v-for="(item, index) in list" :value="item[replaceKey.value]" :key="`${index}-${item[replaceKey.value]}`">
          {{ `${item[replaceKey.value].includes(selectInvertKey) && isSelectAll ? '取消' : ''}${item[replaceKey.label]}` }}
        </Option>
      </slot>
    </Select>
  </div>
