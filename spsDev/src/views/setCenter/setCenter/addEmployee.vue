        ...
        <div class="addEmpolyeeData" >
          <Form
            ...
            <Form-item label="电话" prop="mobile" >
              <Input v-model.trim="newEmployeeParams.mobile" :disabled='newEmployeeParams.mobileDisabled' ></Input >
            </Form-item >
...
export default {
  ...
  methods: {
    getUserMes () {
      let v = this;
      v.axios.post(api.check_user + v.uid).then(function (response) {
        if (response.data.code == 0) {
          ...
          v.newEmployeeParams.mobileDisabled = data.mobileBind == 1;
    ...
    getStrongPassword () { // 强密码生成
      var v = this;
      var length = 16;
      if (!chars) var chars = 'abcdefghijkmnpqrstuvwxyzABCDEFGHJKMNPQRSTUVWXYZ23456789*&%$#@!';
      var randomChars = '';
    ...
    getShopList () {
      var v = this;
      v.axios.get(api.get_shopList).then(function (response) {
        if (response.data.code == 0) {
          var data = response.data.datas;
    ...
    makePermisssionList (list) { // 解构权限数据
      var v = this;
      var pos = [];
      var // 平台类别数组
        groupPos = {};// 平台权限组合
      Promise.resolve(list.map(function (n) {
    ...
    getGroup (groupPos) {
      var v = this;
      v.platformTypeList.forEach(function (n, i) {
    ...
    addNewEmployee () { // 保存员工数据
      var v = this;
      Promise.all([v.handleSubmit('newEmployeeParams'), v.integrateSaleAccountId()]).then(function (result) {
        if (result[0]) {
          v.axios.post(api.add_user, JSON.stringify(v.newEmployeeParams)).then(function (response) {
            if (response.data.code == 0) {
              v.$Message.success('新增成功');
              v.$emit('saveSuccess');
              v.$parent.$parent.modal9 = false;
            } else if (response.data.code == '102010') {
    ...
    editEmployee () { // 保存员工数据
      ...
      Promise.all([v.handleSubmit('newEmployeeParams'), v.integrateSaleAccountId()]).then(function (result) {
        if (result[0]) {
          var obj = v.newEmployeeParams;
          v.newEmployeeParams.userId = v.uid;
          if (obj.password == '') {
            delete obj.password;
          }
          v.axios.put(api.add_user, JSON.stringify(obj)).then(function (response) {
    ...
    authEmail (value) {
      var v = this;
      return Promise.resolve(v.axios.get(api.check_email + '?email=' + value).then(function (response) {
        if (response.data.success && response.data.datas.data == 0) {
          return true;
        } else {
          return false;
        }
    ...
    integrateSaleAccountId () {
      ...
      var pos = [];
      // var str = '';
      v.checkAllGroup.map(function (n) {
        pos = pos.concat(n);
      });
      v.newEmployeeParams.saleAccountIdList = pos;
      return pos;
    },
    validatePass () { // 密码校验
      let v = this;
    ...
    checkAllGroupChange (index) {
      var v = this;
      var data = v.checkAllGroup[index];
