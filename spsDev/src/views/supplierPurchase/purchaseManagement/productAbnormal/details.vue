<template>
  <div class="page-details" v-show="detailsVisible" :class="{ 'page-loading': pageLoading }">
    <div class="details-head">
      <div class="details-head-text">
        <span class="details-back" @click="backList">
          <Icon type="ios-arrow-back" />返 回
        </span>
      ...
      <div class="button-group">
        ...
        <Button type="primary" v-if="isEdit && viewType == 'add'" @click="detailsSubmit(true)" :disabled="pageLoading">提交后停留当前页</Button>
        <Button type="primary" v-if="isBack" @click="sendBack" :disabled="pageLoading">退 回</Button>
        <Button v-if="isEdit && viewType != 'add'" @click="detailsSave(false)" :disabled="pageLoading">保 存</Button>
      ...
      <Form ref="fromRef" class="main-from" :model="formData" :rules="formRules" :label-width="120" :inline="true">
        ...
        <FormItem label="到货异常编号：" class="form-inline-item">
          {{
            viewType == "add"
            ? "保存后创建"
            : this.formData.abnormalArrivalNo || ""
          }}
        </FormItem>
        <FormItem label="创建人：" class="form-inline-item">
          {{ this.createdByInfo ? this.createdByInfo.name : "" }}
        </FormItem>
        <FormItem label="仓库：" class="form-inline-item">
          {{ formData.warehouseName || '' }}
        </FormItem>
        <FormItem label="所属事业部：" class="form-inline-item">
          {{ businessName }}
          <!-- {{ formData.businessDeptName || '' }} -->
        </FormItem>
        ...
        <FormItem label="对应采购员：" prop="purchaserId" class="form-inline-item"
          ...
          <div v-else>
            <dyt-select v-model="formData.purchaserId" :disabled="!registerEdit" @on-change="purchaserIdChange">
              <Option v-for="(item, index) in userInfoList" :value="item.id" :key="`u-${index}`" :label="item.name" />
            </dyt-select>
          </div>
        ...
        <FormItem label="供应商名称：" prop="supplierId" class="form-inline-item">
          <div v-if="formData.isBindingSupplier == 1 || !registerEdit">
            {{ showText('supplierId', formData.supplierId) }}
          </div>
          <div v-else>
            <dyt-select v-model="formData.supplierId" :disabled="formData.isBindingSupplier == 1 || !registerEdit">
        ...
        <!--表格-->
        <Table border highlight-row ref="tableRef" :height="pagination.total > 8 ? 450 : 0" :columns="tableColumns"
          :data="formTableList">
          <!-- 异常采购单号 -->
          <template slot-scope="{ index }" slot="abnormalPurchaseNumber">
            <FormItem v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]" label=""
              ...
              <div v-if="!registerEdit">
                {{ formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalPurchaseNumber }}
              </div>
              <div v-else>
                <Input v-model="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalPurchaseNumber"
                  placeholder="请输入异常采购单号" :clearable="true" :disabled="!registerEdit"
                />
              </div>
            </FormItem>
          </template>
          <!-- 异常sku -->
          <template slot-scope="{ index }" slot="abnormalSku">
            <FormItem v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]" label=""
              ...
              <!-- <Button
                @click="openSelectSku(row, (tableIndex + index))"
                v-if="
                  $common.isEmpty(
                    formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalSku
                  )
          ...
          <!-- 异常数量 -->
          <template slot-scope="{ index }" slot="abnormalNumber">
            <FormItem v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]" label=""
              :prop="`processAbnormalArrivalDetailBOS[${(tableIndex + index)}].abnormalNumber`" class="table-form-item" :label-width="0"
              :rules="tableItemRules.abnormalNumber">
              <div v-if="!registerEdit">
                {{ formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalNumber }}
              </div>
              <div v-else>
                <Input v-model.number="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalNumber"
                  placeholder="请输入不小于 0 的整数" :clearable="true" :disabled="!registerEdit" type="number"
                />
              </div>
            </FormItem>
          </template>
          <!-- 异常原因 -->
          <template slot-scope="{ index }" slot="abnormalReason">
            <FormItem label="" v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]"
              :prop="`processAbnormalArrivalDetailBOS[${(tableIndex + index)}].abnormalReason`" class="table-form-item" :label-width="0"
              :rules="tableItemRules.abnormalReason">
              <div v-if="!registerEdit">
          ...
          <!-- 仓库异常说明 -->
          <template slot-scope="{ index }" slot="abnormalWarehouseReason">
            <FormItem label="" v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]"
              ...
              <div v-if="!registerEdit">
                {{ formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalWarehouseReason }}
              </div>
              <div v-else>
          ...
          <template slot-scope="{ row, index }" slot="abnormalImage">
            <FormItem
              label=""
              v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]"
              :prop="`processAbnormalArrivalDetailBOS[${(tableIndex + index)}].abnormalImage`" class="table-form-item" :label-width="0"
            >
            <div v-if="registerEdit">
              <dytUpload ref="uploadImageRef" name="files" :maxSize="1024 * 10" :show-upload-list="false"
                ...
                <Button @click="selectTableRow(row, (tableIndex + index))"
                  v-if="$common.isEmpty(formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalImage) && registerEdit"
                >上传图片</Button>
                <Poptip trigger="hover"
                  v-if="!$common.isEmpty(formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].abnormalImage)"
                  transfer placement="left"
                >
            ...
            <div v-else>
              <div v-if="!$common.isEmpty(formData.processAbnormalArrivalDetailBOS[index].abnormalImage)">
                <large-picture 
                class="larPicStyle" 
                v-if="formData.processAbnormalArrivalDetailBOS[
                  index
                ].abnormalImage.includes('http://') ||
                  formData.processAbnormalArrivalDetailBOS[
                    index
                  ].abnormalImage.includes('https://')
          ...
          <!-- 收货地址 -->
          <template slot-scope="{ index }" slot="abnormalAddress">
              <FormItem label="" v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]"
                ...
                <div v-if="!feedbackEdit">
                  {{ formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].deliveryAddress }}
                </div>
                <div v-else>
          ...
          <!-- 采购/仓库处理说明 -->
          <template slot-scope="{ index }" slot="processReason">
            <FormItem label="" v-if="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)]"
              ...
              <div v-else>
                <Input v-model="formData.processAbnormalArrivalDetailBOS[(tableIndex + index)].processReason" placeholder="请输入采购/仓库处理说明" :clearable="true" :disabled="!feedbackEdit" />
              </div>
            </FormItem>
          </template>
        </Table>
        ...
        <template v-if="formData.status >= 2">
          ...
          <FormItem label="需退回总数量：" prop="returnTotalNumber" class="form-inline-item"
            ...
            <div v-else>
              <Input v-model.number="formData.returnTotalNumber" placeholder="请输入需退回总数量" :clearable="true"
                :disabled="!feedbackEdit" type="number" />
            </div>
          </FormItem>
        ...
        <template v-if="formData.status >= 3">
          ...
          <FormItem label="寄出数量：" prop="shipmentNumber" class="form-inline-item">
            ...
            <div v-else>
              <Input v-model.number="formData.shipmentNumber" placeholder="请输入寄出数量" :clearable="true"
                :disabled="!afterSaleEdit" type="number" />
            </div>
          </FormItem>
    ...
    <Modal v-model="addProductModal" title="添加产品" width="1200" footer-hide :mask-closable="false">
      <addProduct :disabledSlt="false" :from="true" :onlyChoseOne="true" @addProductSuccess="addProductSuccess"
        @addProductCancel="addProductCancel" v-if="addProductModal"></addProduct>
    </Modal>
    ...
    <Modal v-model="transferModal" title="转交采购员" width="500" :mask-closable="false">
      ...
      <div slot="footer">
        <Button @click="transferModal = false">取消</Button>
...
export default {
  ...
  data() {
    return {
      ...
      imgAddress: api.upload_files, // 上传图片的接口
      // 表单数据
      formData: {},
      formInitData: {
        arrivalDate: "",
      ...
      formRules: {
        arrivalDate: [
          {
            getRequired: () => this.isEdit,
            validator: this.validatorItem,
            msg: "请选择到仓日期",
            trigger: "change",
          },
        ],
        ...
        processReason: [
          {
            getRequired: () => this.reasonRequired,
            validator: this.$common.strMaxVerify,
            msg: "请输入采购处理说明",
            maxLength: 200,
            trigger: "blur",
        ...
        receiptSupplierId: [
          {
            getRequired: () => this.supplierIdRequired,
            validator: this.validatorItem,
            msg: "请选择收件供应商名称",
            trigger: "change",
          },
        ...
        // receiptSupplierInfo: [
        //   {
        //     getRequired: () => this.supplierInfoRequired,
        //     validator: this.$common.strMaxVerify,
        //     msg: "请输入供应商收件信息",
        //     maxLength: 200,
        //     trigger: "blur",
        ...
        returnTotalNumber: [
          {
            getRequired: () => this.returnNumberRequired,
        ...
        shipmentDate: [
          {
            getRequired: () => this.shipmentDateRequired,
            validator: this.validatorItem,
            msg: "请选择寄出日期",
            trigger: "change",
          },
        ...
        shipmentTrackingNumber: [
          {
            getRequired: () => this.sTracNumberRequired,
            validator: this.$common.strMaxVerify,
            maxLength: 30,
            msg: "请输入寄出物流单号",
            trigger: "blur",
  ...
  computed: {
    ...
    isBack() {
      return (
        this.isEdit &&
        !["add"].includes(this.viewType) &&
        this.formData.status > 1 &&
        this.permission.back
      );
    },
    // 是否可以编辑 异常明细登记
    registerEdit() {
      return (
        this.isEdit &&
        (this.formData.status == 1 || ["add"].includes(this.viewType))
      );
    },
    // 是否可以编辑 采购处理反馈
    feedbackEdit() {
    ...
    reasonRequired() {
      return (
        [1, 3].includes(Number(this.formData.processMeasure)) &&
        this.feedbackEdit
      );
    },
    // 收件供应商名称 必填
    supplierIdRequired() {
      return (
        [2].includes(Number(this.formData.processMeasure)) && this.feedbackEdit
      );
    },
    // 供应商收件信息 必填
    ...
    shipmentDateRequired() {
      return (
        [2].includes(Number(this.formData.processMeasure)) && this.afterSaleEdit
      );
    },
    // 寄出物流单号是否必填
    sTracNumberRequired() {
    ...
    // warehouseInfo() {
    //   let warehouseItems = this.warehouseArr.filter(k => {
    //     return k.warehouseId === this.formData.warehouseId;
    //   })
    //   return warehouseItems[0] || {};
    // },
    // ID
    abnormalArrivalId() {
    ...
    erpUserId() {
      ...
      if (this.$common.isEmpty(this.$store.getters.authUserInfo.securityUser))
        return "";
      return this.$store.getters.authUserInfo.securityUser.erpUserId || "";
    },
    // 创建人
    createdByInfo() {
      const list = this.$common.arrayToObj(this.userInfoList, "name");
      let userInfo = {};
      if (this.viewType != "add") {
        userInfo = this.$common.isEmpty(this.formData.createdBy)
          ? {}
          : list[this.formData.createdBy];
      } else {
        userInfo = this.$common.isEmpty(this.erpUserId)
          ? {}
          : list[this.erpUserId];
      }
      return this.$common.isEmpty(userInfo)
        ? { userName: this.formData.createdBy || "" }
        : userInfo;
    },
    // 异常原因
    reasonJson() {
    ...
    tableColumns() {
      ...
      if (this.registerEdit) {
        if (this.viewType == "add") {
          return [...defaultColumns, abnormalImage, operation];
        }
        return [...defaultColumns, abnormalImage, operation];
      }
      if (this.formData.status > 1) {
        return [...defaultColumns, processReason, abnormalImage, abnormalAddress];
      }
      return [...defaultColumns, abnormalImage, abnormalAddress];
    },
    businessName() {
  ...
  methods: {
    ...
    getDetails(id) {
      return new Promise((resolve) => {
        ...
        this.axios
          .post(`${api.abnormalDetail}?abnormalArrivalId=${id}`)
          .then((res) => {
            ...
            // let warehouseInfo = this.moduleData.warehouseInfo || {};
            // this.formData.warehouseName = warehouseInfo.warehouseName || "";
    ...
    getRemarkDetail(id) {
      return new Promise((resolve) => {
        ...
        this.axios
          .post(`${api.abnormalRemarkDetail}?abnormalArrivalId=${id}`)
          .then((res) => {
            ...
            this.logData = this.handRemarkDetail(
              this.logAllData,
              this.showRemarks
            );
            resolve(res.data.datas);
    ...
    // 新增异常数据
    addTableRow(type) {
      const tableData = this.$common.copy(
        this.formData.processAbnormalArrivalDetailBOS
      );
      const tableNumber = tableData.map((row) => {
        return this.$common.isEmpty(Number(row.serialNumber))
          ? 0
          : Number(row.serialNumber);
      });
      const maxIndex = this.$common.isEmpty(tableNumber)
        ? 1
        : Math.max.apply(null, tableNumber) + 1;
      let newRow = {
        productGoodsId: null,
      };
      this.tableColumns.forEach((item) => {
        if (!["operation"].includes(item.key)) {
          if (!this.$common.isEmpty(item.key)) {
            newRow[item.key] =
              item.key == "serialNumber" ? maxIndex : item.defaultVal;
          }
          if (!this.$common.isEmpty(item.slot)) {
            newRow[item.slot] =
              item.slot == "serialNumber" ? maxIndex : item.defaultVal;
          }
        }
      });
    ...
    addProductSuccess(data) {
      let { index } = this.tableSelectInfo;
      let item = this.$common.isEmpty(data) ? {} : data[0];
      let tableList = this.formData.processAbnormalArrivalDetailBOS;
      let list = tableList.filter((k) => k.abnormalSku === item.sku);
      if (!list.length) {
        this.$set(
          this.formData.processAbnormalArrivalDetailBOS[index],
          "abnormalSku",
          item.sku
        );
        this.$set(
        ...
        this.$Message.warning("该SKU已存在列表内!");
      }
    ...
    addLogRemark() {
      ...
      this.remarkLoading = true;
      this.axios
        .post(`${api.addOperate}?abnormalArrivalId=${this.abnormalArrivalId}`, [
          this.remarks,
        ])
        .then((res) => {
          if (!res || !res.data || res.data.code != 0) return;
    ...
    handleSuccess(res) {
      ...
      let tableData = this.$common.copy(
        this.formData.processAbnormalArrivalDetailBOS
      );
    ...
    getFormData(isVerify, isSubmit) {
      return new Promise((resolve) => {
        ...
        newData.arrivalDate = this.$common.isEmpty(newData.arrivalDate)
          ? null
          : this.$common.toLocaleDate(newData.arrivalDate, "YYYY-MM-DD", 0);
        newData.shipmentDate = this.$common.isEmpty(newData.shipmentDate)
          ? null
          : this.$common.toLocaleDate(newData.shipmentDate, "YYYY-MM-DD", 0);
        ...
        if (isVerify) {
          this.$refs.fromRef.validate((valid) => {
            ...
            if (
              this.registerEdit &&
              this.$common.isEmpty(newData.processAbnormalArrivalDetailBOS)
            ) {
    ...
    backHand(type) {
      ...
      this.$refs.modalFromRef.validate((valid) => {
        ...
        this.axios
          .post(
            `${api.abnormalReturn}?abnormalArrivalId=${this.abnormalArrivalId}&returnReason=${this.backData.returnReason}`
          )
          .then((res) => {
            if (!res || !res.data || res.data.code != 0) return;
    ...
    detailsSave(isVerify, isSubmit, isRemain) {
      ...
      this.getFormData(isVerify, isSubmit).then((res) => {
        ...
        let data = res.data
        this.axios
          .post(`${api.addOrUpdateProcess}?type=${isSubmit ? 1 : 2}`, res.data)
          .then((res) => {
            if (!res || !res.data || res.data.code != 0) return;
            this.$Message.success("操作成功");
            if (isRemain) {
    ...
    selectSupplier(item) {
      ...
      let deliveryAddress = 
      this.$common.trim(`${item.contactMan || ''} ${item.telephone || ''} ${item.rAddress || ''} ${item.rAddressDetail || ''}`);
    ...
    showText(type, val) {
      ...
      if (type == 'purchaserId' && val) {
        const list = this.$common.arrayToObj(this.userInfoList, "id");
          ...
          if(res.data.code == 0) {
            ...
            resultList.forEach(rItem => {
              ...
              abnorMalData.wmsGoodsStockApplyDetailList.push({
                ...
                price: null,
                discount: null,
                goodsName: rItem.goodsCnDesc,
                goodsId: rItem.productGoodsId,
                productGoodsSpecifications: rItem.goodsAttributes,
                skuNo: rItem.goodsSku,
...
.page-details {
  ...
  .dyt-select-demo .ivu-select-input[disabled] {
    opacity: 1;
    color: #333;
