<template>
  <div class="productAbnormal">
    <template v-if="curShow == 'managementPage'">
      <div class="listPage">
          ...
          <Form ref="pageSearchForm" :model="searchFormData" :label-width="90">
            ...
            <dyt-filter ref="dyt-filter" @expand="expand">
              ...
              <FormItem label="仓库" prop="warehouseId">
                <dyt-select v-model="searchFormData.warehouseId">
        ...
        <div class="funMain funMain__flex">
          <div>
            <Dropdown @on-click="exportData" class="mr10 fl" v-if="permission.export">
              <Button type="primary">
                Execl导出
          ...
          <div class="tableBox">
            ...
            <Table border highlight-row :height="tableHeight" ref="tableRef" :columns="tableColumns"
              ...
              <template slot-scope="{ row }" slot="abnormalImage">
                <div class="mt10 mb10">
                  <large-picture :url="row.abnormalImage" imageHigh="50px"></large-picture>
                </div>
              </template>
            </Table>
          </div>
    ...
    }" @resetTable="resetTable" :userInfoList="userInfoList" :warehouseArr="warehouseArr"
      @createAddApply="createAddApply" />
    <!-- 此处为跳转到创建入库出库申请单 -->
...
import addReceivedProApply from "../receivedProOutboundApply/addReceivedProApply.vue";
...
export default {
  ...
  components: { detailsPage, largePicture, addReceivedProApply },
  data() {
    return {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          align: 'center',
          render: (h, { row }) => {
            if (this.$common.isEmpty(row.abnormalPurchaseNumber))
              return h("div", {}, "-");
        ...
        },
        {
          ...
          align: 'center',
          render: (h, { row }) => {
            if (this.$common.isEmpty(row.abnormalNumber))
              return h("div", {}, "-");
        ...
        },
        {
          ...
          render: (h, { row }) => {
            ...
            let power = permission[powerList[status]];
            return h(
              "Dropdown",
              {
                props: {
              ...
              [
                ...
                h(
                  ...
                  [
                    ...
                    power && [1, 2, 3].includes(status)
                      ? h(
                        "DropdownItem",
                        {
                          nativeOn: {
                            click: () => {
                              this.addEditViewData(row, "submit");
                            },
                          },
                        },
                        "提交"
                      ...
                      : null,
                    this.permission.delete && status === 1
                      ? h(
                        "DropdownItem",
                        {
                          nativeOn: {
  ...
  methods: {
    ...
    searchData(type, resetTime) {
      ...
      // 是否重置时间范围
      if (resetTime) {
        searchFormData.createTime = [];
        searchFormData.updatedTime = []
      }
      searchFormData.status = this.tabIndex < 0 ? null : this.tabIndex - 0;
    ...
    getSearchParams() {
      ...
      let securityUser = this.authUserInfo.securityUser || {};
      paramsData.businessDeptIds = securityUser.businessDeptIds;
      delete paramsData.createTime;
      delete paramsData.updatedTime;
      return paramsData;
    },
    // 重置搜索条件
    resetSearch() {
    ...
    exportData(str) {
      ...
      this.axios.post(api.abnormalExport, paramsObj).then(({ data }) => {
        if (!data || data.code !== 0) return;
        this.$Message.success(`导出成功，可在导出任务查看${data.datas ? '：' + data.datas : ''}!`);
        if (str == "check") {
    ...
    getWarehouseList() {
      // , { warehouseStatus: "0" }
      this.axios.post(api.allWarehouseList, {}).then(res => {
        if (res.data.code == 0) {
