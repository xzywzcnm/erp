...
<template >
  <div >
    <template v-if="purchaseShow == 'localWarehouse'" >
      ...
      <div class="flexBox spaceBetween" >
        <div class="tableControl" v-if="currentTabItem == '1'" >
          ...
          <Button type="primary" class="iconbuttons" style="margin-left: 12px" @click="exportEcxel" >
            导出备货建议 </Button >
          <Button type="primary" @click="printFn" class="iconbuttons" style="margin-left: 12px" >
            打印备货建议 </Button >
        </div >
      ...
      <div class="normalTop20" >
        <Form ref="transWarehouse" :model="transWarehouse" :label-width="100" :rules="ruleValidate" >
          <FormItem label="设置中转仓为" prop="warehouseId" >
            <dyt-select v-model="transWarehouse.warehouseId" placeholder="备货建议列表" > <Option
...
export default {
  ...
  data () {
    ...
    return {
      ...
      total: 0,
      curPage: 1,
      currentTabItem: 0, // 标签页
      operationType: '',
      calculateTime: '',
      isShowstockPlan: false, // 展示备货计划弹窗
      isShowSettingWarehouse: false, // 批量设置中转仓弹窗
      whSaveLoading: false, // 批量设置中转仓加载状态
      warehouseArr: [], // 备货仓库
      transferWarehouseArr: [], // 中转仓库
      goodsStatusArr: [],
      pageParams: {
        pageNum: 1,
        pageSize: 10, // calcType: 0,
        backupWarehouseId: '',
        suggestType: 0, // 备货仓库类型(0本地仓(自营)，1第三方仓，2：FBA仓，3快速采购)
        dataType: '1',
        goodsStatus: null,
        skuStr: '',
        skuList: []
  ...
  methods: {
    ...
    reset () {
      this.purchaseShow = 'localWarehouse';
    ...
    calStockRequirement () {
      let v = this;
      v.axios.post(api.suggestCalc + '?suggestType=0').then(res => {
        if (res.data.code == 0) {
          v.getList();
        }
        v.isCalStock = true;
        ...
        v.selectionArr.forEach((n, i) => {
          ...
          if (!v.planDatas || v.planDatas.length == 0) {
            v.planDatas.push({
              ...
              backupWarehouseType: 0, // 备货仓库类型(0本地仓，1第三方仓库)
    ...
    intiBaseDataList () {
      ...
      v.axios.post(api.warehouse, {
        ...
        if (res.data.code == 0) {
          ...
          v.transWarehouseArr = res.data.datas;
        }
      });
