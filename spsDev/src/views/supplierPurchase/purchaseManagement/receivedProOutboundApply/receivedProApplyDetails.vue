  ...
  <div>
    ...
    ">
          ...
          <FormItem label="收件供应商名称:" prop="supplierId">
            <div v-if="detailsData.type === 'edit' &&
              receivedProApplyForm.status == 1">
              <dyt-select v-model="receivedProApplyForm.supplierId" clearable filterable :disabled="!(
                detailsData.type === 'edit' &&
                receivedProApplyForm.status == 1
              )
                ">
          ...
          <FormItem label="结算方式:">
            ...
            <!-- <div v-if=" detailsData.type === 'edit' &&
              receivedProApplyForm.status == 1">
              <dyt-select v-model="receivedProApplyForm.settlementTypeName" clearable filterable :disabled="!(
              detailsData.type === 'edit' &&
              receivedProApplyForm.status == 1
            )
              ">
          ...
          <FormItem label="出库原因:" prop="pickingReason">
            <div v-if="detailsData.type === 'edit' &&
              receivedProApplyForm.status == 1">
              <dyt-select v-model="receivedProApplyForm.pickingReason" clearable filterable :disabled="!(
                detailsData.type === 'edit' &&
                receivedProApplyForm.status == 1
              )
                ">
          ...
          <FormItem label="申请出库明细:">
            ...
            <div v-else>
              <div v-if="receivedProApplyForm.applyDetailFilePath">
                ...
                <Icon type="ios-trash" v-if="detailsData.type === 'edit' &&
                  receivedProApplyForm.status == 1
                  " size="20" class="ml20" style="cursor: pointer" @click="getDetailsExcel('delete')" />
              </div>
              <div v-else>-</div>
            </div>
          </FormItem>
        ...
        <div class="normalTop">
          ...
          >
            <template slot-scope="{ index }" slot="returnNumber">
              ...
              >
                <div v-if="(detailsData.type === 'edit' && receivedProApplyForm.status == 1)">
                  <InputNumber v-model="receivedProApplyForm.wmsGoodsStockApplyDetailList[(tableIndex + index)].returnNumber" :min="1" :precision="0" />
                </div>
                <div v-else>
            ...
            <template slot-scope="{ index }" slot="stockReason">
              <FormItem class="mt20" :label-width="0">
                <div v-if="detailsData.type === 'edit' && receivedProApplyForm.status == 1">
                  <Input
                    v-model="receivedProApplyForm.wmsGoodsStockApplyDetailList[(tableIndex + index)].stockReason"
                    placeholder="请输入"
                    :maxlength="200"
                    type="textarea"
                    :rows="3"
                    show-word-limit :disabled="!(detailsData.type === 'edit' && receivedProApplyForm.status == 1)"
                  />
                </div>
                <div v-else>
        ...
        <Row>
          ...
          <FormItem label="金额退/抵方式(核对):" :label-width="150" prop="amountMode">
            <div v-if="detailsData.type === 'edit' &&
              ...
              <dyt-select v-model="receivedProApplyForm.amountMode" clearable filterable :disabled="!(
                detailsData.type === 'edit' &&
                receivedProApplyForm.status == 4
              )
                ">
          ...
          <FormItem label="记账月份(核对）:" prop="tallyTime">
            <!-- <DatePicker
                v-model="receivedProApplyForm.tallyTime"
                type="month"
                placeholder="选择日期"
                style="width: 200px"
                :disabled="
              ...
              <div class="form-pic-item">
                <div v-if="receivedProApplyForm.picturePath">
                  <div class="pic-item-view">
                    <large-picture imageHigh="60px" :url="`./filenode/s${receivedProApplyForm.picturePath}`" />
                  </div>
                  <!-- <div slot="content">
                    <large-picture style="width: 350px" :src="`./filenode/s${receivedProApplyForm.picturePath}`" />
                  </div> -->
                </div>
              </div>
            </div>
...
export default {
  ...
  mixins: [Mixin],
  components: { addProduct,largePicture },
  data() {
    return {
      ...
      ruleValidate: {
        ...
        pickingNo: [{ required: true, message: "请输入", trigger: "change" }],
        pickingReason: [
          {
            required: true,
            type: "number",
      ...
      goodsColumn: [
              ...
              [
                ...
                h("div", { class: "ml20" }, [
                  h(
                    "div",
                    {},
                    `SKU：${!this.$common.isEmpty(params.row.skuNo)
                      ? params.row.skuNo
                      : params.row.goodsSku || "-"
                    }`
                  ),
                  h(
                    "div",
                    {},
                    `名称：${!this.$common.isEmpty(params.row.goodsName)
        ...
        },
        {
          ...
          render: (h, params) => {
            let result =
              this.detailsData.type === "edit" &&
                this.receivedProApplyForm.status == 1
                ? h(
                  "div",
                  {
                    style: {
  ...
  computed: {
    ...
    businessDept() {
      ...
      let data =
        businessDeptInfo[this.receivedProApplyForm.businessDeptId] || {};
      return data.name;
    },
    // 获取当前登录账号可查看的事业部ID列表
        ...
        if (newVal && oldVal) {
          this.supplierArr.forEach((item) => {
            if (item.supplierId === newVal) {
              ...
              this.receivedProApplyForm.settlementTypeName =
                item.settlementType;
              ...
              this.receivedProApplyForm.supplierReceivingInfo =
                this.$common.isEmpty(item.supplierReceivingInfo.trim())
                  ? "--"
    ...
    detailsData: {
      handler(newVal, oldVal) {
        if (newVal) {
          ...
          // }
          this.receivedProApplyForm = this.$common.copy(
            receivedProApplyForm
          );
          ...
          this.settlementMethodArr = this.$common.copy(
            newVal.settlementMethodArr
          );
          this.wmsGoodsStockApplyLogList = this.$common.copy(
            receivedProApplyForm.wmsGoodsStockApplyLogList
          );
          this.oldWmsGoodsStockApplyLogList = this.$common.copy(
            receivedProApplyForm.wmsGoodsStockApplyLogList
          );
          this.searchTable();
        }
  ...
  methods: {
    ...
    editDeatils(type) {
      ...
      this.receivedProApplyForm.applyTime = this.$common.isEmpty(
        this.receivedProApplyForm.applyTime
      )
        ? this.receivedProApplyForm.applyTime
        : this.formatTime(this.receivedProApplyForm.applyTime);
      // this.receivedProApplyForm.tallyTime = this.$common.isEmpty(
      //   this.receivedProApplyForm.tallyTime
      // )
      //   ? this.receivedProApplyForm.tallyTime
      //   : this.formatTime(this.receivedProApplyForm.tallyTime, "month");
      this.receivedProApplyForm.sendTime = this.$common.isEmpty(
        this.receivedProApplyForm.sendTime
      )
        ? this.receivedProApplyForm.sendTime
        : this.formatTime(this.receivedProApplyForm.sendTime);
      this.$refs.receivedProApplyForm.validate((valid) => {
        if (valid) {
          ...
          type === "save" ? (obj.isSave = 1) : null;
          // console.log(obj)
          this.axios
            .post(api.update_stockApply, obj)
            .then((res) => {
              if (res.data.code == 0) {
                this.$Message.success("操作成功");
                this.$emit("goBack", true);
    ...
    confirmReturnApply() {
      ...
      if (this.$common.isEmpty(this.operateReturnReason))
        return this.$Message.error("请输入退回原因");
      this.loading = true;
      this.axios
        .post(api.return_stockApply, obj)
        .then((res) => {
          if (res.data.code == 0) {
            this.$Message.success("操作成功");
            this.$emit("goBack", true);
    ...
    addRemark() {
      if (
        this.$common.isEmpty(this.operationRemark) ||
        this.$common.isEmpty(this.operationRemark.trim())
      )
        return this.$Message.error("备注为空无法添加");
      this.loading = true;
      let obj = {
      ...
      this.axios
        ...
        .then((res) => {
          if (res.data.code === 0) {
            ...
            this.isShowRemark
              ? this.wmsGoodsStockApplyLogList.unshift(remark)
              : null;
    ...
    isShowOperationRemark(val) {
      if (val) {
        this.wmsGoodsStockApplyLogList = this.$common.copy(
          this.oldWmsGoodsStockApplyLogList
        );
      } else {
        this.wmsGoodsStockApplyLogList =
          this.oldWmsGoodsStockApplyLogList.filter(
            (item) => item.logTypeDesc !== "07"
          );
      }
    },
    // 校验
    ...
    showText(type, val) {
      ...
      if (type == 'warehouseId' && val) {
        let list = this.$common.arrayToObj(this.warehouseList, "id");
