...
import orderDetail from '../cne/orderDetail.vue';
...
export default {
    ...
    return {
      ...
      outColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              this.getPermission('wmsGoods_reassociation') || h('Button', {
                props: {
                  size: 'small'
                },
                on: {
  ...
  methods: {
    ...
    getList() {
      ...
      this.checkedDate = []
      this.axios.post(api.get_cneOutBoundOrderList, this.getPageParams()).then(response => {
    ...
    exportFn(obj) {
      this.axios.post(api.exportInventoryOutBoundOrder, obj).then(res => {
        if (res.data.code === 0) {
    ...
    syncAllOrSlt(name) {
      ...
      this.axios.post(api.syncCneOutBoundOrder, obj).then(res => {
        if (res.data.code === 0) {
    ...
    placeAnOrder(name) {
      ...
      this.axios.post(api.batchUploadCnePackage, obj).then(res => {
        if (res.data.code === 0) {
          this.$Message.success('下单成功');
          this.getList()
        } else {
