<template >
  <div class="outboundOrderCommon">
    <div class="listPage" v-if="workShow !== 'detail'">
      <div class="searchMain">
        <Form ref="pageParams" :model="pageParams" :label-width="100">
          <dyt-filter :filter-row="1" ref="dyt-filter">
            ...
            <!-- <Col :xxl="fourItemCol" :xl="threeItemCol" :lg="twoItemCol" :md="oneItemCol"> -->
            <FormItem label="物流方式">
              <Select v-model="pageParams.merchantShippingMethodId" clearable transfer>
            ...
            <Form-item label="LAPA状态">
              <dyt-select v-model="pageParams.ttStatus">
                <Option v-for="(item, index) in cognateSku" :value="item.value" :key="index + 'cognateSku'"
                  :label="item.label">
                </Option>
              </dyt-select>
            </Form-item>
            ...
            <FormItem label="创建时间">
              <Date-picker type="datetimerange" style="width:100%" @on-clear="resetDate" @on-change="getDateValue"
                ...
                placeholder="选择日期" transfer :value="payTimeArr"></Date-picker>
            </FormItem>
            <!-- </Col > -->
            <!-- <Col :xxl="fourItemCol" :xl="threeItemCol" :lg="twoItemCol" :md="oneItemCol"> -->
            <FormItem label="下单成功时间">
              <Date-picker type="datetimerange" style="width:100%" @on-clear="orderResetDate"
                ...
                placement="bottom-end" placeholder="选择日期" transfer :value="payTimeArr"></Date-picker>
            </FormItem>
            <!-- </Col > -->
            <!-- <Col :xxl="fourItemCol" :xl="threeItemCol" :lg="twoItemCol" :md="oneItemCol" > -->
            <FormItem label="物流商发货时间">
              <Date-picker type="datetimerange" style="width:100%" @on-clear="despatchResetDate"
                ...
                format="yyyy-MM-dd HH:mm:ss" placement="bottom-end" placeholder="选择日期" transfer
            ...
            <FormItem label="">
              <div class="select-main">
                <Select v-model="selectType" class="selectStyle" @on-change="selectValue" transfer>
                  <Option v-for="item in selectTypeList" :value="item.value" :key="item.value">{{ item.name }}</Option>
                </Select> <dyt-input-tag :limit="1" :string="true" type="textarea" v-model="textareaList"
            ...
            <Form-item label="同步状态">
              ...
              <dyt-select v-model="pageParams.carrierSendStatus">
                <Option v-for="(item, index) in SynchronizationStatus" :value="item.value"
                  :key="index + 'SynchronizationStatus'" :label="item.label">
                </Option>
              </dyt-select>
            </Form-item>
            ...
            <div slot="operation">
              <Button type="primary" @click="search" :disabled="SearchDisabled" icon="ios-search"
                v-if="getPermission('packageInfo_winitQuery')">查询 </Button>
            </div>
          </dyt-filter>
        </Form>
      </div>
          ...
          <div>
            ...
            <Dropdown>
              <a href="javascript:void(0)">
                <Button type="primary" v-if="getPermission('packageInfo_uploadWinitPackage')"
              ...
              <DropdownMenu slot="list">
                <DropdownItem @click.native="uploadWinitAllPackage"
                  v-if="getPermission('packageInfo_batchUploadWinitPackage')">下单（所有结果集）
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
            ...
            <!--同步发货状态-->
            <Dropdown style="margin-left: 20px;" v-if="getPermission('packageInfo_getWinitPackageStatus')">
              <a href="javascript:void(0)">
                <Button type="primary" @click="ShipmentStatusBtn(checkedDate)">同步发货状态
                  <Icon type="ios-arrow-down" style="margin-left: 10px"></Icon>
                </Button>
              </a>
              <DropdownMenu slot="list" v-if="getPermission('packageInfo_batchWinitGcPackageStatus')">
      ...
      <div class="tableMain">
        <div class="tableBox">
          <Table :columns="columns1" :height="tableHeight" :data="data1" :loading="TableLoading"
            @on-selection-change="checkedDateMt"></Table>
        </div>
      </div>
    ...
    <div v-if="workShow === 'detail'">
      <sellStockOutDtl :ttStatus="ttStatus" :workShow="workShow" :pickingNo="pickingNo" :packageId="activePackageId"
...
export default {
      ...
      columns1: [
        ...
        },
        {
          ...
          render: (h, params) => {
            let text = params.row.ttStatus;
            return h('div', [
              !v.getPermission('wmsPicking_queryWinitSalesPackageDetail') || h('Button', {
                props: {
                  size: 'small'
                },
                on: {
              ...
              }, '查看详情'), !v.getPermission('packageInfo_uploadWinitPackage') || h('Button', {
                props: {
                  size: 'small'
                },
                style: {
  ...
  methods: {
    ...
    uploadWinitAllPackage() {
      ...
      // 包裹-万邑通所有结果集申请运单号
      v.axios.put(api.batchUploadWinitPackage, v.pageParams).then(res => {
        if (res.data.code === 0) {
    ...
    uploadWinitOnePackage(packageId) {
      let v = this;
      v.axios.put(api.set_uploadWinitPackageOne + packageId).then(res => {
        if (res.data.code === 0) {
          v.$Message.success('下单成功');
          v.getList();
        }
    ...
    getList() {
      let v = this;
      if (!v.getPermission('packageInfo_winitQuery')) {
        v.gotoError();
      }
      ...
      v.axios.post(api.set_winitQuery, v.pageParams).then(response => {
    ...
    ShipmentStatusBtn(data) { // 操作同步发货状态
      ...
      v.axios.put(api.put_WinitPackageStatusAll, v.pageParams).then(res => {
        if (res.data.code === 0) {
