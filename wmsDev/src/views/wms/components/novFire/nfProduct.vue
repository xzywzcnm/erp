<template >
  <div>
          ...
          <div class="searchMain">
            <Form ref="pageParams" :model="pageParams" :label-width="labelWidth" inline>
              <dyt-filter ref="dyt-filter">
                <Form-item label="新火 SKU：" prop="skuCodeList">
    ...
    <div class="funMain" style="margin:10px;padding:0">
      <div class="funMain__flex">
        <div class="tableControl">
          <Button style="marginLeft:20px;" v-if="permission.sync" type="primary" @click="syncOnlineProduct">同步商品 </Button>
          <Button type="primary" class="ml10" @click="openInportModal" icon="md-archive"
            :disabled="!permission.import">导入关联 </Button>
    ...
    <importTemp @getList="search" ref="importTemp" :actionUrl="importUrl" :loadTemplateLocalApi="loadTemplateApi"
      :files="'files'" @affirmImport="affirmImport" />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      importUrl: `${api.nfSkuRelatedImport}?warehouseId=${this.getWarehouseId()}`,
            ...
            return h('div', {
              ...
              on: {
                click: () => {
                  ...
                  this.activeWmsProductId = params.row.wmsNfProductId;
                }
              }
  ...
  methods: {
    ...
    getList() {
      ...
      this.axios.post(api.nfQuery, pageParams).then(response => {
    ...
    // 确认导入
    affirmImport() {
      this.axios.post(api.nfSkuRelatedImport, {}).then(response => {
        if (response.data.code === 0) {
    ...
    selectOver(selectRow) {
      // 选择sku后执行的操作
      const obj = {
        wmsNfProductId: this.activeWmsProductId,
        productGoodsId: selectRow.productGoodsId,
        // warehouseId: this.getWarehouseId()
      };
      this.axios.post(api.nfRelatedSku, obj).then(response => {
        if (response.data.code === 0) {
          this.$Message.success('操作成功');
          this.pageParamsStatus = true;
        } else {
    ...
    // 同步在线商品
    syncOnlineProduct() {
      this.axios.get(`${api.nfSync}?warehouseId=${this.wareId}`).then(response => {
        if (response.data.code === 0) {
          this.$Message.success('操作成功');
          this.pageParamsStatus = true;
        } else {
    ...
    exportAllOrSlt(name) {
      ...
      if (name === '0') {
        ...
        obj.wmsOutstoreProductIdList = this.tableSltData.map(val => val.wmsNfProductId);
      } else {
        // 所有
    ...
    exportFn(obj) {
      this.axios.post(api.nfExport, obj).then(res => {
        if (res.data.code === 0) {
    ...
    disassociate(name) {
      ...
      this.axios.post(api.nfCancelRelate, obj).then(response => {
        if (response.data.code === 0) {
          this.$Message.success('操作成功');
          this.pageParamsStatus = true;
        } else {
