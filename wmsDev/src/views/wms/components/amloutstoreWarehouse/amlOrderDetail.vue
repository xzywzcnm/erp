    ...
    <div style="background-color: #fff;margin-bottom: 20px;text-align: center;">
      ...
      <div :style="`width: 800px;margin: 0 auto;height: 74px;`">
        <Steps :current="currentStatus" :status="detailApiObj.carrierPackageStatus === 'INV_ERROR' ? 'error' :'process'">
          ...
          <Step title="已发货"  v-else :content="detailApiObj.despatchTime ? $uDate.getDataToLocalTime(currentIconStatus.despatchTime, 'fulltime') : ''" icon="md-car"></Step><!--已上传物流商-->
        </Steps>
      </div>
    </div>
            ...
            <Row >
              ...
              <Col span="6" v-if="workType === 'otherStock'" >
                <FormItem label="参考编号：" >
                  ...
                  <span >{{ mainInfo.referenceNum }}</span > <span style="margin-left:10px;" >{{
                    mainInfo.shipmentID
                  }}</span >
                </FormItem >
              </Col >
...
export default {
  ...
  methods: {
    ...
    searchData () {
      ...
      this.axios.post(api.queryCneSalesPackageDetail, obj).then(res => {
        if (res.data.code === 0) {
