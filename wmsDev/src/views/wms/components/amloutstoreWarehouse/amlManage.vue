        ...
        <div class="card-content searchMain">
          <Form :model="pageParams" :inline="true" :label-width="100" ref="pageParams">
            <dyt-filter ref="dyt-filter">
              <Form-item label="艾姆勒 SKU：" prop="goodsSkuList">
              ...
              <div slot="operation">
                <Button style="margin-left:90px;" type="primary" icon="ios-search" :disabled="SearchDisabled"
    ...
    <div class="funMain" style="margin:10px;padding:0">
      <div class="funMain__flex">
        <div class="tableControl">
          <Button style="margin-left:20px;" type="primary" v-if="permission.sync" @click="syncInventory">同步库存
          </Button>
...
export default {
  ...
  methods: {
    ...
    // 同步库存
    syncInventory() {
      this.axios.get(`${api.amlInventorySync}?warehouseId=${this.wareId}`).then(response => {
    ...
    getList() {
      ...
      this.axios.post(api.amlInventoryQuery, params).then(response => {
    ...
    exportFn(obj) {
      this.axios.post(api.amlInventoryExport, obj).then(res => {
        if (res.data.code === 0) {
