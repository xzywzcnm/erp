<template >
  <div class="outboundOrder">
    <div v-show="workShow !== 'detail'" class="listPage">
...
import orderDetail from './amlOrderDetail.vue';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.checkedDate = []
      this.axios.post(api.amlOutboundQuery, this.getPageParams()).then(response => {
    ...
    exportFn(obj) {
      this.axios.post(api.amlOutboundExport, obj).then(res => {
        if (res.data.code === 0) {
    ...
    syncAllOrSlt(name) {
      ...
      this.axios.post(api.amlOutboundPackageStatus, obj).then(res => {
        if (res.data.code === 0) {
    ...
    placeAnOrder(name) {
      ...
      this.axios.post(api.amlOutboundPackage, obj).then(res => {
        if (res.data.code === 0) {
          this.$Message.success('下单成功');
          this.getList()
        } else {
