  ...
  <Dropdown @on-click="itemChange" >
    ...
    <DropdownMenu slot="list" v-if="!listenNormal" >
      <DropdownItem
          ...
          :name="v.value"
          :key="i"
          v-if="v.flagCode ? v.flagCode.indexOf(v.status) > -1 : ''" >{{ v.code }}
      </DropdownItem >
    </DropdownMenu >
    <DropdownMenu slot="list" v-if="listenNormal" >
      <DropdownItem v-for="(v, i) in dropList" :name="v.value" :key="i" >{{ v.code }}</DropdownItem >
    </DropdownMenu >
  </Dropdown >
</template >
