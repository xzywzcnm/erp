<template >
  <div class="stockOutDlt">
    ...
    <div style="background-color: #fff;margin-bottom: 20px;text-align: center;">
      ...
      <div :style="`width: 800px;margin: 0 auto;height: 74px;`">
        <Steps :current="currentStatus" :status="detailApiObj.carrierPackageStatus === 'INV_ERROR' ? 'error' : 'process'">
          <Step title="等待下单" icon="md-cart"
            :content="detailApiObj.createdTime ? $uDate.getDataToLocalTime(detailApiObj.createdTime, 'fulltime') : ''">
          </Step>
          <!--已创建-->
          <Step title="下单成功" icon="md-cash"
            :content="detailApiObj.trackingNumberTime ? $uDate.getDataToLocalTime(detailApiObj.trackingNumberTime, 'fulltime') : ''">
          </Step><!--已配货-->
          <Step title="配货异常" v-if="detailApiObj.carrierPackageStatus === 'INV_ERROR'" icon="md-close-circle"></Step>
          <Step title="已发货" v-else
            :content="detailApiObj.despatchTime ? $uDate.getDataToLocalTime(currentIconStatus.despatchTime, 'fulltime') : ''"
            icon="md-car"></Step><!--已上传物流商-->
        </Steps>
      </div>
    </div>
            ...
            <Row>
              ...
              <FormItem label="订单号：">
                ...
                <template v-if="mainInfo.packageOrderBoList.length" v-for="(item, index) in mainInfo.packageOrderBoList">
                  ...
                  }}</span> <span v-else :key="`else-${index}`">{{
  item.salesRecordNumber
        ...
        <TabPane label="收件人信息" name="receiver">
          <Form :label-width="100">
            <Row>
              <Col span="6">
              <FormItem label="收件人名称：">
        ...
        <TabPane label="物流商信息" name="logistic">
          <Form :label-width="100">
            ...
            <Row>
              <Col span="24">
              <FormItem label="物流面单：">
                <a href="javascript:;" target="_blank" @click="downLink">{{ logister.labelName }}</a>
              </FormItem>
              </Col>
            </Row>
          </Form>
    ...
    <div class="clear p20">
      <Button class="fr" type="primary" v-if="ttStatus !== '下单成功' &&
    ...
    <!-- 操作日志 -->
    <commonDaily :operationLog="optionNoteData" @showNoteFunc="showNoteFunc" from="outStock"
...
import commonDaily from '@/views/wms/components/common/commonDaily';
...
export default {
  ...
  computed: {
    ...
    currentStatus() {
      // if (this.detailApiObj.packageStatus !== 7) {
      //   if (this.detailApiObj.carrierPackageStatus === 'INV_ERROR') {
      //     return 2
      //   } else {
      //     if (this.detailApiObj.trackingNumberStatus === 1 || this.detailApiObj.trackingNumberStatus === 2 || this.detailApiObj.trackingNumberStatus === 4) {
      //       return 0;
      //     } else if (this.detailApiObj.trackingNumberStatus === 3) {
      //       return 1;
      //     }
      //   }
      // } else {
      ...
      if (this.detailApiObj.packageStatus === 7) {
        ...
        if (this.detailApiObj.carrierPackageStatus === 'INV_ERROR') {
          return 2;
        } else if (this.detailApiObj.trackingNumberStatus === 1 || this.detailApiObj.trackingNumberStatus === 2 || this.detailApiObj.trackingNumberStatus === 4) {
          return 0;
        } else if (this.detailApiObj.trackingNumberStatus === 3) {
          return 1;
        }
  ...
  methods: {
    ...
    searchData() {
      ...
      this.axios.post(api.rinid_wmsRinidReceiptDetail, obj).then(res => {
        if (res.data.code === 0) {
          ...
          this.logister.logisticNum = resData.referenceNo1;
          this.logister.sendType = resData.carrierShippingMethodName;
          this.logister.trackingNum = resData.trackingNumber;
          this.logister.linkman = resData.logisticsLinkMan;// 无
          this.logister.tel = resData.logisticsLinkTel; // 无
          this.logister.email = resData.buyerPostalCode;
          let labelName = null;
          if (resData.labelPath) {
            let lastOf = resData.labelPath.lastIndexOf('/');
            labelName = resData.labelPath.substr(lastOf + 1);
          }
          this.logister.labelName = labelName;
          this.logister.labelPath = resData.labelPath;
    ...
    downLink() {
      let url = this.$common.fixHeadUrl(this.logister.labelPath);
      window.open(url);
    },
  }
};
