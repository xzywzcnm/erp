<template >
  <div class="outboundOrder">
    <div v-if="workShow !== 'detail'" class="listPage">
        ...
        <Form ref="pageParams" :model="pageParams" :label-width="100" class="formFlex">
          <dyt-filter ref="dyt-filter">
            <FormItem label="订单号" prop="orderNumberList">
              <dyt-input-tag :limit="1" type="textarea" v-model.trim="pageParams.orderNumberList"
                placeholder="多个请用逗号或回车分隔" />
            </FormItem>
            <FormItem label="物流方式" prop="merchantShippingMethodIdList">
              <dyt-select multiple v-model="pageParams.merchantShippingMethodIdList">
            ...
            <Form-item label="包裹状态" prop="statusList">
              <dyt-select multiple v-model="pageParams.statusList">
                <Option v-for="(item, index) in cognateSku" :value="item.value" :key="index + 'statusList'"
                  :label="item.label" />
              </dyt-select>
            </Form-item>
            ...
            <Form-item label="出库单号" prop="packageCodeList">
              <dyt-input-tag :limit="1" type="textarea" v-model.trim="pageParams.packageCodeList"
                placeholder="多个请用逗号或回车分隔" />
            </Form-item>
      ...
      <div class="funMain">
        <div class="funMain__flex">
          <div>
            <!-- 下单 -->
            <!-- <Dropdown @on-click="placeAnOrder" class="ml10" v-if="getPermission('wmsReceipt_order')">
            ...
            <!--导出-->
            <Dropdown @on-click="exportAllOrSlt" class="ml10" v-if="getPermission('wmsReceipt_export')">
            ...
            <!-- 同步状态 -->
            <Dropdown @on-click="syncAllOrSlt" class="ml10" v-if="getPermission('wmsReceipt_synchronization')">
    ...
    <!-- 下单弹窗 -->
    <Modal v-model="placeOrder.visible" title="下单" width="600">
      <Form ref="formRefDemo" label-position="right" :model="placeOrder.filterData" :rules="placeOrder.formRule"
        :label-width="80" class="form-contain">
        <Form-item label="跟踪号" prop="rinidTrackingNumber">
          <dyt-input placeholder="请输入跟踪号" v-model="placeOrder.filterData.rinidTrackingNumber" />
        </Form-item>
        <Form-item label="面单" prop="rinidLabelUrl">
          <dytUpload ref="excelUploadRef" name="files" :show-upload-list="false" accept=".pdf" :action="uploadFilesUrl"
            :before-upload="uploadBefore" :on-success="uploadSuccess" :on-error="uploadError"
            :on-format-error="fileFormatError">
            <div class="upload-icon">
              <Button type="primary">点击上传面单</Button>
            </div>
            <Spin v-if="placeOrder.uploadLoading" fix></Spin>
          </dytUpload>
          <div class="flexCenter" v-if="placeOrder.fileName">
            <a href="javascript:;" target="_blank" @click="downLink">{{ placeOrder.fileName }}</a>
            <Icon type="ios-close-circle" class="ml20 closeCircle" @click="clearFile" />
          </div>
        </Form-item>
      </Form>
      <div slot="footer" style="text-align:right;">
        <Button @click="placeOrder.visible = false">取消</Button>
        <Button type="primary" @click="handerPlace">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      placeOrder: {
        ...
        formRule: {
          rinidTrackingNumber: [
            { required: true, validator: this.$common.strMaxVerify, maxLength: 150, msg: '请输入跟踪号', trigger: 'change' },
            { required: true, validator: this.$common.strMaxVerify, maxLength: 150, msg: '请输入跟踪号', trigger: 'blur' }
          ],
          rinidLabelUrl: [
            { required: true, message: '请上传文件', trigger: 'change', type: 'string' },
          ]
        }
      },
      ...
      outColumns: [
        ...
        },
        {
          ...
          minWidth: 110,
          render: (h, params) => {
            if (params.row.orderNumberList) {
              return h('p', params.row.orderNumberList);
            }
            return h('p', '')
          }
        },
        ...
        },
        {
          ...
          key: 'status',
          render: (h, { row }) => {
            if (this.$common.isEmpty(row.status)) return h('span', '');
            const info = this.cognateSku.find(f => f.value === Number(row.status) || f.label === row.status);
            if (this.$common.isEmpty(info)) return h('span', row.status);
            return h('span', info.label);
          }
        },
        {
          title: '异常原因',
          key: 'packageFailReason',
          align: 'center',
          minWidth: 120,
          className: 'tooltip-col',
          render: (h, { row }) => {
            const text = row.packageFailReason || '';
          ...
          render: (h, { row }) => {
            ...
            if (this.getPermission('wmsReceipt_detail')) {
              item.push(h('Button', {
                ...
                on: {
                  click: () => {
                    this.pickingNo = row.packageCode;
                    this.workType = 'sellStock';
                    this.workShow = 'detail';
                    this.ttStatus = row.status;
                    this.activePackageId = row.packageId;
                  }
                }
              }, '查看详情'));
            }
            let isShow = false;
            if (!this.$common.isEmpty(row.status)) {
              const info = this.cognateSku.find(f => f.value === Number(row.status) || f.label === row.status);
              if (!this.$common.isEmpty(info) && ![3, 7, 99].includes(info.value)) {
                isShow = true;
              }
            }
            if (this.getPermission('wmsReceipt_order') && isShow) {
              item.push(h('Button', {
                props: { size: 'small', type: 'primary' },
                style: { padding: '0 5px !important', 'margin-left': '5px' },
                on: {
                  click: () => {
                    this.openPlaceModal(row);
                  }
                }
              }, '下单'));
            }
  ...
  methods: {
    ...
    getList() {
      if (!this.getPermission('wmsReceipt_inquire')) {
        return this.$Message.error('没有权限')
      }
      this.TableLoading = true;
      this.SearchDisabled = true;
      this.checkedDate = []
      this.axios.post(api.rinid_wmsRinidPackageInfo, this.getPageParams()).then(response => {
    ...
    exportAllOrSlt(name) {
      if (name === '0') {
        ...
        this.exportFn({
          packageIdList: this.checkedDate.map(val => val.packageId),
          warehouseId: this.getWarehouseId()
        });
        return;
      }
    ...
    exportFn(obj) {
      this.axios.post(api.rinid_exportWmsRinidReceipt, obj).then(res => {
        if (res.data.code === 0) {
    ...
    syncAllOrSlt(name) {
      let obj = {};
      if (name === '0') {
        obj.packageIdList = this.checkedDate.map(val => val.packageId);
      ...
      this.axios.post(api.rinid_synchronousWmsRinidReceipt, obj).then(res => {
        if (res.data.code === 0) {
    ...
    // 下单
    placeAnOrder(name, params) {
      return new Promise((resolve) => {
        let obj = {};
        if (this.$common.isObject(name)) {
          obj = name;
        } else if (name === '0') {
          obj.packageIdList = this.checkedDate.map(val => val.packageId);
          obj.warehouseId = this.getWarehouseId()
        ...
        this.axios.post(api.rinid_batchUploadRinidPackage, obj).then(res => {
          if (res.data.code === 0) {
            this.$Message.success('下单成功');
            this.getList();
            resolve(true);
    ...
    handerPlace() {
      this.$refs.formRefDemo.validate((valid) => {
        if (!valid) return this.$Message.error('请按钮要去填写下单信息');
        this.placeAnOrder({
          packageIdList: [this.placeOrder.row.packageId],
          warehouseId: this.getWarehouseId(),
          ...this.placeOrder.filterData
        }).then(res => {
          if (res) {
            this.placeOrder.visible = false;
          }
        })
      })
    },
    // 上传前
    uploadBefore(file) {
      this.placeOrder.uploadLoading = true;
      this.placeOrder.fileName = file.name;
      return true;
    },
    // 上传成功
    uploadSuccess(res) {
      this.placeOrder.uploadLoading = false;
      if (!res || this.$common.isEmpty(res.code) || res.code != 0) return;
      this.placeOrder.filterData.rinidLabelUrl = res.datas[0];
    },
    // 上传失败
    uploadError() {
      this.placeOrder.uploadLoading = false;
      this.placeOrder.fileName = '';
    },
    // 上传文件格式不对
    fileFormatError() {
      this.placeOrder.uploadLoading = false;
      this.placeOrder.fileName = '';
      this.$Message.error('文件格式不正确，请上传PDF格式的文件');
    },
    // 打开下单弹窗
    openPlaceModal(row) {
      this.placeOrder.row = this.$common.copy(row);
      this.placeOrder.fileName = '';
      this.placeOrder.filterData = {
        rinidTrackingNumber: '',
        rinidLabelUrl: ''
      ...
      this.$nextTick(() => {
        this.placeOrder.visible = true;
      })
    },
    clearFile() {
      this.placeOrder.fileName = '';
      this.placeOrder.filterData.rinidLabelUrl = '';
      this.$refs['excelUploadRef'].clearFiles();
    },
    downLink() {
      let url = this.$common.fixHeadUrl(this.placeOrder.filterData.rinidLabelUrl);
      window.open(url);
    },
  }
}
