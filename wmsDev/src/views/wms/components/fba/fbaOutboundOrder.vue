          ...
          <dyt-filter ref="dyt-filter" @expand="expand">
            ...
            <Form-item label="LAPA状态">
              <dyt-select v-model="pageParams.ttStatus">
                <Option v-for="(item, index) in cognateSku" :value="item.value" :key="index + 'ttStatus'"
                  :label="item.label">
                </Option>
              </dyt-select>
            </Form-item>
            <FormItem label="创建时间">
              <Date-picker type="datetimerange" style="width:100%" @on-clear="resetDate" @on-change="getDateValue"
                ...
                placeholder="选择日期" :value="payTimeArr" transfer></Date-picker>
            </FormItem>
            <FormItem label="下单成功时间">
              <Date-picker type="datetimerange" style="width:100%" @on-clear="orderResetDate"
                ...
                placement="bottom-end" placeholder="选择日期" :value="payTimeArr" transfer>
              </Date-picker>
            </FormItem>
            <FormItem label="物流商发货时间">
              <Date-picker type="datetimerange" style="width:100%" @on-clear="despatchResetDate"
                ...
                format="yyyy-MM-dd HH:mm:ss" placement="bottom-end" placeholder="选择日期" :value="payTimeArr" transfer>
              </Date-picker>
            </FormItem>
            ...
            <Form-item label="同步状态">
              <dyt-select v-model="pageParams.carrierSendStatus">
                <Option v-for="(item, index) in SynchronizationStatus" :value="item.value"
                  :key="index + 'carrierSendStatus'" :label="item.label">
                </Option>
              </dyt-select>
            </Form-item>
            <div slot="operation" v-if="getPermission('packageInfo_outstoreQuery')">
...
import sellStockOutDtl from './orderDetail.vue';
...
export default {
      ...
      columns1: [
        {
          ...
          render: (h, params) => {
            // let text = params.row.ttStatus;
            return h('div', [
              !v.getPermission('wmsPicking_queryOutstoreSalesPackageDetail') || h('Button', {
                props: {
                  size: 'small'
                },
                on: {
  ...
  methods: {
    ...
    getList() {
      ...
      v.axios.post(api.thirdStoreQuery, v.pageParams).then(response => {
