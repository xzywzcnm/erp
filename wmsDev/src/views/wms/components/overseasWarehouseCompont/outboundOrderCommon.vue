  ...
  <div class="outboundOrderCommon">
    ...
    <div class="listPage" v-if="workShow !== 'detail'">
      <div class="searchMain">
        <Form ref="pageParams" :model="pageParams" inline :label-width="100">
          <dyt-filter :filter-row="1" ref="dyt-filter" @expand="expand">
            ...
            <div slot="operation">
              ...
              <Button type="primary" @click="search" :disabled="SearchDisabled" icon="ios-search"
                v-if="getPermission('packageInfo_outstoreQuery')" class="mr10">查询
              </Button>
            </div>
          ...
          <div>
            <Dropdown>
              <a href="javascript:void(0)">
                <Button type="primary" v-if="getPermission('packageInfo_uploadOutstorePackage')"
              ...
              <DropdownMenu slot="list">
                <DropdownItem @click.native="uploadWinitAllPackage"
                  v-if="getPermission('packageInfo_batchUploadOutstorePackage')">下单（所有结果集）
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
...
export default {
    ...
    return {
      ...
      columns1: [
        ...
        },
        {
          ...
          minWidth: 90,
          render: (h, { row }) => {
            const countryInfo = v.formValidate.country.filter(i => i.twoCode === row.buyerCountryCode)[0];
            if (this.$common.isEmpty(countryInfo)) return h('span', {}, '');
            const text = countryInfo.cnName
            return h('span', {}, text);
          }
        },
        {
  ...
  methods: {
    ...
    uploadWinitAllPackage() {
      ...
      // 包裹-所有结果集申请运单号
      v.axios.put(v.batchUploadPackageApi, v.pageParams).then(res => {
        if (res.data.code === 0) {
    ...
    uploadWinitOnePackage(packageId) {
      let v = this;
      v.axios.put(v.uploadPackageApi + packageId).then(res => {
        if (res.data.code === 0) {
          v.$Message.success('下单成功');
          v.getList();
        }
    ...
    getList() {
      ...
      v.axios.post(v.getListApi, v.pageParams).then(response => {
    ...
    getDetailData(id) {
      ...
      v.axios.post(v.getDetailApi, obj).then(response => {
        if (response.data.code === 0) {
          let data = response.data.datas;
    ...
    ShipmentStatusBtn(data) { // 操作同步发货状态
      ...
      v.axios.put(v.syncSendPackageApi, v.pageParams).then(res => {
        if (res.data.code === 0) {
