    ...
    <div style="background-color: #fff;margin-bottom: 20px;text-align: center;">
      ...
      <div :style="`width: 800px;margin: 0 auto;height: 74px;`">
        <Steps :current="currentStatus">
          <Step title="等待下单" icon="md-cart" :content="detailApiObj.createdTime"></Step>
          <!--已创建-->
          <Step title="下单成功" icon="md-cash" :content="detailApiObj.trackingNumberTime"></Step><!--已配货-->
          <Step title="已发货"  :content="currentIconStatus.despatchTime" icon="md-car"></Step><!--已上传物流商-->
        </Steps>
      </div>
    </div>
            ...
            <Row >
              ...
              <Col span="6" v-if="workType !== 'otherStock'" >
                <FormItem label="订单号：" >
                  ...
                  <template
                      v-if="mainInfo.packageOrderBoList.length"
                      v-for="(item, index) in mainInfo.packageOrderBoList" >
        ...
        <TabPane label="物流商信息" name="logistic" >
          <Form :label-width="100" >
            ...
            <Row>
              <Col span="6" >
                <FormItem label="承运商：">
                  <span >{{ logister.tongtoolCarrierName }}</span>
                </FormItem>
              </Col>
            </Row>
            <!-- <Row>
    ...
    <div class="clear p20" >
      <Button
          ...
          ttStatus !== '下单成功' &&
            ...
            getPermission('packageInfo_uploadOutstorePackage')
...
export default {
  ...
  props: ['pickingNo', 'workType', 'packageId', 'ttStatus', 'warehouseName'],
  mixins: [Mixin],
  created () {
    this.getCountrys();
    this.searchData();
      ...
      boxColumns: [
        {
          ...
          key: 'updatedTime',
          render: (h, params) => {
            if (this.$common.isEmpty(this.warehouseName) || !ignoreWarehouse.includes(this.warehouseName)) {
              return h('div', this.$uDate.getDataToLocalTime(params.row.updatedTime));
            }
    ...
    searchData () {
      ...
      this.axios.post(api.queryWinitSalesPackageDetail, obj).then(res => {
        ...
        if (res.data.code === 0) {
          ...
          if (this.$common.isEmpty(this.warehouseName) || !ignoreWarehouse.includes(this.warehouseName)) {
            this.mainInfo.createTime = this.$uDate.getDataToLocalTime(resData.createdTime);
            this.mainInfo.lastModifyTime = this.$uDate.getDataToLocalTime(resData.updatedTime);
          } else {
            this.mainInfo.createTime = resData.createdTime;
            this.mainInfo.lastModifyTime = resData.updatedTime;
          }
          ...
          this.logister.tongtoolCarrierName = resData.tongtoolCarrierName;
          ...
          // 查询状态
          this.axios.get((this.workType === 'sellStock' ? api.getSalesPickingStatus : api.getOtherPickingStatus) + this.pickingNo).then(res => {
            if (res.data.code === 0) {
              // this.currentIconStatus
              let obj = this.$common.copy(res.data.datas);
              if (this.$common.isEmpty(this.warehouseName) || !ignoreWarehouse.includes(this.warehouseName)) {
                obj.createdTime = obj.createdTime ? this.$uDate.getDataToLocalTime(obj.createdTime) : '';
                obj.despatchTime = obj.despatchTime ? this.$uDate.getDataToLocalTime(obj.despatchTime) : '';
                obj.pickingGoodsTime = obj.pickingGoodsTime ? this.$uDate.getDataToLocalTime(obj.pickingGoodsTime) : '';
                obj.pickingTime = obj.pickingTime ? this.$uDate.getDataToLocalTime(obj.pickingTime) : '';
                obj.printTime = obj.printTime ? this.$uDate.getDataToLocalTime(obj.printTime) : '';
                obj.getLabelTime = obj.getLabelTime ? this.$uDate.getDataToLocalTime(obj.getLabelTime) : '';
              }
