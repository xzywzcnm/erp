  ...
  <div class="stockOutDlt">
    ...
    <div style="background-color: #fff;margin-bottom: 20px;text-align: center;">
      ...
      <div :style="`width: 800px;margin: 0 auto;height: 74px;`">
        <Steps :current="currentIconStatus.salesPickingStatus">
          <Step :title="item.label" :icon="item.icon" :content="currentIconStatus[item.params]"
            v-for="(item, index) in stepList" :key="index"></Step>
        </Steps>
      </div>
    </div>
...
export default {
  ...
  props: ['pickingNo', 'workType', 'packageId'],
  mixins: [Mixin],
  created() {
    this.getCountrys();
    this.searchData();
  ...
  methods: {
    ...
    searchData() {
      ...
      this.axios.post(api.queryOutstoreSalesPackageDetail, obj).then(res => {
        if (res.data.code === 0) {
            ...
            : api.getOtherPickingStatus) + this.pickingNo).then(res => {
              if (res.data.code === 0) {
                ...
                this.stepList.forEach((item, key) => {
                  obj[item.params] = obj[item.params] ? this.$uDate.dealTime(obj[item.params]) : '';
                  if (obj[item.params]) step = key;
                })
                this.currentIconStatus = obj;
                this.currentIconStatus.salesPickingStatus = step;
