    ...
    <template v-if="curShow === 'managentPage'">
              ...
              <Form ref="pageParams" :model="pageParams" :label-width="90">
                <dyt-filter>
                  <FormItem label="运费核对编号" prop="freightCheckNoList">
                    <dyt-input-tag :limit="1" type="textarea" v-model="pageParams.freightCheckNoList"
                      placeholder="多个请用逗号或回车分隔" />
                  </FormItem>
                  <FormItem label="物流单号" prop="logisticsNoList">
                    <dyt-input-tag :limit="1" type="textarea" v-model="pageParams.logisticsNoList"
                      placeholder="多个请用逗号或回车分隔" />
                  </FormItem>
                  <FormItem label="采购单号" prop="purchaseNumberList">
                    <dyt-input-tag :limit="1" type="textarea" v-model="pageParams.purchaseNumberList"
                      placeholder="多个请用逗号或回车分隔" />
                  </FormItem>
                  ...
                  <FormItem label="记账月份:" prop="startFreightMonth">
                    <DatePicker transfer type="month" placeholder="选择月份" style="width: 100%"
                      v-model="pageParams.startFreightMonth" @on-clear="resetDate('startFreightMonth')" format="yyyy-MM"
                      @on-change="(e) => {
                  ...
                  <FormItem label="采购人:" prop="purchaserIdList">
                    <dyt-select v-model="pageParams.purchaserIdList" :limit="1" multiple clearable filterable>
                  ...
                  <FormItem label="所属事业部:" prop="businessDeptIdsList">
                    <dyt-select v-model="pageParams.businessDeptIdsList" clearable filterable>
                  ...
                  <FormItem label="运费承担方退/抵形式:" prop="freightBearInfo" :label-width="150">
                    <dyt-select v-model="pageParams.freightBearInfo" clearable placeholder="请选择">
                      <Option v-for="(item, index) in freightCarrierMethodList" :value="item.value" :key="index">{{
                        item.label }}
                      </Option>
                    </dyt-select>
                  </FormItem>
                  <div slot="operation">
                    <Button type="primary" @click="search" icon="ios-search" class="mr10 ml20">查询</Button>
      ...
      <div class="flexBox spaceBetween">
        <div class="tableControl mb10" style="margin-left: 12px">
          <Button type="primary" class="mr10" @click="addCheck"
            v-if="getPermission('supplierFreightCheck_created')">新增</Button>
          <!-- <Button type="primary" class="mr10" @click="batchReview()" v-if="getPermission('supplierFreightCheck_batchSummit') &&
            (curTab === '1' || curTab === '2')
            ">审核</Button> -->
          <Dropdown @on-click="exportBtn" v-if="getPermission('supplierFreightCheck_exportList')">
            <Button type="primary">
          ...
          <Button type="primary" class="ml10" v-if="getPermission('supplierFreightCheck_import') && isShowImport"
            @click="importExcel = true">运费导入</Button>
        </div>
        <div class="mr10">
          <span class="mr20 importStyle" v-if="getPermission('supplierFreightCheck_importList')"
            @click="openImportDataModal">已导入运费</span>
      ...
      <Modal v-model="importExcel" title="导入模板" @on-ok="importExcel = false" class-name="verticalCenterModal"
        ...
        <template v-if="uploadList.length == 0 ||
          ...
          <div style="text-align: center; margin: 20px 0">
            <dyt-upload ref="import" type="drag" :format="['XLS', 'XLSX']" :action="action" name="file" :headers="headObj"
              :data="{ warehouseId: this.getWarehouseId() }" :on-success="okUpload" :on-error="uploadError"
        ...
        <div class="subLayer infoDetails">
          <div class="topper">
            ...
            <div style="margin-left: 130px; position: absolute">
              <Button type="primary" class="mr10" v-if="getPermission('supplierFreightCheck_summit')"
                @click="saveOrSubmit('submit')">提交</Button>
              <!-- <Button type="default" class="mr10" v-if="getPermission('supplierFreightCheck_save')"
                @click="saveOrSubmit('save')">暂存</Button> -->
            </div>
          </div>
          <div class="mainContent">
            <addCheckShippingCosts ref="addCheckShippingCosts" :purchaserArr="purchaserArr" :supplierArr="supplierArr"
              @goBack="goBack"></addCheckShippingCosts>
          </div>
        </div>
      </template>
      <template v-if="curShow === 'editCheck'">
        <div class="subLayer infoDetails">
          <div class="topper">
            ...
            <div style="margin-left: 130px; position: absolute" v-if="this.detaildsData.type === 'edit'">
              <Button type="primary" class="mr10" v-if="getPermission('supplierFreightCheck_summit')"
                @click="editPageBtn('submit')">{{ detaildSubmitNameMap[this.detaildsData.checkShippingCostForm.status] }}</Button>
              <Button type="primary" v-if="getPermission('supplierFreightCheck_back') &&
                this.detaildsData.checkShippingCostForm.status !== 0
                " class="mr10" @click="editPageBtn('return')">退回</Button>
              <Button type="default" class="mr10" v-if="getPermission('supplierFreightCheck_save') && detaildsData.checkShippingCostForm.status == 0"
                @click="editPageBtn('save')">保存</Button>
            </div>
          </div>
          <div class="mainContent">
            <checkShippingCostsDetails ref="checkShippingCostsDetails" :purchaserList="purchaserArr"
              :supplierArr="supplierArr" :detaildsData="detaildsData" @goBack="goBack"></checkShippingCostsDetails>
          </div>
        </div>
      </template>
    </transition>
    <Modal v-model="cancelData.isVisible" title="确认作废" width="400">
      <Input v-model="cancelReason" placeholder="请输入作废原因" :maxlength="150" type="textarea" :rows="3" show-word-limit />
      <div style="color: red">作废后更为“已作废”状态，流程终止</div>
      <div slot="footer">
        <Button @click="resetCancelData">取消</Button>
        <Button type="primary" @click="cancelApply" :loading="loading">确定</Button>
      </div>
    </Modal>
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            let statusMap = {
              ...
              1: { label: "待审核", color: 'gold' },
              2: { label: "待复审", color: 'volcano' },
  ...
  methods: {
    ...
    getPurchaserList() {
      this.axios.get(api.get_userInfoCommon).then(res => {
        if (res.data.code == 0) {
          ...
          this.purchaserArr = this.purchaserArr.map(item => {
            return {
              ...item,
              name: item.userName
            }
    ...
    getList() {
      if (!this.getPermission("supplierFreightCheck_list")) { return this.$Message.error("暂无权限"); }
      this.selectionArr = [];
      this.loading = true;
      // let searchOptionMap = {
    ...
    getDetails(type, row) {
      if (!this.getPermission("supplierFreightCheck_detail")) { return this.$Message.error("暂无权限"); }
      this.loading = true;
      this.axios
        .get(
    ...
    // 作废订单
    cancelApply() {
      if (this.$common.isEmpty(this.cancelReason)) { return this.$Message.error("请输入作废原因"); }
      this.loading = true;
