    ...
    <Form ref="receivedProApplyForm" class="mt20" :model="receivedProApplyForm" :label-width="120" :rules="ruleValidate">
      ...
      <Row>
        ...
        <FormItem label="仓库:" prop="warehouseId">
          <dyt-select v-model="receivedProApplyForm.warehouseId" disabled clearable filterable>
            <Option v-for="(item, index) in warehouseList" :value="item.warehouseId" :key="index">{{ item.warehouseName }}
            </Option>
          </dyt-select>
        </FormItem>
        </Col>
        ...
        <FormItem label="申请出库明细">
          <div v-if="!receivedProApplyForm.applyDetailFilePath">
            <Upload ref="excelUploadRef" name="files" :show-upload-list="true" accept=".xlsx,.xls"
      ...
      <div class="normalTop">
        <Table :border="true" :columns="goodsColumn" :data="tableData"
          ...
          <template slot-scope="{ index }" slot="returnNumber">
            ...
            >
              <InputNumber v-model="receivedProApplyForm.wmsGoodsStockApplyDetailList[(tableIndex + index)]
                .returnNumber
          ...
          <template slot-scope="{ index }" slot="stockReason">
            <FormItem class="mt20" :label-width="0">
              <Input v-model="receivedProApplyForm.wmsGoodsStockApplyDetailList[(tableIndex + index)]
                .stockReason
...
export default {
      ...
      goodsColumn: [
        {
          ...
          render: (h, params) => {
            ...
            if (
              ...
              spec = params.row.goodsAttributes;
            }
            ...
            return h(
              ...
              [
                ...
                h("div", { class: "ml20" }, [
                  h("div", {}, `SKU：${params.row.goodsSku || "-"}`),
                  h("div", {}, `名称：${params.row.goodsCnDesc || "-"}`),
                  h(
                    "div",
                    {
                      style: {
  ...
  methods: {
    ...
    initAbnormalData() {
      ...
      // let bList = this.$common.isEmpty(this.$store.getters.getBusinessDeptList) ? [] : this.$store.getters.getBusinessDeptList
      //   bList.forEach(item => {
              ...
              this.receivedProApplyForm.wmsGoodsStockApplyDetailList.forEach(
                (m, j) => {
                  if (n.productGoodsId == m.goodsId || n.goodsId == m.goodsId) {
                    // 如果该商品存在，数量累加
                    this.receivedProApplyForm.wmsGoodsStockApplyDetailList[j].returnNumber += n.returnNumber;
                    isExist = true;
                  }
                }
              );
    ...
    submitOrSave(type) {
      this.$refs.receivedProApplyForm.validate((valid) => {
        if (valid) {
          ...
          if (!obj.wmsGoodsStockApplyDetailList.length) { return this.$Message.error("请添加商品"); }
          let authInfo = this.$store.state.authUserInfo;
    ...
    validPrice(rule, value, callback) {
      this.$nextTick(() => {
        ...
        if (!/^\d+(\.\d{1,2})?$/.test(value)) {
          ...
          return callback(new Error("小数位数不能超过两位且不为0"));
        }
        // 校验通过
        callback();
      });
