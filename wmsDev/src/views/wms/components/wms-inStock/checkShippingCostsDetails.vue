      ...
      <div class="card-content mt20">
        ...
        <Row>
          ...
          <FormItem label="采购人:" prop="purchaserId">
            ...
            <div v-else>
              <dyt-select v-model="checkShippingCostForm.purchaserId" :disabled="!(isEdit && isWaitProcess)" clearable
                filterable>
          ...
          <FormItem label="运费承担方 和 退/抵形式:" :label-width="180" prop="freightBearInfo">
            ...
            <div v-else>
              <dyt-select v-model="checkShippingCostForm.freightBearInfo" clearable placeholder="请选择"
                ...
                :value="item.value"
                :key="index"
                :disabled="isDisabledOption && item.value === '累计月结抵扣'">{{ item.label }}
                </Option>
              </dyt-select>
            </div>
          </FormItem>
        ...
        <Row  v-if="isShowFreightFollowUp">
          <Col :span="6">
          <FormItem label="退抵日期:" prop="refundTime">
            <div v-if="(isEdit && isWaitProcess)">
              <DatePicker v-model="checkShippingCostForm.refundTime" type="date"
                @on-change="(e) => { getDateValue(e, 'refundTime') }" placeholder="选择日期" style="width: 270px">
              </DatePicker>
            </div>
            <div v-else>
          ...
          <Col :span="6">
          <FormItem label="退抵金额:" prop="refundAmount">
            <div v-if="(isEdit && isWaitProcess)">
          ...
          <FormItem label="退/抵付款图片:">
            <div v-if="(isEdit && isWaitProcess)">
              <div class="form-pic-item">
                ...
                <Upload v-if="!checkShippingCostForm.imageUrl" ref="imgUploadRef" name="files" :show-upload-list="false"
            ...
            <div class="flexBox" v-else>
              <div class="form-pic-item">
                <div  v-if="checkShippingCostForm.imageUrl">
                  <div class="pic-item-view">
                    <dyt-previewImg :url="`./filenode/s${checkShippingCostForm.imageUrl}`" />
                  </div>
                  <!-- <div slot="content">
                    <img style="width:350px" :src="`./filenode/s${checkShippingCostForm.imageUrl}`" />
                  </div> -->
              ...
              <Icon type="md-download" v-if="checkShippingCostForm.imageUrl" size="20" class="ml10"
                style="cursor:pointer;margin:auto" @click="downPic()" />
            </div>
          </FormItem>
          </Col>
          <Col :span="6">
          <FormItem label="退抵说明:" prop="refundRemark">
            <div v-if="(isEdit && isWaitProcess)">
              <Input v-model="checkShippingCostForm.refundRemark" type="textarea" :rows="2" :maxlength="200"
                show-word-limit />
            </div>
        ...
        <div>
          ...
          <Row>
            ...
            <Table border :columns="orderLogColumns" :data="remarkDTOList" :loading="loading" max-height="450"></Table>
            </Col>
          </Row>
        </div>
      </div>
    </Form>
    <Modal v-model="returnShow" title="确认退回" width="400" :loading="loading">
      <Input v-model="returnRemark" placeholder="请输入退回原因" :maxlength="200" type="textarea" :rows="3" show-word-limit />
      <div slot="footer">
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        // refundTime: [
        //   { required: this.isCash, message: '请选择日期', trigger: 'change' }
        // ],
        // refundAmount: [
        //   { required: this.isCash, type: 'number', message: '请输入 ', trigger: 'change' }
        // ]
      },
      sendingTypeList: [
        {
  ...
  computed: {
    ...
    businessDeptName() {
      let resultName = ''
      let businessDeptArr = this.$store.getters['allowBusinessDept']
      businessDeptArr.forEach(item => {
        if (item.id == this.checkShippingCostForm.businessDeptId) {
    ...
    addRemark() {
      ...
      this.axios.post(api.add_freightCheckRemark, obj).then(res => {
        if (res.data.code === 0) {
          ...
          this.isShowRemark && this.remarkDTOList.unshift(remark);
