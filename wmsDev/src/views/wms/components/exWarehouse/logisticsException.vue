...
<template>
  <div class="pendingPage">
        ...
        <div class="card-content platformParamsSelect">
          <Form ref="pageParams" :model="pageParams" :label-width="80">
            <dyt-filter>
              ...
              <FormItem label="申报" class="notplatformParamsSelect">
                <Button-group>
    ...
    <div class="clear mt10">
      <div class="fl ml10" v-show="!ymsMerchant">
        <!-- <Button type="primary" @click="voidIssueDocFn"
        ...
        <!-- </span> -->
        <Dropdown @on-click="exportExcel" v-if="multipleExportPower || allExportPower">
          <Button type="primary">导出<Icon type="md-arrow-dropdown"></Icon></Button>
          <DropdownMenu slot="list">
            <DropdownItem v-if="multipleExportPower" name="1">导出选中数据</DropdownItem>
            <DropdownItem v-if="allExportPower" name="2">导出所有结果集</DropdownItem>
          </DropdownMenu>
        </Dropdown>
...
export default {
      ...
      orderColumn: [
        {
          ...
          align: 'center',
          render: (h, params) => {
            // distributionLogisticsStatus: 4 已下发物流异常
            let iconColor = '#ff3300';
            let icon = 'icon iconfont icon-weijiuxu';
            let reason = params.row.distributionLogisticsMsg;
            let status = params.row.distributionLogisticsStatus;
            if (self.$common.isEmpty(status) || status == 4) {
              return h('Tooltip', {
                style: {
                  textAlign: 'center',
                  color: iconColor
                ...
                props: {
                  ...
                  maxWidth: 250,
                  content: reason,
                  disabled: self.$common.isEmpty(reason)
                }
              }, [
                h('div', [
                  h('span', {
                ...
                props: {
                  dropList: [
                    ...
                    // },
                    {
                      ...
                      value: '4',
                      flagCode: ['2'],
                      status: self.getPermission('packageInfo_modifyDimensions') && self.showSizeOperat ? '2' : -1
                    }
                  ],
                  title: '查看出库单详情',
                  listenNormal: false,
  ...
  computed: {
    ...
    multipleExportPower() {
      let powerList = ['distributionLogisticsError_export'];
      return this.returnPower(powerList);
    },
    allExportPower() {
      let powerList = ['distributionLogisticsError_export_all'];
      return this.returnPower(powerList);
    },
  },
  methods: {
    ...
    startLoading() {
      let v = this;
      v.$Loading.start();
      Promise.resolve(v.getPermission('packageInfo_queryForDistributionLogisticsError') ? v.getList()
        : v.gotoError()).then(() => { v.$Loading.finish(); });
    },
    getList() {
      // 获取分页数据
      let v = this;
      ...
      v.axios.post(api.queryForDistributionLogisticsError, JSON.stringify(paramsObj)).then(response => {
        if (response.data.code === 0) {
          ...
          if (data.list !== null) {
            data.list.forEach((n, i) => {
              let temp = v.$common.copy(n);
              temp.buyerCountryCodeList = n.buyerCountryCode;
              temp.carrierName = null;
              temp.carrierShippingMethodName = null;
              // let temp = Object.assign({}, ...n, {
    ...
    returnRequestParams() {
      ...
      // let paramsObj = this.$common.removeEmpty(v.pageParams);
    ...
    exportExcel(name) {
      ...
      this.$Spin.show();
      this.axios
        .post(api.exportForDistributionLogisticsError, temp)
        .then(({ data }) => {
