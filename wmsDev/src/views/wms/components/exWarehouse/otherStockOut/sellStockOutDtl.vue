<template>
  <div class="stockOutDlt">
    ...
    <div style="background-color: #fff; margin-bottom: 20px; text-align: center">
      ...
      <div :style="`width: 1100px;margin: 0 auto;height: 74px;`">
        <Steps v-if="currentIconStatus.createdTime ||
          ...
          " :current="currentIconStatus.otherPickingStatus">
          ...
          <Step title="已拣货" icon="ios-basket" v-if="!currentIconStatus.pickingGoodsTime ||
            ...
            currentIconStatus.pickingGoodsTime >= currentIconStatus.getLabelTime
            " :content="currentIconStatus.pickingGoodsTime"></Step>
          <Step title="已装箱" icon="md-cube" v-if="detailApiObj.needBox === 1" :content="detailApiObj.boxFinishTime"></Step>
    ...
    <div style="background-color: #fff">
      <Tabs value="main" @on-click="tabClick">
        <TabPane label="主信息" name="main">
          <Form :label-width="180">
            ...
            <Row>
              ...
              <Col span="6" v-if="detailApiObj.needBox === 1">
              <FormItem label="装箱状态：">
                <span>{{
        ...
        <!-- <TabPane :label="valAddServiceLabel" name="valAddService" v-if="[4, '4'].includes(mainInfo.status)">
          <valAddService :valAddServiceData="valAddServiceData" @searchData="searchData" />
        </TabPane> -->
      </Tabs>
    </div>
    <!--工具栏-->
    <div class="clear" style="padding: 10px 0" v-if="!ymsMerchant">
      <div class="fl" v-if="(mainInfo.status === '0' || mainInfo.status === '1') &&
        ...
        <Button type="error" v-if="getPermission('wmsPicking_updatePicking') &&
      ...
      <div class="fr">
        <span v-if="detailApiObj.pickingOutStatus !== 2">
          <Button type="primary" v-if="currentIconStatus.otherPickingStatus < 1" @click="assignInventory"
            :loading="waitLoading" :disabled="!getPermission('wmsPicking_modifySalesDistributionSingle') &&
              !getPermission('wmsPicking_modifyDistributionSingle')
              ">分配库存
          </Button>
          <Button type="primary" v-if="currentIconStatus.otherPickingStatus < 1" @click="FreezeAssignmentBtn"
            :loading="waitLoading" style="margin-left: 10px"
            :disabled="!getPermission('wmsPicking_otherPickingDistributionFrozen')">
            冻结分配
          </Button>
          <Button type="primary" :loading="waitLoading" v-if="+currentIconStatus.otherPickingStatus < 2"
    ...
    <div style="margin: 10px 0; text-align: center">
      <Button type="error" style="margin-left: 10px" v-if="getPermission('wmsPicking_obsoletePicking') &&
        mainInfo.status === '0'
        " @click="delObsoletePicking">删除
      </Button>
      <Button type="primary" style="margin-left: 10px" v-if="getPermission('wmsPicking_updatePicking') &&
        (mainInfo.status === '0' || mainInfo.status === '1')
    ...
    <freezeAssignmentModal ref="frozenModal" :frozenModalTalg="frozenModalTalg" @updateData="updateData"
      :deliveryOrder="pickingNo" :mulSearchInput="true"></freezeAssignmentModal>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    //     deliveryListData: this.deliveryListData,
    //   }
    // },
    // completeList() {
    //   let list = this.$common.copy(this.valAddServiceData.deliveryListData || []).map(k => {
    ...
    //     return k.vacuumizeNumber > 0 || k.qualityNumber > 0 || k.replacePackingNumber > 0;
    //   });
    // },
  },
  watch: {
  ...
  methods: {
    ...
    getOrderPrefix(params) {
      ...
      this.axios.post(api.getOrderFront + "?saleAccountIds=" + [params]).then((res) => {
        if (res.data.code === 0) {
          ...
          if (data.length > 0) {
            data[0].accountCode = res.data.datas[0].accountCode;
          }
    ...
    searchData(type) {
      ...
      this.axios.post(api.get_stockOutDtl, obj).then((res) => {
        if (res.data.code === 0) {
                    ...
                    : val.pickingDetailStatus === "2"
                      ...
                      : val.pickingDetailStatus === "4"
                        ...
                        : val.pickingDetailStatus === "5"
                          ? "订单完成"
                          : "";
              val.actualPickingNumber = resData.pickingStatus > 2 ? val.doneAssignedNumber : 0;
            });
      ...
      // 查询状态
      let apiUrl = api.getOtherPickingStatus;
    ...
    FreezeAssignmentBtn() {
      ...
      const flag = JSON.stringify(this.deliveryListData) === JSON.stringify(this.deProductList);
      if (!flag) {
        this.$Modal.confirm({
          title: "温馨提示",
          content: "是否保存并冻结分配",
    ...
    assignFn(params, index) {
      // 分配函数
      this.$Spin.show();
      let apiUrl = api.assign_stockSingle;
      this.axios
        .post(apiUrl, params)
        .then((res) => {
          this.$Spin.hide();
