<template>
  <div>
    <div class="modalItemContent">
      <Form :model="pageParams" :label-width="80" label-position="right" inline>
        <Form-item label="所属库区：">
          <Select
            v-model="pageParams.warehouseBlockId"
            @on-change="blockIdChange"
                ...
                h(
                  "Button",
                  {
                    props: {
                      size: "small",
                      disabled:
                        params.row.availableNumber + params.row.defectiveNumber === 0 ||
  ...
  methods: {
    getWarehouseLocation(query) {
      ...
      return new Promise((resolve) => {
        ...
        v.loading2 = true;
        this.getPositionListNew(["00", "10", "11", "12", "20"], "1", query).then(
          (data) => {
            v.loading2 = false;
            if (query !== "") {
              if (data.length === 0) {
    ...
    searchData: function (flag) {
      ...
      var paramsObj = {
        ...
        // pickingFlag: v.$store.state.pickingType === 'O3' || v.$store.state.pickingType === 'O4' ? '1,3' : '1' // 03 是出库单类型为其他出库 O4 退货
        // pickingFlag: ['O3', 'O4'].includes(v.pickingType) ? '1,3' : '1' // 03 是出库单类型为其他出库 O4 退货
        pickingFlag: v.pickingType == "O4" ? "1,3" : "1", // O4 退货
      };
      paramsObj.warehouseLocationId =
        v.pageParams.wareLocate === "" ? null : v.pageParams.wareLocate;
      if (v.pageParams.warehouseBlockId !== "*") {
        paramsObj.warehouseBlockId =
          v.pageParams.warehouseBlockId === "*" ? null : v.pageParams.warehouseBlockId;
      } // 所属库区
      // if (v.pageParams.yOrNPickLocation !== '*') paramsObj.pickingFlag = (v.pageParams.yOrNPickLocation === '*' ? null : v.pageParams.yOrNPickLocation); // 库位使用
      if (flag === "noPageSearch") {
        paramsObj.pageNum = 1; // 点击查询按钮将页码重置为1
        this.pageParams.pageNum = 1;
      }
      // 库区组
      if (v.warehouseBlockGroupId) {
        paramsObj.warehouseBlockGroupId = v.warehouseBlockGroupId;
      }

      v.TableLoading = true;
      v.SearchDisabled = true;
      v.axios
        .post(api.get_locationInv, paramsObj)
        .then((res) => {
    ...
    getWareLocateListData(obj) {
      ...
      v.axios
        .post(
          this.type === "fromGoodProcess" ? api.stockLocation : api.get_locationInv,
          paramsobj
        )
        .then(({ data }) => {
          if (data.code === 0) {
            let datas = data.datas || {};
            this.data6 = datas.list || [];
            this.totalRecords = datas.total || 0;
            let obj2 = {
              total: datas.total || 0,
              limitMaxTotal: datas.limitMaxTotal,
            };
    ...
    getWareBlockName: function () {
      ...
      v.axios
        .get(
          this.type === "fromGoodProcess"
            ? api.blockList + "?warehouseId=" + this.wareId
            : api.get_wareList + "?warehouseId=" + this.wareId
        )
        .then((res) => {
          ...
          if (v.warehouseBlockGroupId) {
            v.wareBlockList = (data || []).filter((k) => {
              return k.warehouseBlockGroupId === v.warehouseBlockGroupId;
            });
          } else {
      ...
      if (this.type === "fromGoodProcess") {
        str = params
          ? api.locationList +
            ...
            this.wareId +
            "&pickingFlag=1"
          : api.locationList + "?warehouseId=" + this.wareId + "&pickingFlag=1";
      } else {
        str = params
          ? api.get_wareLocationList +
            "?warehouseBlockId=" +
            params +
            "&warehouseId=" +
            this.wareId
