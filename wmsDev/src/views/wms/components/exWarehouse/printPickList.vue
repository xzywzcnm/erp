    ...
    <div class="print-n-box">
      <div id="printBox">
        <div style="
          ...
          <font id="printSkuBarCode" style="font-family: IDAutomationC128S; margin-left: 10px" v-html="barCodeString">
          </font>
        </div>
        ...
        <div style="padding: 20px 0; border-bottom: 1px solid #ccc">
          ...
          <template v-if="type === 'pickList'">
            <div>
              <span>
                ...
                <span v-if="detailInfo.carrierMethodNameList &&
                  detailInfo.carrierMethodNameList.length
                  ">
                  {{
            ...
            <div>
              <span style="display: inline-block; width: 300px; white-space: nowrap">
                <span class="textTitle ab">物流渠道：</span>
                <span v-if="detailInfo.carrierNameList &&
                  detailInfo.carrierNameList.length
                  ">
                  {{
          ...
          <div class="line-block">
            <template v-if="type === 'pickList'">
              ...
              <span style="display: inline-block; width: 180px; white-space: nowrap">
            ...
            <div class="line-block">
              <span class="left-block">
                ...
                <span v-if="detailInfo.platformType">
                  <span v-for="(pk, pi) in detailInfo.platformType.split(',') || []" :key="pi">
        ...
        <table width="100%" style="table-layout: fixed">
          <thead style="background-color: #ccc">
            <tr>
              ...
              <th style="padding: 6px 0" v-if="type === 'checkList' &&
                filterColumns.includes('checkedInventoryNumber')
                ">
                盘点数
              </th>
              <th style="padding: 6px 0" v-if="type === 'checkList' &&
                filterColumns.includes('profitAndLossNumber')
                ">
                差异数
              </th>
            </tr>
          ...
          <tbody>
            <tr v-for="(val, i) in totalItem" :key="i">
              ...
              <td align="center" v-if="type === 'checkList' &&
                filterColumns.includes('checkedInventoryNumber')
                ">
                {{ val.checkedInventoryNumber }}
              </td>
              <td align="center" v-if="type === 'checkList' &&
                filterColumns.includes('profitAndLossNumber')
                ">
                {{ val.profitAndLossNumber }}
              </td>
            </tr>
          </tbody>
        </table>
  ...
  created() {
    ...
    if (this.type === "pickList") {
      paramsObj = {
        isPage: "1",
        status: "1",
        pickingGoodsNos: obj ? obj.split(",") : null,
      };
    ...
    this.axios.post(requestApi, paramsObj).then((res) => {
      if (res.data.code === 0) {
        if (this.type === "checkList") {
          this.detailInfo = this.processObjTime(res.data.datas, [
            "createdTime",
          ]);
        } else {
          this.detailInfo = this.processObjTime(res.data.datas[0], [
            "createdTime",
          ]);
        }
        if (this.type === "checkList") {
          ...
          if (paramsObj.status === "0") {
            ...
            this.totalItem =
              res.data.datas[0].queryByGoodsDetailResultPage.list;
          }
        }
      }
    });
    ...
    cancelPrint() {
      if (this.type === "checkList") {
        ...
        if (this.where === "sell") {
          ...
          window.location.href =
            "#/otherStockOutPickList?warehouseId=" + getWarehouseId();
        }
      }
    },
