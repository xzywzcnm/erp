    ...
    <div class="print-n-box">
      <div id="printBox">
        <div style="page-break-before:always" class="print-mb100" v-for="(detailInfo, index) in batchPrintData"
          :key="index">
          <div
            style="font-size: 24px;font-weight: 600;border-bottom: 1px solid #ccc;padding-bottom: 15px;line-height: 60px;">
          ...
          <div style="padding: 20px 0;border-bottom: 1px solid #ccc">
            <div>
              <span v-if="type === 'pickList'"><span class="textTitle">拣货仓库：</span><span>{{ detailInfo.warehouseName
              }}</span></span>
              <span v-if="type === 'checkList'"><span class="textTitle">盘点仓库：</span><span>{{ detailInfo.warehouseName
              }}</span></span>
            </div>
            <div>
              <span v-if="type === 'pickList'"><span class="textTitle ab">物流商：</span><span>{{
            ...
            <div>
              <span v-if="type === 'pickList'" style="display: inline-block;width: 300px;white-space: nowrap"><span
                  class="textTitle ab">物流渠道：</span><span>{{
            ...
            <div>
              <span style="display: inline-block;width: 250px" v-if="type === 'pickList'"><span
                  class="textTitle">拣货单类型：</span><span>{{
                    detailInfo.packageGoodsType === 'MM'
                    ? '多品'
                    : '单品'
                  }}</span></span> <span v-if="type === 'pickList'" style="display: inline-block;width: 180px"><span
                  class="textTitle ab">出库单数量：</span><span>{{ detailInfo.pickingNumber }}</span></span>
              <span style="display: inline-block;width: 250px" v-if="type === 'checkList'">
                <span class="textTitle">盘点单状态：</span><span>{{
              ...
              <span v-if="type === 'checkList'"><span class="textTitle ab">创建时间：</span><span>{{
                $uDate.getDataToLocalTime(detailInfo.createdTime) }}</span></span>
            </div>
            <div>
              <span style="display: inline-block;width: 250px" v-if="type === 'pickList'"><span
                  class="textTitle">创建时间：</span><span>{{ $uDate.getDataToLocalTime(detailInfo.createdTime, 'fulltime')
                  }}</span></span>
              <span v-if="type === 'pickList'" style="display: inline-block;width: 180px;white-space: nowrap"><span
                  class="textTitle">SKU总数：</span><span>{{ detailInfo.goodsQuantityNumber }}</span></span>
              <span style="display: inline-block;width: 250px" v-if="type === 'checkList'">
                <span style="display: inline-block;width: 250px" v-if="type === 'checkList'"><span
                    class="textTitle">动态盘点：</span><span>{{ detailInfo.beginDate ? 'Y' : 'N' }}</span></span>
                <span v-if="type === 'checkList'"><span class="textTitle">盲盘：</span><span>{{
            ...
            <div>
              <span style="display: inline-block;width: 250px" v-if="type === 'pickList'"><span
                  class="textTitle">拣货人员：</span><span>{{
                    detailInfo.pickingGoodsUser ? detailInfo.pickingGoodsUser : ''
                  }}</span></span>
              <span v-if="type === 'pickList'"><span class="textTitle">SKU种类：</span><span>{{ detailInfo.goodsSkuNumber
              }}</span></span>
              <span style="display: inline-block;width: 250px"
                v-if="type === 'checkList' && detailInfo.checkStatus !== '0'"><span class="textTitle">盘点人员：</span><span>{{
                  detailInfo.checkUserName ? detailInfo.checkUserName : ''
                }}</span></span>
            </div>
          </div>
          <!--表格-->
          <!-- <Table class="iview-table-setWidth" :columns="filterColumns"   :data="detailInfo.totalItem"></Table>-->
          <table width="100%">
...
export default {
  ...
  created() {
    ...
    this.$Spin.show();
    this.axios.post(this.type === 'checkList' ? api.get_scanDetailData : api.get_pickListDtl, paramsObj).then(res => {
      if (res.data.code === 0) {
        let arr = [];
        arr = res.data.datas;
        if (this.type === 'checkList') {
  ...
  methods: {
    ...
    print() {
      let obj = this.$route.query.data;
      this.axios.post(api.printListNum, {
        pickingGoodsNos: obj.split(',')
      }).then(res => {
        if (res.data.code === 0) {
    ...
    cancelPrint() {
      if (this.type === 'checkList') {
        window.location.href = '#/inventoryScan?warehouseId=' + getWarehouseId(); // 盘点单
      } else {
        if (this.where === 'sell') {
