...
<style >
.newInputError .ivu-input,
.newInputError .ivu-select-selection {
  ...
  <div >
    <div class="packageDetailsTitle clear" >
      ...
      <div class="fr normalPaddingRight" >
        <span class="icon iconfont orderBox normalPaddingRight" >&#xe6b2;</span > <span class="normalPaddingRight" >{{
          packageInfo.buyerCountryCode
        }}</span > <span
    ...
    <div class="packageDetailsProccessed" >
      <Steps :current="getCurrent()" >
        <Step
            :title="
            $uDate.getDataToLocalTime(packageInfo.trackingNumberTime, 'fulltime')
          " icon="upload" ></Step >
        <Step :title="$uDate.getDataToLocalTime(packageInfo.printTime, 'fulltime')" icon="md-print" ></Step >
        <Step :title="$uDate.getDataToLocalTime(packageInfo.despatchTime, 'fulltime')" icon="md-paper-plane" ></Step >
      </Steps >
    </div >
    <div class="modalItem" >
      ...
      <div class="modalItemContent" >
        <div class="buyerMessageCotainer" >
          <Row >
            <Col :span="12" >
              <Row >
                <Col :span="1" class="buyerIdName" >ID:</Col >
                <Col :span="23" class="buyerAddress" >
                  <div >{{ packageInfo.buyerAccountId }}</div >
                </Col >
              </Row >
              <Row >
                <Col :span="1" class="buyerAddressTo" >
                  TO:
                </Col >
                <Col :span="23" class="buyerAddress" >
                  ...
                  <div v-if="packageInfo.buyerMobile !== null" >
                    {{ packageInfo.buyerMobile }}
                  </div >
                  <div v-if="packageInfo.buyerPhone !== null" >
                    {{ packageInfo.buyerPhone }}
                  </div >
                  <div >
                    {{ packageInfo.buyerCity }} {{ packageInfo.buyerState }} {{ packageInfo.buyerPostalCode }}
                  </div >
            ...
            <Col :span="10" :offset="2" >
              <div class="logisticsItem" >
                <div class="actuallyLogisticsItemHeader" >
                  实际发货物流方式：<span class="blueColor fontWeight" >{{ packageInfo.merchantCarrierName }} >
                    {{ packageInfo.merchantShippingMethodIdName }}
                  </span >
                </div >
                <div class="actuallyLogisticsItemHeader normalTop" >
                  运单号： <span class="redColor fontWeight " >{{
                    packageInfo.trackingNumber
                  }}</span >
                </div >
                <div class="actuallyLogisticsItemHeader normalTop" >
                  实际重量： <span class="blueColor fontWeight" >{{
                    packageInfo.totalWeight
                  }}</span >
                </div >
                <div class="actuallyLogisticsItemHeader normalTop" >
                  仓库： <span class="blueColor fontWeight" >{{ storeName }}</span >
                </div >
              </div >
            </Col >
          </Row >
    ...
    <div class="modalItem" >
      ...
      <div class="modalItemContent" v-if="packageDetails !== ''" >
        <div class="orderDetailsProductList" >
          <div class="orderDetailsProductItem" v-for="(item, index) in packageDetails" :key="index" >
            <Row >
              <Col :span="2" >
                ...
                <Row >
                  <Col :span="17" >
                    ...
                    <div class="odpidProductParams" v-if="item.variations !== null" >
                      <span
                          class="productHaveATitle" v-if="
                          item.variations.indexOf('color') >= 0 &&
                            item.variations.indexOf(';') > 0
                        " >Color:<em >{{ item.variations.split(': ')[1].split(';')[0] }}
                        </em ></span > <span
                        class="productHaveATitle" v-if="
                          item.variations.indexOf('color') >= 0 &&
                            item.variations.indexOf(';') < 0
                        " >Color:<em >{{ item.variations.split(': ')[1] }}
                        </em ></span > <span
                        class="productHaveATitle" v-if="
                          item.variations.toLowerCase().indexOf('pack of') >= 0
                        " >Pack of:<em >{{
                          item.variations.split(';')[1].split(': ')[1]
                        }}</em ></span >
                    </div >
                  </Col >
                  <Col :span="5" :offset="2" class="orderDetailsQuantity" >
                    <span >X {{ item.quantity }}</span >
                    <div class="flexBox red" v-if="item.remark !== null" >
    ...
    <div class="modalItemTwo" >
      <div class="modalItemHeader" >
        <h6 >申报</h6 >
        <Button
            size="small" type="primary" icon="md-add" class="declareAdd" @click="addDeclare" v-if="
            packageInfo.packageStatus !== 7 &&
              packageInfo.trackingNumberStatus !== 2 &&
              packageInfo.carrierSendStatus !== 2
          " >添加</Button >
        <div
            class="modalItemHeaderRight" v-if="
            packageInfo.packageStatus !== 7 &&
              packageInfo.trackingNumberStatus !== 2 &&
              packageInfo.carrierSendStatus !== 2
          " >
          <p class="setEdit" >
            <span class="icon iconfont" @click="edit()" v-if="!editStatus" >&#xe611;</span >
            <span class="icon iconfont orderSave" @click="editSave()" v-if="editStatus" >&#xe615;</span >
            <span class="icon iconfont orderCancelSave" @click="cancelEdit()" v-if="editStatus" >&#xe61a;</span >
          </p >
        </div >
      </div >
      <div class="declareTable normalTop" >
        <Table :data="declareData" :columns="declareColumns" highlight-row border ></Table >
      </div >
    </div >
  </div >
</template >
...
export default {
  ...
  methods: {
    ...
    getCurrent () {
      let v = this;
      let pos = [
        v.$uDate.getDataToLocalTime(v.packageInfo.trackingNumberTime, 'fulltime'),
        v.$uDate.getDataToLocalTime(v.packageInfo.printTime, 'fulltime'),
        v.$uDate.getDataToLocalTime(v.packageInfo.despatchTime, 'fulltime')
      ];
      let l = 2;
