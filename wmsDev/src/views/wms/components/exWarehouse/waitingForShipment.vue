...
<template>
  <div>
          ...
          <Form ref="pageParams" :model="pageParams" :label-width="80">
            <dyt-filter>
              ...
              <!-- <Form-item label="货主" v-show="!isCargoOwner()">
                <Select v-model="pageParams.cargoOwnerId">
        ...
        <span v-if="tableControlStatus">
          <Buttons type="primary" class="ml10" trigger="click" @on-click="getWayBillHasCondition">
            <Button type="primary" class="iconbuttons" @click="getWayBill"
              :disabled="!getPermission('packageInfo_batchConfirm')">
              <Icon type="cloud"></Icon>交运
            </Button>
            <Buttons-menu slot="list">
              <Buttons-item name="1" v-if="getPermission('packageInfo_batchConfirm')">交运（所有结果集）</Buttons-item>
            </Buttons-menu>
          </Buttons>
        </span>
        <Button type="primary" class="ml10" v-if="getPermission('packageInfo_batchInvalidPackage')"
          @click="voidIssueDocFn">作废出库单
        </Button>
        <!--云仓时，隐藏跳过交运按钮-->
        <Button class="ml10" @click="modifySkipFn" v-if="getPermission('packageInfo_ignoreConfirm') && isYms !== 1"
          type="primary">跳过交运 </Button>
    ...
    <div class="orderTable normalTop">
      <Table highlight-row border v-table-scroll="TableLoading" :columns="orderColumn" :height="tableHeight"
        :data="pendingData" :loading="TableLoading" @on-selection-change="getSelectValue" ref="selection"></Table>
...
export default {
      ...
      orderColumn: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              'span',
              self.$uDate.getDataToLocalTime(params.row.createdTime, 'fulltime')
            );
          }
        },
        {
          ...
          width: 100,
          render: (h, params) => {
            let d = params.row.carrierConfirmStatus;
            let text = '交运失败';
            let icon = 'icon iconfont icon-weijiuxu';
            let iconColor = '#ff3300';
            let reason = params.row.carrierConfirmFailReason;
            if (d === 1) {
              text = '未交运';
          ...
          render(h, params) {
            return h('Tooltip', {
              ...
              props: {
                transfer: true,
                maxWidth: 250,
                content: params.row.carrierConfirmFailReason
              }
            }, [
              h('span', {
                class: 'text-overflow',
                style: { color: '#f00' }
              }, params.row.carrierConfirmFailReason)
            ]);
          }
        }, {
          title: '操作',
                ...
                ? h(
                  compoundBtn,
                  {
                    props: {
                      dropList: [
                        {
                          ...
                          value: '1',
                          flagCode: ['1'],
                          status: self.getPermission('packageInfo_confirm')
                            ? '1'
                            : -1
                        }
                      ],
                      ...
                      disabled: !self.getPermission('packageInfo_confirm')
                    },
                    on: {
                      click: (val) => {
                        if (!val) {
  ...
  computed: {
    ...
    multipleExportPower() {
      let powerList = ['queryForConfirm_export', 'yms_queryForConfirm_export'];
      return this.returnPower(powerList);
    },
    allExportPower() {
      let powerList = ['queryForConfirm_export_all', 'yms_queryForConfirm_export_all'];
      return this.returnPower(powerList);
    },
  },
  methods: {
    ...
    modifySkipFn() {
      ...
      let params = v.tableSltData.map((i) => i.packageId);
      v.$Modal.confirm({
        title: '温馨提示',
        content: '所选出库单将跳过交运环节，操作不可退回，是否确定？',
        onOk: () => {
          v.axios.post(api.update_ignoreConfirm, params).then((response) => {
    ...
    startLoading() {
      ...
      v.$Loading.start();
      Promise.resolve(
        v.getPermission('packageInfo_queryForWaitingUpload') ||
          v.getPermission('packageInfo_queryForTrackingNumberUncatch') ||
          v.getPermission('packageInfo_queryForWaitingGetLabel')
          ? v.getList()
          : v.gotoError()
      ).then(() => {
        v.$Loading.finish();
    ...
    getList() {
      ...
      v.axios.post(api.get_queryForConfirm, JSON.stringify(paramsObj)).then(response => {
        if (response.data.code === 0) {
          ...
          if (data.list !== null) {
            data.list.forEach((n, i) => {
              pos.push({
                ...
                carrierConfirmStatus: n.carrierConfirmStatus,
                carrierConfirmFailReason: n.carrierConfirmFailReason,
    ...
    selectStatus(index, status) {
      ...
      v.pageParams.confirmStatus = status;
    ...
    getWayBill(type, id) {
      ...
      if (type === 'single') {
        v.axios.put(api.get_confirm + id).then((response) => {
          if (response.data.code === 0) {
            v.$Message.success('操作成功');
            v.packageIds = [];
            v.pageParamsStatus = true;
      ...
      v.axios
        .put(api.get_batchConfirm, JSON.stringify(obj))
        .then((response) => {
          if (response.data.code === 0) {
            v.$Message.success('操作成功');
            v.packageIds = [];
    ...
    exportExcel(name) {
      ...
      this.$Spin.show();
      this.axios
        .post(api.exportQueryForConfirm, temp)
        .then(({ data }) => {
