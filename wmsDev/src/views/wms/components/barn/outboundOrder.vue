<template >
  <div class="outboundOrder">
    <div v-if="workShow !== 'detail'" class="listPage">
      <div class="searchMain">
        <Form ref="pageParams" :model="pageParams" :label-width="110">
          <dyt-filter ref="dyt-filter" @expand="expand">
            ...
            <FormItem label="物流方式">
              <dyt-select v-model="pageParams.merchantShippingMethodId">
            ...
            <Form-item label="LAPA状态">
              ...
              <dyt-select v-model="pageParams.ttStatus">
                <Option v-for="(item, i) in cognateSku" :key="i + 'cognateSku'" :value="item.value" :label="item.label">
                </Option>
              </dyt-select>
            </Form-item>
            <FormItem label="创建时间">
              <Date-picker type="datetimerange" @on-clear="resetDate" @on-change="getDateValue" :clearable="clearAble"
                :options="dateOptions" format="yyyy-MM-dd HH:mm:ss" placement="bottom-end" placeholder="选择日期"
                :value="payTimeArr" transfer></Date-picker>
            </FormItem>
            <FormItem label="下单成功时间">
              <Date-picker type="datetimerange" @on-clear="orderResetDate" @on-change="getOrderDateValue"
                :clearable="clearAble" :options="dateOptions" format="yyyy-MM-dd HH:mm:ss" placement="bottom-end"
                placeholder="选择日期" :value="payTimeArr" transfer>
              </Date-picker>
            </FormItem>
            <FormItem label="物流商发货时间">
              <Date-picker type="datetimerange" @on-clear="despatchResetDate" @on-change="getDespatchDateValue"
                :clearable="clearAble" :options="dateOptions" format="yyyy-MM-dd HH:mm:ss" placement="bottom-end"
                placeholder="选择日期" :value="payTimeArr" transfer>
              </Date-picker>
            </FormItem>
            <div style="display: inline-flex;" class="mr10">
              <dyt-select v-model="selectType" class="selectStyle" @on-change="selectValue" :clearable="false">
                <Option v-for="item in selectTypeList" :value="item.value" :key="item.value" :label="item.name">
                </Option>
              </dyt-select>
              <dyt-input-tag :limit="1" :string="true" v-model="textareaList" type="textarea" style="width: 240px;"
                placeholder="支持批量查询，多个单号用逗号或换行分隔" />
            </div>
            <FormItem label="同步状态">
              <!-- <Button-group>
                <Button v-for="d in SynchronizationStatus" :key="d.value" :type="d.checked ? 'primary' : 'default'"
                  @click="changeSynchronizationStatus(d.value)">{{ d.label }} </Button>
              </Button-group> -->
              <dyt-select v-model="pageParams.carrierSendStatus">
                <Option v-for="(item, i) in SynchronizationStatus" :key="i + 'SynchronizationStatus'" :value="item.value"
                  :label="item.label">
                </Option>
              </dyt-select>
            </FormItem>
            ...
            <div slot="operation" v-if="getPermission('packageInfo_gcQuery')">
          ...
          <div>
            ...
            <Dropdown>
              <a href="javascript:void(0)">
                <Button type="primary" v-if="getPermission('packageInfo_batchUploadGcPackage')"
                  @click="uploadBarnPackage(checkedDate)">下单
                  <Icon type="ios-arrow-down" style="margin-left: 10px"></Icon>
                </Button>
              </a>
              <DropdownMenu slot="list">
                <DropdownItem @click.native="uploadBarnAllPackage" v-if="getPermission('packageInfo_uploadGcPackage')">
                  下单（所有结果集）
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
            ...
            <!--同步发货状态-->
            <Dropdown style="margin-left: 20px;" v-if="getPermission('packageInfo_batchGetGcPackageStatus')">
              <a href="javascript:void(0)">
      ...
      <div class="tableMain">
        <div class="tableBox">
          <Table :columns="columns1" :data="data1" :loading="TableLoading" @on-selection-change="checkedDateMt"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      pageParams: {
        ttStatus: '1', // LAPA状态
        carrierSendStatus: '', // 同步状态
        searchValue: '', //
        merchantShippingMethodId: null, // 物流方式
        pageNum: 1,
        pageSize: 10,
        orderBy: 'CT',
        upDown: 'up',
      ...
      columns1: [
        ...
        },
        {
          ...
          render: (h, params) => {
            let text = params.row.ttStatus;
            return h('div', [
              !v.getPermission('wmsPicking_queryGcSalesPackageDetail') || h('Button', {
                props: {
                  size: 'small'
                },
                on: {
              ...
              }, '查看详情'), !v.getPermission('packageInfo_uploadGcPackage') || h('Button', {
                props: {
                  size: 'small'
                },
                style: {
                  display: (() => {
                    return text === '下单成功' || text === '已发货' ? 'none' : 'line-block';
                  })()
                },
                on: {
                  click: () => {
                    v.uploadBarnOnePackage(params.row.packageId);
                  }
                }
              }, '下单'), !v.getPermission('packageInfo_getGcPackageStatus') || h('Button', {
                props: {
                  size: 'small'
                },
                on: {
                  click: () => {
                    v.ShipmentOneStatusBtn(params.row.packageId);
                  }
                }
              }, '同步发货状态')
            ]);
  ...
  methods: {
    ...
    uploadBarnAllPackage() {
      let v = this;
      v.pageParams.orderSeq = v.pageParams.upDown === 'up' ? 'ASC' : 'DESC';
      v.pageParams.packageIds = [];
      v.axios.put(api.batchUploadBarnPackage, v.pageParams).then(res => {
        if (res.data.code === 0) {
    ...
    uploadBarnOnePackage(packageId) {
      let v = this;
      v.axios.put(api.set_uploadBarnPackageOne + packageId).then(res => {
        if (res.data.code === 0) {
          v.$Message.success('下单成功');
          v.getList();
        }
    ...
    getList() {
      let v = this;
      if (!v.getPermission('packageInfo_gcQuery')) {
        v.gotoError();
      }
      ...
      if (v.selectType === 'order') {
        ...
        v.pageParams.specifiedOrderNoList = v.textareaList === '' ? [] : v.sepCommasFn(v.textareaList);
      } else if (v.selectType === 'outbound') {
        v.pageParams.specifiedSkuList = [];
        v.pageParams.specifiedOrderNoList = [];
        v.pageParams.specifiedPackageCodeList = v.textareaList === '' ? [] : v.sepCommasFn(v.textareaList);
      } else if (v.selectType === 'sku') {
        v.pageParams.specifiedOrderNoList = [];
        v.pageParams.specifiedPackageCodeList = [];
        v.pageParams.specifiedSkuList = v.textareaList === '' ? [] : v.sepCommasFn(v.textareaList);
      }

      v.TableLoading = true;
      v.SearchDisabled = true;
      v.axios.post(api.set_barnQuery, v.pageParams).then(response => {
    ...
    getDetailData(id) {
      ...
      v.TableLoading = true;
      v.axios.post(api.get_winitSalesPickingDetail, obj).then(response => {
        v.TableLoading = false;
        if (response.data.code === 0) {
          let data = response.data.datas;
    ...
    ShipmentOneStatusBtn(packageId) {
      let v = this;
      v.axios.put(api.put_GcPackageStatus + packageId).then(res => {
        if (res.data.code === 0) {
    ...
    ShipmentStatusBtn(data) {
      ...
      v.axios.put(api.put_GcPackageStatusAll, v.pageParams).then(res => {
        if (res.data.code === 0) {
          v.$Message.success('操作成功');
          v.pageParams.packageIds = [];
          v.getList();
    ...
    exportAllOrSlt(name) {
      ...
      if (name === '0') {
        ...
        if (obj.ttStatus !== '6') {
          if ([99].includes(Number(obj.ttStatus))) {
            obj.packageProcessStatus = [obj.ttStatus];
          } else {
            obj.trackingNumberStatus = obj.ttStatus;
          }
        } else {
