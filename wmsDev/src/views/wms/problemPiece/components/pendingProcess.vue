<template>
  <div class="qualityInspectionDocument-page">
    <div class="listPage" v-if="tab == 1">
      <div class="searchMain">
        <Form ref="pageParams" :model="pageParams" :label-width="80">
          <dyt-filter ref="document-filter" @expand="expand" :tableClassName="tableBoxClass">
            ...
            <Form-item label="SKU:" prop="skuList">
              <dyt-input-tag :limit="1" type="textarea" v-model="pageParams.skuList" placeholder="多个SKU请用逗号或回车分隔" />
            </Form-item>
        ...
        <div class="funMain__flex">
          <div>
            <Button type="primary" class="mr10" @click="createHandSheet" :disabled="!tableCheckedList.length"
              v-if="getPermission('problemPiece_create_processing')">创建处理单</Button>
      ...
      <div class="tableMain">
        <div class="tableBox" :class="tableBoxClass">
          <Table highlight-row border :loading="tableLoading" @on-selection-change="checkedDataHandel" :columns="columns"
            :data="tableList" ref="selection" :height="tableHeight" v-table-scroll="tableLoading">
    ...
    <!-- 查看质检单 -->
    <inspection-document-detail :dialogVisible.sync="documentInfo.visible" :modalData="documentInfo.data"
      :type="documentInfo.type" @checkSearch="getList" />
    <!-- 质检问题产品转合格产品 -->
    <quality-problem-products :modelVisible.sync="problemInfo.visible" :modalData="problemInfo.data"
      :type="problemInfo.type" />
    <!-- 问题商品处理单 -->
    <problem-good-hand-sheet @submitSuccess="getList" :modelVisible.sync="handSheetInfo.visible" type="create"
      :module-list="handSheetInfo.list" />
  </div>
</template>
<script>
...
import inspectionDocumentDetail from '../../qualityManage/components/inspectionDocumentDetail';
import qualityProblemProducts from '../../qualityManage/components/qualityProblemProducts';
import problemGoodHandSheet from './problemGoodHandSheet';
import { checkStatusList, checkTypeList, readyTypeList } from '../../qualityManage/components/commonData.js';

export default {
  name: "pendingProcess",
  components: {
    inspectionDocumentDetail,
  ...
  methods: {
    ...
    // 获取质检单列表
    getList() {
      let power = this.getPermission('wms_receiptCheckToHandle');
      if (!power) {
    ...
    // 赋值
    dropSetting() {
      let power = this.getPermission('problemPiece_pending_exprotAll');
      this.exportList = [
        {
          label: "导出",
          power: power,
    ...
    transferQualify(row, problemType) {
      ...
      this.problemInfo.type = problemType;
      this.problemInfo.data = Object.assign({}, row, { apiType: 'list' });
