...
const common = {
  ...
  validatorMaxNumber (rule, value, callback) {
    ...
    if (common.isFunction(rule.getRequired)) {
      required = rule.getRequired(rule, value);
    }
  ...
  trim: (obj, exceptionKey) => {
    const exception = common.isString(exceptionKey)
      ? [exceptionKey]
      : exceptionKey || [];
    if ((common.isString(obj) && obj === '') || common.isEmpty(obj)) return obj;
   ...
   * @returns
   */
  flat(arr, type = Infinity) {
    return arr.flat(type);
  },
  // 获取cookie, 支持数组和json
  getCookie(keys) {
  ...
  */
  ...
  splicingPath(pdfUrl) {
    ...
    if (pdfUrl.indexOf("http") >= 0) {
      ...
      let path = pdfUrl.indexOf(prefix) >= 0 ? pdfUrl : prefix + pdfUrl;
      url = window.location.origin + "/wms-service/" + path;
      // url = "http://172.20.200.30/wms-service/" + path;
    }
    return url;
  },
  getBase64(url) {
  ...
  getClipboard() {
    return new Promise((resolve, reject) => {
      ...
      if (common.isEmpty(clipboardObj)) {
        console.error('当前浏览器不支持异步 Clipboard API 或 只有 HTTPS 协议的页面才能使用这个 API!');
        return reject(new Error('当前浏览器不支持异步 Clipboard API 或 只有 HTTPS 协议的页面才能使用这个 API!'));
      }
      clipboardObj.readText().then(clipText => {
        resolve(clipText);
      }).catch(err => {
        console.error(err);
        reject(new Error('无法读取剪贴板内容!'));
      });
    })
  },
  // px 转换为 mm
  ...
  isUrl(str) {
    ...
    if (common.isEmpty(str)) return false;
  ...
  */
  ...
  // 获取设置设置的日期前或后的时间返回(时间范围)
  getRangeDate (number, format, baseTime) {
    const end = common.isEmpty(baseTime) ? new Date() : baseTime;
    let nformat = format;
    const newNumber = common.isEmpty(number) ? -7 : number;
    if (common.isEmpty(nformat) || ['fulltime'].includes(format)) {
      nformat = 'YYYY-MM-DD HH:mm:ss';
    }
    return [common.dayjs(end).add(newNumber, "day").format(nformat), common.dayjs(end).format(nformat)];
  },
  /**
   * base64 转为 Blob
  ...
  // 处理文件路径
  fixHeadUrl(url) {
    if (['', null, undefined].includes(url)) return url;
    let exp = /(http|https):\/\/([\w.]+\/?)\S*/;
    if (exp.test(url)) return url;
    const prefix = 'filenode/s/';
    const path = url.indexOf(prefix) >= 0 ? url : prefix + url;
    return window.location.origin + '/wms-service/' + path;
    // return `http://testing.lapa.com.cn/wms-service/${path}`;
  },
  // 将数组转化为对象
  arrayToObj(arr, key) {
