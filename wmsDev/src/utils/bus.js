...
const AUTHUrl = envHost.auth;
const newTime = new Date().getTime();

const tool = {
  messageKey: mProcess.VUE_APP_BROADCASTKEY,
  broadcastUrl: `/index.html#/broadcastMessage?newTime=${newTime}`,
  linkAuth: 'getAuthInfoKey',
  clearPassTime: null,
  iframeLoadTime: 1000 * 60 * 2, // 测试文件或iframe加载超时时间，单位毫秒
  testLoadedTime: 0, // 测试文件加载时长单，位毫秒
  frequency: 200, // 测试文件或iframe计数频率，单位毫秒
  iframeLoadedTime: 0, // iframe加载时长，单位毫秒
  subscribe: {},
  authSysSub: {},
  // broadcast: window.BroadcastChannel ? new window.BroadcastChannel(`${mProcess.VUE_APP_SYSTEMCODE}-broadcast-channel`) : false,
  // 部分浏览器不支持 window.BroadcastChannel 使用 BroadcastChannel 库， 当不支持 BroadcastChannel 时，采用监听LocalStorage 和 IndexedDB 实现通讯
  broadcast: new BroadcastChannel(`${mProcess.VUE_APP_SYSTEMCODE}-broadcast-channel`),
  isLoaded: 'init',
  isIframeLoad: 'init',
  loadingTime: 0,
  iframeDemo: null,
  homologyMessageKey: [],
  networkTest: `${AUTHUrl}/static/js/networkTest.js?newTime=${newTime}`,
  // 判断是否能连接上中间服务
  isOpenAuth () {
  ...
  createIframe () {
    return new Promise((resolve) => {
      ...
      iframe.src = `${AUTHUrl}${this.broadcastUrl}`;
