...
class PrintTongtu {
  ...
  pdfPrint(res, userQs = '') {
    ...
    return new Promise((resolve, reject) => {
      ...
      postId = (postId || '') + parseInt(Math.random() * 10000000000);// 看通途的代码，是面单id，但是总觉得是每一次打印带过去的标识 所以这里用了随机数代替
      let jsonObj = {};
      if (res.pdf) {
        jsonObj.pdf = url;
      ...
      instance.post('http://localhost:10099/print?', Qs.stringify(jsonObj)).then(response => {
        if (response.status === 200) {
          this.startPrintMonitor(userQs === 'userQs' ? jsonObj : postId, userQs).finally(() => {
            resolve('打印完毕');
          })
        } else {
          this.downloadPrint();
  ...
  downloadPrint() {
    ...
    let tipPopup = (url) => {
      ...
      if (store.state.erpConfig) {
        prefix = store.state.erpConfig.filenodeViewTargetUrl;
      }
      let path = prefix + (url || defaultUrl);
      Modal.info({
        title: '提示',
        okText: '关闭',
        render: (h, params) => {
