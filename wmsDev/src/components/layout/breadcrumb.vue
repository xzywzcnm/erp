<template>
  <!-- 面包屑 -->
  <Breadcrumb>
    <BreadcrumbItem v-for="(item, index) in BreadMenuData"
      :to="item.path ? item.path.includes('/details') ? null : item.path : null" :key="`bre-${index}`">
      {{ item.name }}
    </BreadcrumbItem>
  </Breadcrumb>
</template>
...
export default {
  ...
  methods: {
    ...
    getBreadMenuData() {
      // this.BreadMenuData = this.BreadMenuAllData.filter(i=>i.path === this.$route.path)[0].breadMenu;
      this.BreadMenuAllData.map((item) => {
        if (item.path === this.$route.path) {
          this.BreadMenuData = item.breadMenu;
        }
      });
    }
  },
