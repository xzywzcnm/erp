      ...
      <template v-if="!row.pid && row.pid !== 0" >
        <tr
          ...
          <template v-for="(column, n) in colPos[index]" >
            <td
                :class="alignCls(column, row)" v-if="column.rowSpan && !column.colSpan" :rowSpan="column.rowSpan" >
              <Cell
                  :fixed="fixed"
            ...
            <td
                :class="alignCls(column, row)" v-if="!column.rowSpan && column.colSpan" :colSpan="column.colSpan" >
              <Cell
                  :fixed="fixed"
                  :prefix-cls="prefixCls"
                  :row="row"
...
export default {
  ...
  methods: {
    ...
    makeColSpan (dataIndex, colIndex, colSpan, rowSpan) {
      // 需要合并的tr,第几个td,输入的合并格数
    ...
    showRelated (grid, sIndex) {
      // 实际上是改变status..[grid,sIndex]组ID，节点索引
