<template >
  <div :class="classes" v-clickoutside="handleClose" >
    <div :class="selectionCls" ref="reference" @click="toggleMenu" >
      <slot name="input" >
        <input type="hidden" :name="name" :value="model" >
        <div class="ivu-tag ivu-tag-checked" v-for="(item, index) in selectedMultiple" >
          <span class="ivu-tag-text" >{{ item.label }}</span >
          <Icon type="ios-close" @click.native.stop="removeTag(index,item.nodeKey)" ></Icon >
        </div >
        <span
            :class="[prefixCls + '-placeholder']"
            v-show="showPlaceholder && !filterable" >{{ localePlaceholder }}</span > <span
          :class="[prefixCls + '-selected-value']"
          v-show="!showPlaceholder && !multiple && !filterable" >{{ selectedSingle }}</span > <input
          :id="elementId"
          type="text"
          v-if="filterable"
          v-model="query"
        ...
        <Icon type="md-arrow-dropdown" :class="[prefixCls + '-arrow']" v-if="!remote" ></Icon >
      </slot >
    </div >
    <transition :name="transitionName" >
      <Drop
          ...
          v-transfer-dom >
        <ul
            v-if="filterable && cacheData.length > 0 && cacheData[0].hasOwnProperty('_hide')"
            v-show="cacheData[0]._hide"
...
<script >
import Icon from '../d-Table/relation/icon';
import Drop from './dropdown.vue';
import clickoutside from '../d-Table/relation/directives/clickoutside';
import TransferDom from '../d-Table/relation/directives/transfer-dom';
import {
  oneOf,
  findComponentDownward,
  deepCopy
} from '../d-Table/relation/utils/assist';
import Emitter from '../d-Table/relation/mixins/emitter';
      ...
      return debounce(function () {
        if (!this.remote) {
          ...
          this.findChild((child) => {
            ...
            child.selected = (this.multiple || this.showCheckbox)
