<template >
  <div class="tabInputBox" @click="focusIn" >
    <div
        style="margin: 3px 5px;border: 1px solid #ddd;padding: 2px 10px;
    background-color: #f3f3f3;display: inline-block;
    border-radius: 3px;" v-for="(item, index) in tags" :key="index" v-if="tags.length > 0" :title="item.attrVal" >
      <span class="tabText" >{{ item.attrVal }}</span >
      <Icon
          v-show="!disabled" type="android-close" @click="delTag(index)" class="tagClose" size="14" ></Icon >
    </div >
    <Input
        v-show="!disabled"
        class="tagInput"
        ref="tagIpt"
        placeholder="enter确定关键词"
