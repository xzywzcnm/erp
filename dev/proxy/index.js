...
// 在设置区间内获取遇到的第一个空闲端口
const portStr = getFreePort(8080, 50000);

const erpEnv = {
  dev: "http://172.20.200.30",
  uat: "http://testing.lapa.com.cn",
  prod: "https://erp.lapa.com.cn",
  pd: "http://172.20.200.116:8007",
  gpd: "http://172.20.200.116:8123",
  hcs: "http://172.20.201.185:8123",
  nrq: "http://172.20.200.112:8107",
  zcd: "http://172.20.200.102:8125",
  lzj: "http://172.20.200.109:8123",
  caixin: "http://172.20.200.100:8123",
  wh: "http://172.20.200.172:8125",
  hwn: "http://172.20.201.186:8007",
};
...
const proxyToTargetServe = (target, key, isHtml = false) => {
  return {
    ...
    ws: false,
    onProxyRes: function (proxyRes, req, res) {
      proxyRes.headers["area1"] = isHtml ? 'setting-service' : key;
    },
