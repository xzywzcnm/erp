  ...
  <Modal v-model="model1" title="买家发送消息" @on-visible-change="visibleMt" width="600" >
    <Form ref="formInline" @submit.native.prevent :model="formInline" :rules="ruleInline" :label-width="130" >
      <FormItem v-for="(item,index) in allMessageTemplateList" :key="index" :label="item.categoryName" >
        <dyt-select v-model="item.model" > <Option
            v-for="(citem,cindex) in item.children"
            @click.native.stop="whiteTemp(citem)"
            :key="cindex"
            :value="citem.messageTemplateName" >{{ citem.messageTemplateName }}</Option> </dyt-select >
      </FormItem >
      <Row >
...
export default {
  ...
  methods: {
    sendMessage () {
      ...
      v.requestBo.itemId = v.$store.state.contactBuyer.webstoreItemId;
      v.requestBo.itemTitle = v.$store.state.contactBuyer.title;
    ...
    searchTemplateByCode () { // 根据模版编号查询模版内容
      let v = this;
      if (v.templateCode) {
        let activePlatform = v.activePlatform === 'ebay' ? 'ebay' : v.activePlatform === 'customer' ? 'ebay' : 'ebay';
        v.axios.get('/order-service/erpCommon' + api.query_tempCode + '?platformId=' + activePlatform + '&templateCode=' + v.templateCode).then(response => {
          if (response.data.code === 0) {
            let data = response.data.datas;
            if (data) {
              v.requestBo.messageContent = data.messageContent;
              v.getOrderBuyerIdAndItemId().then(res => {
                v.requestBo.messageContent = data.messageContent;
    ...
    replaceTemplatePlaceholders (orderData) { // 替换模版占位符
      let v = this;
      let tempMessageContent = v.requestBo.messageContent;
      ...
      let recipientAddress = orderData.buyerName + ' ' + (orderData.buyerPhone
      ...
      tempMessageContent = tempMessageContent.replace('${trackNumber}', orderData.trackingNumber
                                                                        ...
                                                                        : '无跟踪号');
      v.requestBo.messageContent = tempMessageContent;
    },
    whiteTemp (item) {
      let v = this;
      ...
      v.getOrderBuyerIdAndItemId().then(res => {
        v.requestBo.messageContent = item.messageContent;
        if (v.orderList.length === 0) {
          v.requestBo.messageContent = item.messageContent;
        } else if (v.orderList.length === 1) {
          v.replaceTemplatePlaceholders(v.orderList[0]);
    ...
    getAllTemp () {
      ...
      let v = this;
      let activePlatform = v.activePlatform === 'ebay' ? 'ebay' : v.activePlatform === 'customer' ? 'ebay' : 'ebay';
      v.axios.get('/order-service/erpCommon' + api.get_allTemplate + '?platformId=' + activePlatform).then(response => {
        if (response.data.code === 0) {
          let data = response.data.datas;
          let obj = {};
          let arr = [];
