            ...
            <Form ref="pageParams" :model="pageParams" :label-width="labelWidth">
              <dyt-filter>
                <Form-item label="国家/地区" prop="buyerCountryCodeList">
                  <dyt-select v-model="pageParams.buyerCountryCodeList" @on-change="countryChange" :max-tag-count="1" multiple placeholder="请选择国家/地区">
                ...
                <Form-item label="金额" prop="searchPrice">
                  <dyt-input v-model.trim="pageParams.searchPrice">
                    <dyt-select v-model="pageParams.priceOperator" value="大于" slot="prepend" style="width: 60px">
                    ...
                    <dyt-select v-model="pageParams.priceCurrency" value="USD" slot="append" style="width: 60px">
                      <Option value="USD">USD</Option>
                    </dyt-select>
                  </dyt-input>
                </Form-item>
                <Form-item label="订单号" prop="orderNo">
                  <dyt-input-tag type="textarea" placeholder="请输入订单号,多个订单号用逗号分隔" :limit="1" :string="true" v-model="pageParams.orderNo" />
                </Form-item>
                ...
                <Form-item label="买家类型" prop="amazonIsBusinessOrder">
                ...
                <Form-item label="Prime订单" prop="amazonIsPrime">
                ...
                <Form-item label="Item ID" prop="amazonIsPrime">
                  <dyt-input v-model='pageParams.asin' />
                </Form-item>
                <Form-item label="搜索字符" prop="searchValue">
                  <dyt-input-tag type="textarea" placeholder="可输入订单号、包裹号、买家ID、买家姓名、SKU、平台产品编码" :limit="1" :string="true" v-model="pageParams.searchValue" @on-enter="search"/>
                </Form-item>
                <Form-item label="买家类型" prop="amazonIsBusinessOrder">
                  <dyt-select v-model="pageParams.amazonIsBusinessOrder" clearable>
                    <Option v-for="(item,index) in buyerTypeList" :key="index" :value="item.value">{{ item.type }}</Option>
            ...
            if (params.row.orderShippingOrderBoList !== null && params.row.orderShippingOrderBoList.length > 0) {
              params.row.orderShippingOrderBoList.forEach((n, i) => {
                ...
                pos.push(h('div', {
                  ...
                  params.row.amazonIsPrime === 1 ? h('img', {
                    style: {
                      width: '32px'
                    },
                    attrs: {
  ...
  methods: {
        ...
        if (data.list !== null && data.list.length > 0) {
          data.list.forEach((n, i) => {
            pos.push({
              ...
              amazonFulfillmentChannel: n.amazonFulfillmentChannel, // 特殊处理
              amazonIsBusinessOrder: n.amazonIsBusinessOrder, // 特殊处理
              amazonIsPrime: n.amazonIsPrime// 特殊处理
            });
    ...
    ONShippingMethod (value) {
      /*
       ...
       value.type => 3--获取账号数据
       value.type => 4--获取账号的isOnlineShip
       */
      if (value.type === 1) {
