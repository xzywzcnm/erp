...
export default {
  ...
  methods: {
    ...
    searchTemplateByCode () { // 根据模版编号查询模版内容
      let v = this;
      if (v.templateCode) {
        let inGroup = v.inGroup === 'ebay' ? 'ebay' : v.inGroup === 'customer' ? 'ebay' : 'ebay';
        v.axios.get('/order-service/erpCommon' + api.query_tempCode + '?platformId=' + inGroup + '&templateCode=' + v.templateCode).then(response => {
          if (response.data.code === 0) {
    ...
    getAllTemp () {
      ...
      let v = this;
      let inGroup = v.inGroup === 'ebay' ? 'ebay' : v.inGroup === 'customer' ? 'ebay' : 'ebay';
      v.axios.get('/order-service/erpCommon' + api.get_allTemplate + '?platformId=' + inGroup).then(response => {
        if (response.data.code === 0) {
          let data = response.data.datas;
          let obj = {};
          let arr = [];
    ...
    handleBeforeUpload () {
      const check = this.uploadedImgList.length < 6;
      if (!check) {
        this.$Notice.warning({
