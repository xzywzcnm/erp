...
Vue.use(Router);
store.commit('inGroup', 'ozon');// 需AJAX向server请求菜单
store.commit('platformType', 'ozon');// 需AJAX向server请求菜单
...
const routes = [
  {
    ...
    children: [
      ...
      // {
      //   path: '/combineOrder',
      //   component: () => import('@/views/ozon/components/ozon/combineOrder') // 可合并
      // },
      {
        path: '/nonPayment',
        component: () => import('@/views/ozon/components/ozon/nonPayment')// 未付款
      },
      {
        path: '/matchTheLogistics',
        component: () => import('@/views/ozon/components/ozon/matchTheLogistics')
      },
      {
        path: '/cancelOrder',
        component: () => import('@/views/ozon/components/ozon/cancelOrder')// 取消订单
      },
      {
        path: '/manualOrder',
...
router.beforeEach(function (to, from, next) {
  ...
  document.title = title ? title + '-订单系统-OZON' : '订单系统-OZON';

  let prefix = '';
  // 浏览器回退前进点击时菜单状态改变
  let pathIndex = to.path.lastIndexOf('/');
  let pathName = to.path.slice(pathIndex + 1, to.path.length);
  let menuKey = 'ozon_' + pathName === 'ozon_' ? 'ozon_messageToBeProcessed' : 'ozon_' + pathName;
