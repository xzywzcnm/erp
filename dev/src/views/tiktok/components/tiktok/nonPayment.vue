      ...
      <div style="flex: 100;">
        ...
        <Buttons
          ...
          <Button
            ...
            :disabled="!getPermission('orderInfo_cancelOrderForNonPayment_batch')">
            <span class="icon iconfont">&#xe67e;</span> 作废订单 </Button>
          <Buttons-menu slot="list">
            <Buttons-item
              name="1" :disabled="!getPermission('orderInfo_cancelOrderForNonPayment_all')">作废订单（所有结果集）
            </Buttons-item>
          </Buttons-menu>
        </Buttons>
        <Dropdown
...
export default {
  ...
  methods: {
    ...
    cancelDeliveryHasCondition () { // 作废订单 all condition
      this.cancelDelivery('all');
    },
    cancelDelivery (type) { // 作废订单
      let v = this;
      ...
      v.$nextTick(function () {
        if (type === 'all') {
          v.cancelDeliveryModel.title = '批量作废订单（所有结果集）';
          v.cancelDeliveryModel.type = 'all';
        } else {
          v.cancelDeliveryModel.title = '批量作废订单';
