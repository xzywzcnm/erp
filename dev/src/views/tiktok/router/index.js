...
Vue.use(Router);
store.commit('inGroup', 'tiktok');// 需AJAX向server请求菜单
store.commit('platformType', 'tiktok');// 需AJAX向server请求菜单
...
const routes = [
  {
    ...
    children: [
      ...
      {
        path: '/combineOrder',
        component: () => import('@/views/tiktok/components/tiktok/combineOrder') // 可合并
      },
      {
        path: '/nonPayment',
        component: () => import('@/views/tiktok/components/tiktok/nonPayment')// 未付款
      },
      {
        path: '/matchTheLogistics',
        component: () => import('@/views/tiktok/components/tiktok/matchTheLogistics')
      },
      {
        path: '/cancelOrder',
        component: () => import('@/views/tiktok/components/tiktok/cancelOrder')// 取消订单
      },
      {
        path: '/manualOrder',
...
router.beforeEach(function (to, from, next) {
  ...
  document.title = title ? title + '-订单系统-Tiktok' : '订单系统-Tiktok';

  let prefix = '';
  // 浏览器回退前进点击时菜单状态改变
  let pathIndex = to.path.lastIndexOf('/');
  let pathName = to.path.slice(pathIndex + 1, to.path.length);
  let menuKey = 'tiktok_' + pathName === 'tiktok_' ? 'tiktok_messageToBeProcessed' : 'tiktok_' + pathName;
