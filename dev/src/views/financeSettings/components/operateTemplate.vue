    ...
    <div v-if="!detailView" style="background-color: #f5f7f9" class="mainBody">
      ...
      <div class="addBinding">
        <Button
          type="primary"
          @click="edit()"
          v-if="getPermission('templateInfo_addOperateTemplate')"
          >新建费用模版
        </Button>
      </div>
      <div class="shopTable">
          ...
          <template slot="operate" slot-scope="scope">
            <div>
              <Button
                ...
                v-if="getPermission('templateInfo_updateOperateTemplate')"
                >修改</Button
              >
              <Button
                size="small"
                ...
                v-if="
                  scope.row.status == '0' &&
                  getPermission('updateStatus_operateTemplate')
                "
                >停用</Button
              >
              <Button
...
import operateTemplateDetail from "./operateTemplateDetail.vue";

export default {
  mixins: [Mixin],
  components: { operateTemplateDetail },
  data() {
    return {
      // 仓库状态
      statusList: [
  ...
  methods: {
    // 页面查询
    searchData() {
      if (!this.getPermission("search_operateTemplateList")) {
        return this.$Message.warning("没权限");
      }
      this.axios.post(api.queryOperateTemplate, this.pageParams).then((res) => {
    ...
    changeStatus(row, type) {
      ...
      this.axios.post(api.updateOperateTemplateStatus, temp).then((res) => {
