    ...
    <div v-if="!detailView" style="background-color: #f5f7f9" class="mainBody">
      ...
      <div class="addBinding">
        <Button
          type="primary"
          @click="edit()"
          v-if="getPermission('templateInfo_addFreightTemplate')"
          >新建费用模版
        </Button>
      </div>
      <div class="shopTable">
          ...
          <template slot="operate" slot-scope="scope">
            <div>
              <Button
                ...
                v-if="getPermission('templateInfo_updateFreightTemplate')"
                >修改</Button
              >
              <Button
                size="small"
                ...
                v-if="
                  scope.row.status == '0' &&
                  getPermission('updateStatus_freightTemplate')
                "
                >停用</Button
              >
              <Button
...
import freightTemplateDetail from "./freightTemplateDetail.vue";

export default {
  mixins: [Mixin],
  components: { freightTemplateDetail },
  data() {
    return {
      // 仓库状态
      statusList: [
  ...
  methods: {
    // 页面查询
    searchData() {
      if (!this.getPermission("search_freightTemplateList")) {
        return this.$Message.warning("没权限");
      }
      this.axios.post(api.queryFreightTemplate, this.pageParams).then((res) => {
    ...
    changeStatus(row, type) {
      ...
      this.axios.post(api.updateFreightTemplateStatus, temp).then((res) => {
