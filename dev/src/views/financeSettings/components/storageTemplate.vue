    ...
    <div v-if="!detailView" style="background-color: #f5f7f9" class="mainBody">
      ...
      <div class="addBinding">
        <Button
          type="primary"
          @click="edit()"
          v-if="getPermission('templateInfo_addStorageTemplate')"
          >新建费用模版
        </Button>
      </div>
      <div class="shopTable">
          ...
          <template slot="operate" slot-scope="scope">
            <div>
              <Button
                ...
                v-if="getPermission('templateInfo_updateStorageTemplate')"
                >修改</Button
              >
              <Button
                size="small"
                ...
                v-if="
                  scope.row.status == '0' &&
                  getPermission('updateStatus_storageTemplate')
                "
                >停用</Button
              >
              <Button
...
import storageTemplateDetail from "./storageTemplateDetail.vue";

export default {
  mixins: [Mixin],
  components: { storageTemplateDetail },
  data() {
    return {
      // 仓库状态
      statusList: [
  ...
  methods: {
    // 页面查询
    searchData() {
      if (!this.getPermission("search_storageTemplateList")) {
        return this.$Message.warning("没权限");
      }
      this.axios.post(api.queryStorageTemplate, this.pageParams).then((res) => {
    ...
    changeStatus(row, type) {
      ...
      this.axios.post(api.updateStorageTemplateStatus, temp).then((res) => {
