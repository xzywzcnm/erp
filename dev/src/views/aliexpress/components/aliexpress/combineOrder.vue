    ...
    <keep-alive>
      <sliderModal
        ...
        <div slot="footer" v-if="getPermission('orderInfo_queryForMergeDigest')">
          <div class="combineButton" v-if="isAdmin || controlList.orderInfo_updateForMerge">
            <Button type="primary" @click="combineSinagleOrder">合并 </Button>
            <Button @click="orderDetails = !orderDetails">取消</Button>
          </div>
        </div>
        <orderDetails
      ...
      orderColumn: [
        {
          ...
          width: 200,
          render: (h, params) => {
            let level = params.row.aliexpressBuyerAccountLevel;
            let levelClass;
            switch (level) {
              case 'A0':
          ...
          if (data.list !== null) {
            data.list.forEach((n, i) => {
              pos.push({
                ...
                buyerAccountLevel: n.aliexpressBuyerAccountLevel
              });
            });
            v.orderData = pos;
          } else {
            ...
            v.getShippingMethodName(merchantShippingMethodIdArr)// 查物流
              .then(response => {
                ...
                if (combineAccountIdPos.length > 0) {
                  v.axios.post(api.get_saleAccountById + '?saleAccountIds=' + combineAccountIdPos + '&time2' + new Date().getTime()).then(response1 => {
                    if (response1.data.code === 0) {
                      let list = response1.data.datas;
                      for (let i = 0; i < list.length; i++) {
