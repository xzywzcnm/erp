<template >
  <div class="allotRule">
            ...
            <dyt-filter :filter-row="1">
              ...
              <FormItem label="状态" :label-width="40">
                <dyt-select v-model="pageParams.status">
                  <Option v-for="(item,index) in allotList" :key="index" :value="item.status">{{ item.title }}</Option>
                </dyt-select>
              </FormItem>
              <!-- 仓库 -->
              ...
              <div slot="operation">
                <Button style="margin-left:10px;" v-if="getPermission('wmsWarehouseAutoRule_queryAutoDistributeRule')" type="primary" :disabled="SearchDisabled" icon="md-search" @click="search">查询</Button>
    ...
    <div class="orderTable normalTop">
      <Table highlight-row border :height="tableHeight" :loading="tableLoading" :columns="allotRuleColumn" :data="allotRuleData"></Table>
    </div>
    <!-- 新建、编辑规则 -->
    <Modal width="700" :title="allotRuleModalTit" v-model="allotRuleModal" v-if="allotRuleModalStatus">
      <!-- <span>仓库：</span>
      <dyt-select style="width:230px;margin-left:10px;" v-model="allotRuleparams.warehouseId" filterable>
      ...
      <Card :bordered="false" dis-hover style="overflow-x:auto;">
        <Form class='timeSet'>
          <FormItem label="自动分配库存时间：">
            <RadioGroup v-model="allotRuleparams.timeSetting">
              <Radio label="1">
                <span>固定时间</span> <span style="margin-left:10px;">每过<InputNumber size='small' v-model='allotRuleparams.repeatIntervalAutoPoint'></InputNumber>
                </span>
              </Radio>
              ...
              </dyt-select> <span>分配一次库存</span> <br>
              <Radio label="2">
                <span>每天定时</span> <span style="margin-left:10px;">每天<TimePicker format="HH:mm" v-model='v.time' style="width: 80px;margin-right: 5px" size='small' v-for='(v,i) in addInputDataList' :key='i'></TimePicker>
...
export default {
  ...
  data () {
    return {
            ...
            return h('div', [
              h('Button', {
                style: {
                  marginRight: '10px',
                  display: (() => {
                    return this.getPermission('wmsWarehouseAutoRule_setAutoDistributeTask') ? 'line-block' : 'none';
                  })()
                },
                props: {
                  size: 'small'
                ...
                style: {
                  display: (() => {
                    return this.getPermission('wmsWarehouseAutoRule_disableAutoDistributeRule') ? 'line-block' : 'none';
                  })()
                },
                props: {
                  size: 'small'
                ...
                style: {
                  display: (() => {
                    return this.getPermission('wmsWarehouseAutoRule_enableAutoDistributeRule') ? 'line-block' : 'none';
                  })()
                },
                props: {
                  size: 'small'
      ...
      allotRuleparams: {
        ...
        repeatIntervalAutoPointUnit: 1, // 默认小时
  ...
  methods: {
    ...
    reset () { // 重置
      ...
      v.pageParams.warehouseId = 'all-options';
    },
    addAllotRule () { // 新建规则
      let v = this;
      v.allotRuleModal = true;
      v.allotRuleModalStatus = true;
      v.allotRuleModalTit = '分配规则--创建';
    },
    getList () { // 查询仓库分配规则
      let v = this;
      if (!this.getPermission('wmsWarehouseAutoRule_queryAutoDistributeRule')) return;
      let obj = {
        warehouseIds: this.$common.isEmpty(this.pageParams.warehouseId) || this.pageParams.warehouseId === 'all-options' ? null : [this.pageParams.warehouseId],
        status: this.$common.isEmpty(this.pageParams.status) || this.pageParams.status == 3 ? null : this.pageParams.status
      };
      let pos = [];
      this.loadingTrue();
      this.axios.post(api.wmsService + api.query_distributeRule, obj).then(response => {
    ...
    disableRule (row) { // 停用
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.axios.put(api.wmsService + api.disable_distributeRule + id).then(response => {
    ...
    enableRule (row) { // 启用
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.axios.put(api.wmsService + api.enable_distributeRule + id).then(response => {
            if (response && response.data && response.data.code === 0) {
    ...
    saveRule () { // 保存规则
      ...
      //   v.$Message.error(v.t('allotRule.saveError'));
      //   v.saveLoading = false;
      //   v.$nextTick(function () {
      //     v.saveLoading = true;
      //   });
      ...
      let valid = true;
      if (v.addInputDataList[0].time && v.allotRuleparams.timeSetting === '2') {
        //  保存传零时区
      ...
      if (v.allotRuleparams.timeSetting === '1') {
        if (v.allotRuleparams.repeatIntervalAutoPoint === '' || v.allotRuleparams.repeatIntervalAutoPoint === null) {
          v.$Message.error('固定时间为空');
          return;
        }
      } else {
        if (pos.join('').length === 0) {
          v.$Message.error('每天定时时间为空');
          return;
        }
      }
      ...
      v.axios.post(api.wmsService + api.set_distributeRule, v.allotRuleparams).then(response => {
        if (response.data.code === 0) {
          // v.saveLoading = false;
    ...
    showModal (val) {
      ...
      v.allotRuleModal = true;
      v.allotRuleModalStatus = true;
      v.allotRuleModalTit = '分配规则--编辑';
    }
  },
  created () {
    let v = this;
    this.tableHeight = this.getTableHeight(185);
    v.getPermissionCallback('logistics_allotRule').then(() => {
      v.getList();
