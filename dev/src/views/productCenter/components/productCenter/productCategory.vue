<template >
  <div class="labelCont productCategoryDome">
    <div class="categoryCont">
      ...
      <div class="categoryDetail">
        <Form ref="productCategory" :model="productCategory" :label-width="95" label-position="right">
          <div class="form-col-item">
            <Form-item label="分类ID" :rules="{ required: true, message:'分类ID不能为空', trigger: 'blur' }" prop="productCategoryCode">
              <dyt-input :style="`${inputWidth}`" :maxlength="50" v-model.trim="productCategory.productCategoryCode" :disabled="viewMode"></dyt-input>
            </Form-item>
            <Form-item label="分类中文名称" :rules="{ required: true,validator:cnameValidator,  trigger: 'blur' }" prop="cnName">
              <dyt-input :style="`${inputWidth}`" :maxlength="50" v-model.trim="productCategory.cnName" :disabled="viewMode"></dyt-input>
            </Form-item>
            <Form-item label="分类英文名称" prop="enName">
              <dyt-input :style="`${inputWidth}`" :maxlength="100" v-model.trim="productCategory.enName" :disabled="viewMode"></dyt-input>
            </Form-item>
            <Form-item label="报关编码" prop="hsCode" :rules="{ required: true, validator: hsCodeVerify, msg: '请输入报关编码', minLength: 10, maxLength: 10, trigger: 'blur' }">
              <dyt-input :style="`${inputWidth}`" :maxlength="10" v-model.trim="productCategory.hsCode" :disabled="viewMode"></dyt-input>
            </Form-item>
            <Form-item label="中文报关名称" prop="cnHsName" :rules="{ required: true, validator: hsNameMaxVerify, msg: '请输入中文报关名称', maxLength: 200, trigger: 'blur' }">
              <dyt-input :style="`${inputWidth}`" :maxlength="200" v-model.trim="productCategory.cnHsName" :disabled="viewMode"></dyt-input>
            </Form-item>
            <Form-item label="英文报关名称" prop="enHsName" :rules="{ required: true, validator: enNameMaxVerify, msg: '请输入英文报关名称', maxLength: 200, trigger: 'blur' }">
              <dyt-input :style="`${inputWidth}`" :maxlength="200" v-model.trim="productCategory.enHsName" :disabled="viewMode"></dyt-input>
            </Form-item>
            <Form-item label="描述" prop="description">
              <dyt-input type="textarea" :autosize="{ minRows: 3, maxRows: 5 }" :style="`${inputWidth}`" :maxlength="500" v-model.trim="productCategory.description" :disabled="viewMode"></dyt-input>
            </Form-item>
          ...
          <div class="form-col-item">
            <Form-item
              label="分类类型"
              prop="productType"
              :rules="{ required: true, message:'分类类型不能为空', trigger: 'submit', type: 'string' }"
            >
              <dyt-select :style="`${inputWidth}`" v-model="productCategory.productType" :disabled="viewMode" placeholder="请选择分类类型">
                <Option v-for="(item,key) in classTypeList" :value="item.productType" :key="key">{{ item.configName }}</Option>
              </dyt-select>
            </Form-item>
            <Form-item label="尺码类型" prop="sizeType">
              <dyt-select :style="`${inputWidth}`" v-model="productCategory.sizeType" :disabled="viewMode" placeholder="请选择尺码类型">
                <Option v-for="(item,key) in sizeList" :value="item.sizeTypeId" :key="key">{{ item.typeName }}</Option>
              </dyt-select>
            </Form-item>
            <Form-item label="分类尺码" prop="classificationId">
              <dyt-select
                :style="`${inputWidth}`"
                v-model="productCategory.classificationId"
                :disabled="viewMode" placeholder="请选择分类尺码"
              >
                <Option v-for="(item, index) in classificationDataList" :value="item.classificationId" :key="`classId-${index}`">{{ item.classificationName }}</Option>
              </dyt-select>
            </Form-item>
            <Form-item label="绑定属性" prop="attributeId">
              <dyt-select
                :style="`${inputWidth}`"
                v-model="productCategory.attributeId"
                :disabled="viewMode || !getPermission('queryProductCategoryProductAttribute')"
              >
            ...
            <Form-item label="绑定质检模板" prop="qualityClassificationId">
              <dyt-select
                :style="`${inputWidth}`"
                placement='top'
                v-model="productCategory.qualityClassificationId"
                :disabled="viewMode"
              >
...
export default {
  ...
  data () {
    let v = this;
    return {
      inputWidth: 'width: calc(100% - 48px); max-width: 300px;',
  ...
  methods: {
    ...
    getProductType() {
      this.axios.post('/pds-service/pds/lapa/workbench/product/getNotDeleteSpuConfig').then(res => {
        if (res.data.code === 0) {
          this.classTypeList = res.data.datas
        }
      })
    },
    exportCategory () {
          ...
          if (valid) {
            self.axios.post(api.add_productCategory, self.productCategory).then(response => {
              if (response.data.code === 0) {
                ...
                self.productCategory.expand = false;
    ...
    getClassificationList () {
      this.axios.get(api.queryAllProductSizeClassificationList).then(res => {
        if (res && res.data && res.data.code === 0) {
          this.classificationDataList = (res.data.datas || []).map(item => {
            this.$common.isEmpty(item.classificationId) && (item.classificationId = '');
            return item
          })
        }
      })
    ...
    getAllQuality () {
      ...
      this.axios.get(api.getAllQualityTemplate).then((res) => {
        if (res && res.data && res.data.code === 0) {
          this.classifyList = (res.data.datas || []).map(item => {
            return {
              ...item,
              qualityClassificationId: Number(item.qualityClassificationId)
            }
          })
        }
      })
    ...
    // getSizeTemplateList (val) {
    //   this.axios.post(api.sizeManageApiConfig.sizeTypeManage.queryProductSizeTemplateName, {
    //     sizeTypeId: val
    //   }).then(res => {
    //     console.log(val, res)
    //   }).catch(() => {})
  ...
  watch: {
    ...
    // 'productCategory.sizeType': {
    //   deep: true,
    //   handler (val) {
    //     !this.$common.isEmpty(val) && this.getSizeTemplateList(val);
    //   }
    // }
  }
};
