...
export default {
  ...
  methods: {
    ...
    getPageData (obj) {
      ...
      this.language = obj.validLanguage; // 选中的语言
    ...
    assemblePic (base64, isSelfUse) {
      return new Promise((resolve, reject) => {
        if (isSelfUse || this.$common.isEmpty(this.language)) return resolve(base64);
        let _self = this;
    ...
    getTotalData () {
      return new Promise((resolve) => {
        this.becomeChartList().then(listRes => {
          resolve(Promise.all(listRes.map(async (titem) => {
    ...
    mulUploadPic () {
      ...
      this.axios({
        ...
        if (res.data && res.data.datas) {
          res.data.datas.forEach((k, key) => {
            this.$set(this.formatList[key], 'pictureUrl', k);
    ...
    resetPic (type) {
      ...
      this.totalData = [];
      this.formatList = [];
      !type && this.parentEvent.finishPicture(false);
    },
  },
}
</script>
