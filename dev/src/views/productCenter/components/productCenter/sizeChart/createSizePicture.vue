  ...
  <div class="promote-pic">
    <div class="sizePicture" v-if="dataItem.size && tableColumns.length > 0" :class="{'woman-shoe-plan': [4].includes(planType)}">
      <div class="size-image-main pichtml" :class="sizeForm[dataItem.size].size" ref="pichtml">
        <div class="fmb0">
          <div class="text-center text-title text-weight mb20" v-if="![4].includes(planType)">{{dataItem.titlelist[0]&&dataItem.titlelist[0].title}}</div>
          <div class="text-center text-title text-weight mb10" v-if="[4].includes(planType)">Product Attributes</div>
          <div style="width:100%;">
            <Form :model="formItem" :label-width="90">
              <div class="form-item">
                        ...
                        <div v-for="(item,index) in dataItem.attrLists" :key="index+'attr'" class="mb10">
                          <FormItem :label="item.attr+':'">
                            <Button
                              ...
                              class="btn-mb" style="margin-right:3px"
                ...
                <div class="text-center split-line text-weight" style="margin-top:0px">
      ...
      <div class="size-pic--main print-self-use" ref="selfUsehtml" v-if="isShowSelfUse">
        <Table :columns="selfUseColumns" :data="tableData" border ref="selfUseTable" style="width:100%;">
          <template slot-scope="{row,index,column}" slot="nameValue">
            <span class="row-col-txt">{{row.size||''}}</span>
          </template>
          <template slot-scope="{row,index,column}" slot="anValue">
...
export default {
  ...
  methods: {
    ...
    initData (data, type) {
      return new Promise((resolve, reject) => {
        ...
        this.resetPic().then(() => {
          ...
          this.$nextTick(() => {
            this.setTabledata(data).then(() => {
              if (this.$common.isEmpty(data.allList)) return resolve(false);
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          tableData.forEach(k => {
            ...
            // if (flag || this.isPrintAll) list.push(k);
            if (flag) list.push(k);
          })
          this.tableData = list;
        } catch (e) { }
    ...
    setItem (type, domClass) {
      this.initData(this.detailInfo, type).then(() => {
    ...
    generatePic (domClass, isSelfUse = false) {
      ...
      html2canvas(dom, opts).then((canvas) => {
        ...
        this.waitImage(base64).then(res => {
          ...
          obj.productId = this.detailInfo.productInfo.productId;
          obj.resolvingPower = res || this.dataItem.screenSize;
          obj.file = file;
          // pictureType图片类型（0橱窗图片，1详情图片，2通用自用尺码图，3婴童自用尺码图，4儿童自用尺码图），默认传的是0
    ...
    mulUploadPic () {
      ...
      this.axios({
        ...
        if (res.data && res.data.datas) {
          res.data.datas.forEach((k, index) => {
            this.$set(this.formatList[index], 'pictureUrl', k);
            delete this.formatList[index].file;
          })
          // 将从接口返回的图片传到父级
          this.parentEvent.getSizePicture(this.formatList);
