<template >
  <div class="codingRules_box">
    <RadioGroup v-model="pageParams.isDefined" vertical @on-change="changeRadio">
      <Radio :label="1">系统默认编号</Radio>
      <Radio :label="0">用户自定义自动编号</Radio>
    </RadioGroup>
      ...
      <Form ref="pageParams" :model="pageParams" :rules="ruleInline" :label-width="140">
        <Row type="flex" style="margin-top: 12px; flex-direction: column;">
          ...
          <Form-item label="SPU编码前缀" prop="spuPrefix">
            <dyt-input v-model.trim="pageParams.spuPrefix" style="width: 120px;"></dyt-input>
          </Form-item>
          </Col>
          <Col>
          <Form-item label="SPU数字位数数量" prop="spuNumberSize">
            <dyt-inputNumber v-model.trim="pageParams.spuNumberSize" :min="0" style="width: 120px;"></dyt-inputNumber>
          </Form-item>
          </Col>
          <Col>
          <Form-item label="起始数值" prop="initNumber">
            <dyt-inputNumber v-model.trim="pageParams.initNumber" :min="0" style="width: 120px;" :disabled="initNumberTalg"></dyt-inputNumber>
            <span class="color_red" style="font-size: 12px; margin-left: 6px;">仅在首次启用时可以设置初始值</span>
          </Form-item>
          </Col>
        </Row>
        <Button type="primary" @click="preservation" style="margin: 0 0 5px 155px;">保存</Button>
      </Form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 获取商户编号信息
    queryProductSpu () {
      if (this.getPermission('productSpuSetting_select')) {
        this.axios.get(api.get_queryProductSpu).then((response) => {
          if (response.data.code === 0) {
            if (response.data.datas) {
              let obj = Object.assign({}, response.data.datas);
              if (obj.initNumber > 0) {
