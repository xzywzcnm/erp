        ...
        if (res && res.data && res.data.code === 0) {
          Object.keys(this.formParams).forEach(key => {
            if (!this.$common.isUndefined(res.data.datas[key])) {
              this.$set(this.formParams, key, res.data.datas[key]);
            }
          })
          if (res.data.datas.laPaProductSizePartInfoVOList) {
            const partIds = res.data.datas.laPaProductSizePartInfoVOList.map(item => {
              return item.partId;
            })
            this.$set(this.formParams, 'sizePartIdList', partIds);
          }
          if (res.data.datas.laPaProductPictureLanguageList) {
            const pic = res.data.datas.laPaProductPictureLanguageList || [];
            let picId = '';
    ...
    getProductSizePartList () {
      this.axios.get(this.api.queryAllProductSizePartList).then(res => {
        if (res.data.code === 0 && res.data.datas) {
          this.productSizePartList = res.data.datas;
        }
      })
    },
    // 重置数据
