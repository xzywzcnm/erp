          ...
          <Form ref="pageParams" class="normalTop" :model="pageParams" :label-width="70" label-position="right" >
            <dyt-filter>
              <Form-item label="导入类型" >
                ...
                <Option value='productImport' >商品导入</Option > <Option value='productSkuMappingImport' >映射导入</Option >
                <Option value='productGoodsTagAddImport' >导入商品标签</Option >
                <Option value='productGoodsTagDeleteImport' >解绑标签</Option > </dyt-select >
              </Form-item >
              <FormItem label="导入状态" class="notplatformParamsSelect">
                <Button-group >
                  <template v-for="(item, index) in typeList" >
                    <Button v-if="item.selected" type="primary" @click="enbaleType(index, item.status)" :key="index">{{ item.title }} </Button>
                    <Button v-if="!item.selected" @click="enbaleType(index, item.status)" :key="index">{{ item.title }}</Button>
                  </template >
                </Button-group >
              </FormItem >
              <Form-item label="任务号" >
      ...
      importColumn: [
        {
          ...
          render: (h, params) => {
            if (params.row.type === 'productImport') {
              ...
              return h('span', '映射导入');
            } else if (params.row.type === 'productGoodsTagAddImport') {
              return h('span', '导入商品标签');
            } else if (params.row.type === 'productGoodsTagDeleteImport') {
              return h('span', '解绑标签');
            }
          }
        }, {
          ...
          align: 'center',
          render: (h, params) => {
            if (params.row.status !== 2 && params.row.status !== 4 && params.row.failure !== 0) {
              return h('div', {
                style: {
                  color: '#0000FF',
                  cursor: 'pointer'
  ...
  methods: {
    ...
    getList () { // 获取列表数据
      ...
      if (!v.pageParams.types.length) {
        types = [
          'productImport',
          'productSkuMappingImport',
          'productGoodsTagAddImport',
          'productGoodsTagDeleteImport',
          'afterSalesExport'
        ];
      } else {
      ...
      v.loadingTrue();
      v.axios.post(api.query_taskInfoData, obj).then(response => {
