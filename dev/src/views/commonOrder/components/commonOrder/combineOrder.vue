...
<template>
  <div>
            ...
            <Form ref="pageParams" :model="pageParams" :label-width="labelWidth">
              <dyt-filter :filter-row="1">
                ...
                <Form-item label="标签" prop="tagIdList">
                  <dyt-select v-model="pageParams.tagIdList" :max-tag-count="1" multiple placeholder="请选择或搜索标签"><Option
                ...
                <Form-item label="合并状态" prop="allowMerge">
                  <dyt-select v-model="pageParams.allowMerge" placeholder="合并状态">
                    <Option
                      v-for="item in combineOrderStatusList"
                      :key="item.value"
                      :value="item.value"
                    >{{ item.label }}</Option>
                  </dyt-select>
                ...
                <Form-item label="总金额" prop="searchPrice" :rules="priceAuth">
                  <dyt-input v-model.trim="pageParams.searchPrice">
                    <dyt-select v-model="pageParams.priceOperator" placeholder="比较" slot="prepend" style="width: 80px">
                ...
                <div slot="operation">
                  <Button
                    ...
                    v-if="isAdmin || controlList.orderInfo_queryForAllowMerge"
    ...
    <div class="tableControl" style="display: flex;">
      <div
       style="flex: 100;"
        v-if="getPermission('orderInfo_updateForBatchMerge_all') || getPermission('orderInfo_updateForBatchMerge')"
      >
        <Buttons
          type="primary"
          trigger="click"
          @on-click="combineOrderHasCondition"
          :disabled="!getPermission('orderInfo_updateForBatchMerge')">
          ...
          <div class="combineOrderDeliveryMes">
            ...
            <div class="combineTable">
              <dTable
                highlight-row
                border
                :columns="combineOrderColumn"
                :data="combineOrderData"
                @on-row-click="changeDetails"></dTable>
            ...
            <div class="uncombineTable">
              <dTable
                highlight-row
                border
                @on-row-click="changeDetails"
                :columns="uncombineOrderColumn"
                :data="uncombineOrderData"
                @on-selection-change="getUnCombineOrderValue"></dTable>
              <div class="shutOutHeader">未合并</div>
            </div>
            <Alert
              type="warning"
              show-icon
              class="normalTop">有留言、未匹配商品或者截留订单不参与合并，如需合并请先标记留言已读、匹配商品或标记不再截留。已指定仓库物流方式的订单，追加进已有包裹将丢失原来指定的仓库物流方式，新建包裹则需要到订单系统中重新指定仓库物流方式。
            </Alert>
    ...
    <keep-alive>
      <Modal
        v-model="combineShippingModel.status"
        :mask-closable="false"
        width="400"
        v-if="combineShippingStatus"
        @on-ok="combine">
      ...
      orderColumn: [
        {
          ...
          render: (h, params) => {
            return h('div', [
              ...
              }, params.row.buyerName), h('div', {
                style: {
                  color: '#000'
                }
              }, params.row.buyerEmail)
            ]);
          }
        }, {
          title: '订单数',
  ...
  methods: {
    ...
    getParams () {
      ...
      if (!this.isGetBusiness) {
        delete pageParams.businessDeptIdList;
      }
      return pageParams
    },
    getList () {
      ...
      v.orderData = [];
      v.loadingTrue();
      v.axios.post(api.get_combineOrderList, this.getParams()).then(response => {
    ...
    loadData (item, callback) {
      let v = this;
      item.loading = true;
      v.axios.get(api.erpServiceCommon + api.get_enableShippingMethods + '?carrierId=' + item.value).then(response => {
        if (response.data.code === 0) {
          let data = response.data.datas;
          let pos = [];
    ...
    getCombineData () {
      ...
      v.unCombineOrderData = [];
      // 可合并详情接口
      v.axios.post(api.get_mergeDigest, JSON.stringify(obj)).then(response => {
        if (response.data.code === 0) {
          let data = response.data.datas;
          let combinePos = [];
          let noCombinePos = [];
          ...
          // 查物流
          v.getShippingMethodName(merchantShippingMethodIdArr).then(response => {
            data.forEach((n, i) => {
            ...
            if (combineAccountIdPos.length > 0) {
              v.axios.post(api.get_saleAccountById + '?saleAccountIds=' + combineAccountIdPos + '&time3' + new Date().getTime()).then(response1 => {
                if (response1.data.code === 0) {
                  let list = response1.data.datas;
                  for (let i = 0; i < list.length; i++) {
