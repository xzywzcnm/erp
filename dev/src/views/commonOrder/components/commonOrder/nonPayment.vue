          ...
          <Form ref="pageParams" :model="pageParams" :label-width="70" label-position="left">
            <div class="platformParamsSelect">
              <Row>
                ...
                <Col
                  :lg="twoItemLg" :md="twoItemMd" :offset="1">
                  <Form-item
                    label="标签" prop="tagIdList">
                    <dyt-select
            ...
            <div
              class="platformParamsMoreSection" :class="{none : !moreFilter}">
              <Row>
                <Col
                  :lg="oneItemLg" :md="oneItemMd">
                  <Form-item
                    label="国家/地区" prop="buyerCountryCodeList">
                    <dyt-select
              ...
              <Row class="normalTop">
                ...
                <Col
                  :lg="threeItemLg" :md="threeItemMd" :offset="1">
                  <Form-item
                    label="平台产品编码" prop="webstoreItemId">
                    <Input
                      placeholder="请输入平台产品编码" v-model.trim="pageParams.webstoreItemId"></Input>
                  </Form-item>
                </Col>
                <Col
                  :lg="threeItemLg" :md="threeItemMd" :offset="1">
                  <Form-item
                    label="SKU" prop="webstoreSku">
                    <dyt-input-tag type="textarea" placeholder="请输入SKU, 多个请用逗号隔开" :limit="1" :string="true" v-model="pageParams.webstoreSku" />
                  </Form-item>
                </Col>
              </Row>
              <Row
                class="normalTop" v-if='inGroup === "shopify"'>
                <Col :span="7">
                  <Form-item label="是否货到付款订单">
                    <local-buttons
                      :data="tabsData1" :value.sync="pageParams.isCashOnDelivery"></local-buttons>
                  </Form-item>
                </Col>
              </Row>
            </div>
            <div
              class="platformSearch clear" v-if="!moreFilter">
              <Row>
                <Col
                  :lg="oneItemLg" :md="oneItemMd">
            ...
            <div
              class="filterItem filterConfirm"
              :class="{normalTop : moreFilter}"
    ...
    <div class="tableControl">
      ...
      <Buttons
        ...
        <Button
          ...
          {{ isCancelPlat.includes(platform) ? '取消订单' : '作废订单' }}
        </Button>
        <Buttons-menu slot="list">
          <Buttons-item
            name="1" :disabled="!getPermission('orderInfo_cancelOrderForNonPayment_all')"
          >
            {{ isCancelPlat.includes(platform) ? '取消订单' : '作废订单' }}（所有结果集）
          </Buttons-item>
        </Buttons-menu>
      </Buttons>
      <Dropdown
            ...
            <div class="normalTop">
              ...
              <Form
                ...
                <Form-item
                  ...
                  <Input
                    v-model.trim="setMarkModel.paymentMethodName" value=""></Input>
                </Form-item>
                <Row>
                  <Col :span="12">
                    <Form-item
                      ...
                      <Input
                        v-model.trim="setMarkModel.webstoreTransactionId" value=""></Input>
                    </Form-item>
                  </Col>
                  <Col
                ...
                <Form-item
                  ...
                  <Input
                    v-model.trim="setMarkModel.paymentNote" :rows="setPayModel.num" value=""></Input>
                </Form-item>
              </Form>
            </div>
          </div>
            ...
            <div class="normalTop">
              ...
              <Form ref="cancelOrderForm" :label-width="100" :rules="formRule" :model="cancelDeliveryModel" class="noramlTop setAllMarkRead">
                <Form-item
                  label="原因"
                  prop="cancelReason"
                  v-if="!showReasonPlat.includes(platform)"
                >
                  <dyt-select
...
export default {
  ...
  data () {
    ...
    return {
      ...
      moreFilterMes: '更多选项（国家/地区、下单日期等）',
      clearAble: true,
      resetStatus: true,
  ...
  methods: {
    ...
    startLoading () {
      var v = this;
      v.$Loading.start();
      Promise.resolve(v.controlList.orderInfo_queryForNonPayment ? v.getList() : v.gotoError()).then(function () {
        v.$Loading.finish();
        v.getShopList();
    ...
    cancelDelivery (type) { // 取消订单
      ...
      this.getReasonList();
      v.$nextTick(function () {
        const txt = v.isCancelPlat.includes(v.platform) ? '取消' : '作废';
        if (type === 'all') {
          v.cancelDeliveryModel.title = `批量${txt}订单（所有结果集）`;
          v.cancelDeliveryModel.type = 'all';
        } else {
          v.cancelDeliveryModel.title = `批量${txt}订单`;
          ...
          if (response.data.code === 0) {
            ...
            if (dataList !== null && dataList.length === 0) {
              ...
              dataList.forEach((n, i) => {
                if (n.code === 110602) {
                  v.$Message.error(`${v.isCancelPlat.includes(v.platform) ? '取消' : '作废'}订单失败`);
                }
              });
            }
            v.pageParamsStatus = true;
