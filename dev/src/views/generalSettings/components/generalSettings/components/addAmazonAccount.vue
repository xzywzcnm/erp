<template >
  <Modal
      ...
      :title="workType==='add' ? '添加amazon账户' : workType==='edit' ? '编辑amazon账户' : '查看amazon账户'" >
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100" >
      <FormItem label="店铺：" prop="storeName" >
        <Input v-model="formValidate.storeName" placeholder="" ></Input >
      </FormItem >
      <FormItem label="账号：" prop="account" >
        <Input v-model="formValidate.account" placeholder="" ></Input >
      </FormItem >
      <FormItem label="市场：" prop="mark" >
        <dyt-select v-model="formValidate.mark" >
        <OptionGroup v-for='(v,i) in siteArr' :key='i' :label="v.siteCountry" v-if='Chinese' >
          <Option
              v-for="item in v.childSite"
              :value="item.amazonSiteBaseDataId"
              :key="item.amazonSiteBaseDataId" >{{ item.siteCountry }}</Option>
        </OptionGroup >
        <OptionGroup v-for='(v,i) in siteArr' :key='i' :label="v.siteArea" v-if='English' >
          <Option
              v-for="item in v.childSite"
              :value="item.amazonSiteBaseDataId"
              :key="item.amazonSiteBaseDataId" >{{ item.siteArea }}</Option>
        </OptionGroup >
        </dyt-select >
      </FormItem >
      <FormItem label="Seller ID：" prop="sellerId" >
        <Input v-model="formValidate.sellerId" placeholder="" ></Input >
      </FormItem >
      <FormItem label="AWS  Access Key ID：" prop="accessKeyId" >
        <Input v-model="formValidate.accessKeyId" placeholder="" ></Input >
      </FormItem >
      <FormItem label="Secret Key：" prop="secretKey" >
        <Input v-model="formValidate.secretKey" placeholder="" ></Input >
      </FormItem >
      <FormItem label="绑定邮箱" >
        <span >注意：</span ><span >为保证买家能正常收到邮件，请绑定amazon后台配置中的“客服邮箱”</span >
      </FormItem >
      <FormItem label="邮箱地址：" >
        <Row >
          <Col span='10' >
            <FormItem prop='addressEmail' >
              <Input v-model="formValidate.addressEmail" placeholder="" ></Input >
            </FormItem >
          </Col >
          <Col span='1' >
          ...
          <Col span='8' >
            <FormItem prop='emailPlatType' >
              <dyt-select v-model="formValidate.emailPlatType" > <Option
                  :value="v.code" v-for='v in emailTypeList' :key='v.code' >{{ v.label }}</Option> </dyt-select >
            </FormItem >
          </Col >
        </Row >
      </FormItem >
      <Row >
        <Col span="19" >
          <FormItem
              :label="this.formValidate.emailPlatType==='qq.com' || this.formValidate.emailPlatType==='163.com' ? '邮箱授权码' + '：' : '邮箱密码' + '：'"
              prop='emailPwd' >
            <Row >
              <Col span='2' >
                <span @click='goToAuthCodeUrl' ><Icon
                    type="help-circled"
                    v-if="this.formValidate.emailPlatType==='qq.com' || this.formValidate.emailPlatType === '163.com'"
                    style='font-size: 16px;cursor:pointer' ></Icon ></span >
              </Col >
              <Col span='22' >
                <Input v-model="formValidate.emailPwd" placeholder="" type='password' ></Input >
              </Col >
            </Row >
          </FormItem >
        </Col >
        <Col span='3' >
          <a
              ...
              @click='validateEmail'
              v-if='!(editAuth === "1" || editAuth === "4")' >验证邮箱</a >
          <!--<a href="javascripr:;" style='display: inline-block;padding-top: 8px;padding-left: 14px' >邮箱授权码?</a>-->
        </Col >
      </Row >
    </Form >
    <div style='padding-left: 30px' v-if='!(editAuth === "1"|| editAuth === "4")' >
      <Button
          type="primary"
          @click='handleSubmit("formValidate")'
          :disabled='editAuth === "1" || editAuth === "4" ? true:false' >确定 </Button >
  ...
  methods: {
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          var obj = {
            emailAddress: this.formValidate.addressEmail + '@' + this.formValidate.emailPlatType,
            ...
            amazonAccountId: this.workType === 'add' ? '' : this.curobj.amazonAccountId,
    ...
    goToAuthCodeUrl () {
      if (this.formValidate.emailPlatType === 'qq.com') {
        window.open('http://service.mail.qq.com/cgi-bin/help?subtype=1&&no=1001256&&id=28');
    ...
    validateEmail () {
      if (this.formValidate.addressEmail === '') {
        this.$Message.warning('请填写邮件地址');
        return;
      } else if (this.formValidate.emailPlatType === '') {
        this.$Message.warning('请填写邮箱类型');
        return;
      } else if (this.formValidate.emailPwd === '') {
        this.$Message.warning('请填写邮箱密码');
        return;
      }
      let obj = {
        emailAddress: this.formValidate.addressEmail + '@' + this.formValidate.emailPlatType,
