      ...
      columns: [
        {
          ...
          align: 'center',
          render: (h, params) => {
            if (params.row.allegroStatus === 1) {
              return h('span', {
                style: {
                  color: '#008000'
                }
              }, '有效');
            } else if (params.row.allegroStatus === 0) {
              return h('span', {
                style: {
                  color: '#FF0000'
                }
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    self.show(params.row.saleAccountId, 'check', params.row.saleAccount.account);
                  }
                }
              }, '查看'), h('Button', {
                props: {
                ...
                on: {
                  click: () => {
                    self.show(params.row.saleAccountId, 'edit', params.row.saleAccount.account);
                  }
                }
              }, '编辑'), params.row.allegroEnabled !== 0 && params.row.saleAccount.status === 0 ? h('Button', {
                props: {
                  size: 'small'
                },
                style: {
              ...
              }, '启用') : params.row.allegroEnabled !== 0 && params.row.saleAccount.status !== 0 ? h('Button', {
                props: {
                  size: 'small'
                },
                style: {
              ...
              h('Button', {
                ...
                on: {
                  click: () => {
                    self.show(params.row.saleAccountId, 'give', params.row.saleAccount.account);
                  }
                }
              }, '授权')
            ]);
