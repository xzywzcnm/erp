...
export default {
  ...
  methods: {
    ...
    getPermission (name) {
      ...
      let { isAdmin, roleList } = this.saveState;
    ...
    fetch (mapi, type, power) {
      ...
      // 获取请求参数
      let reqParams = this.getFilterParams();
    ...
    validateForm () {
      ...
      Object.keys(this.searchParams).forEach(key => {
        var array = ['pageNum', 'pageSize', 'orderBy', 'upDown'];
    ...
    // 返回搜索参数
    getFilterParams () {
      let filterParams = this.$common.copy(this.searchParams);
      if (!this.$common.isEmpty(filterParams.payStartTime)) {
        filterParams.payStartTime = this.$common.getUniversalTime(filterParams.payStartTime, 'fulltime');
      }
      if (!this.$common.isEmpty(filterParams.payEndTime)) {
        filterParams.payEndTime = this.$common.getUniversalTime(filterParams.payEndTime, 'fulltime');
      }
      if (!this.isGetBusiness) {
        delete filterParams.businessDeptIdList;
      }
      return filterParams;
    },
    // 搜索
    ...
    reset () {
      ...
      Object.keys(this.searchParams).forEach(key => {
        ...
        if (!~array.indexOf(key)) {
          ...
          if (this.$common.isNumber(search)) {
            this.searchParams[key] = null;
            return;
          }
          if (!this.$common.isEmpty(search)) {
            this.searchParams[key] = '';
          }
        }
      });
  ...
  created () {
    this.saveState = this.$common.copy(this.$store.state || {});
  }
};
