...
export default {
  ...
  props: {
    ...
    showDisabledPlaceholder: { type: Boolean, default: false },
    // 下拉数据
  ...
  computed: {
    ...
    selectConfig () {
      ...
      let isHidden = !this.showDisabledPlaceholder && (config.disabled || config.readonly);
      if (this.$common.isBoolean(config.hiddenPlaceholder)) {
        isHidden = config.hiddenPlaceholder;
      } else if (this.$common.isBoolean(config['hidden-placeholder'])) {
        isHidden = config['hidden-placeholder'];
      }
      if (isHidden) {
        config.placeholder = '';
      }
