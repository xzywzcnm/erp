<template>
  <div :ref="`tag-${pageId}`" class="dyt-input-tag-content" :class="{
    'dyt-tag-content-limit': limit > 0,
    'tag-content-focus': isFocus,
    'dyt-tag-is-disabled': disabled
  }" @click="focus">
    <template v-if="limit > 0">
      <Poptip v-if="limit > 0" ref="tagPopover" placement="bottom" :width="popoverWidth" @on-popper-show="popoverShow"
        v-model="isFocus" transfer :popper-class="`dyt-input-tag-content${disabled ? ' tag-is-disabled' : ''}`">
        <div v-if="vModel.length > 0" class="tag-limit">
          <span v-for="(tag, index) in vModel.slice(0, limit)" :key="`tag-${index}`"
            class="el-tag el-tag--info el-tag--small" @click="clickTag(tag)">
            <span class="el-tag-text" :title="defaultProp.label ? tag[defaultProp.label] : tag">{{ defaultProp.label ?
              tag[defaultProp.label] : tag }}</span>
            <i v-if="selectConfig.closable" class="ivu-icon ivu-icon-md-close tag-icon-close"
        ...
        <div v-if="vModel.length" class="clearable-all poptip-clearable">
          <i class="ivu-icon ivu-icon-md-close tag-icon-close" title="清空" @click.stop="clearableHand" />
        </div>
        <div class="tag-popper-content" slot="content">
          <dyt-input v-if="!disabled && !readonly && selectConfig.addTag" :ref="`popoverTagInput-${pageId}`"
            v-model="inputValue" :class="{
              'input-empty-tag': vModel.length === 0
            }" :type="type" :placeholder="placeholder" :autosize="{ minRows: 2, maxRows: (preview ? 4 : 15) }"
            v-bind="inputConfig" style="padding: 0 13px 8px 13px;" @on-enter="addTagHand" @on-keyup="tagkeyup"
            @on-blur="blur" />
          <div class="popover-content" v-if="(preview && type === 'textarea') || type !== 'textarea'">
            <template v-if="vModel.length > 0">
              <span v-for="(tag, index) in vModel" :key="`tag-${index}`" class="el-tag el-tag--info el-tag--small"
                @click="clickTag(tag)">
                <span class="el-tag-text"
      ...
      <div class="default-tag-constent">
        <span v-for="(tag, index) in vModel" :key="`tag-${index}`" class="el-tag el-tag--info el-tag--small"
          ...
          <span class="el-tag-text"
            :title="(defaultProp.label || defaultProp.value) ? tag[defaultProp.label || defaultProp.value] : tag">{{
              (defaultProp.label || defaultProp.value) ? tag[defaultProp.label || defaultProp.value] : tag }}</span>
          <i v-if="selectConfig.closable" class="ivu-icon ivu-icon-md-close tag-icon-close" @click.stop="closeTag(tag)" />
        </span>
        <Input v-if="!disabled && !readonly && !(limit > 0) && selectConfig.addTag" :ref="`saveTagInput-${pageId}`"
          v-model="inputValue" class="input-new-tag el-input__inner" :class="{
            'input-empty-tag': vModel.length === 0
          }" :autosize="true" :type="type" :placeholder="vModel.length <= 0 ? placeholder : ''"
          :style="`width:${vModel.length && !inputValue.includes('\n') > 0 ? inputWidth : '100%;'}`" v-bind="inputConfig"
          @on-enter="addTagHand" @on-keyup="tagkeyup" @on-blur="blur" />
      </div>
...
export default {
  ...
  watch: {
    value: {
      ...
      immediate: true,
      handler(val) {
        if (this.$common.isString(this.vModel)) return;
        if (this.string) {
