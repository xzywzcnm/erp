<template >
  <div >
    <div class="moreBtn" >
      <Button class="firBtn" ref="mButton" @click="data.btn.clickFn()" :disabled="data.btn.disabled">
        <span class="more-text">{{ data.btn.text }}</span>
      </Button>
      <Select
        :clearable="false"
        v-model="model"
        :style="style"
        transfer ref="moreButton"
      >
        <Option
...
export default {
  ...
  computed: {
    ...
    style () {
      return {
        width: this.isShowList ? `${this.wid + 16}px` : `${this.wid}px`
      };
    }
  },
  methods: {}
