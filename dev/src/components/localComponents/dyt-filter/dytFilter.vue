    ...
    <div class="filter-search-btn" :ref="`filter-btn-${dytRefFilter}`">
      <div :ref="`serach-${dytRefFilter}`">
        <slot name="operation">
          ...
          <Button icon="md-refresh" @click="btnsHand('refresh')" class="operation-button ml10">重置</Button>
        </slot>
      </div>
...
export default {
  ...
  methods: {
    ...
    expandHand () {
      ...
      this.$nextTick(() => {
        ...
        if (dome) {
          ...
          // })
          this.$nextTick(() => {
            this.tableHeightHand(this.tableClassName, false, 'adjust');
            this.$nextTick(() => {
              this.$emit('expand', {
                isExpand: this.isExpand,
                filterWidth: dome.offsetWidth,
    ...
    // 当展开时，设置适应的表格高度
    tableHeightHand (domeClass, type = false, adjust) {
      if (!this.handleTable) return;
      const dome = this.$refs[`dome-${this.dytRefFilter}`];
      let newClass = domeClass;
      if (!this.$common.isEmpty(newClass) && !newClass.includes('.') && !newClass.includes('#')) {
        newClass = `.${newClass}`;
      }
      let tableDome = [...(document.querySelectorAll(`${!this.$common.isEmpty(newClass) ? `${newClass} .ivu-table-wrapper` : '.ivu-table-wrapper'}`) || [])];
      if (dome && tableDome.length > 0) {
        tableDome.forEach(item => {
          ...
          if (item.tableHeight) {
            ...
            // if (this.isExpand && adjust == 'adjust') {
            //   // 用于修复部分数据变更后表格不对齐
            //   setTimeout(() => {
            //     this.tableHeightHand(domeClass, false, 'adjust');
            //   }, 2000);
            // }
          }
        })
      ...
      this.$nextTick(() => {
        ...
        this.$nextTick(() => {
          ...
          this.$nextTick(() => {
            this.$emit('width-change', {
              isExpand: this.isExpand,
              filterWidth: dome ? dome.offsetWidth : 0,
              filterHeigth: dome ? dome.offsetHeight : 0
            })
          })
        })
      })
