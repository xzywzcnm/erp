...
<template>
  <div class="preview-main">
    <slot>
      <!-- 单个图片 -->
      <div class="image__inner" v-if="!fileList.length" :class="imgOption.className">
        <img :src="urlSetting(pictureUrl)" @click="handleView"
          :style="{ width: imgOption.width + 'px', height: imgOption.height + 'px' }">
      </div>
      <!-- 图片缩略图 -->
      <div class="pic-list" :class="abbreviationClass" v-else>
        <div v-for="(item, index) in fileList" :key="index + 'previewImg' + index" class="pic-items">
          <img :src="urlSetting(item.url)" @click="handleView(index)" :class="imgOption.className"
            :style="{ width: imgOption.listWidth + 'px', height: imgOption.listHeight + 'px' }">
        </div>
      </div>
    </slot>
      ...
      <div class="img-dialog" v-if="visible">
        <div class="img-view">
          ...
          <div class="image-viewer__actions">
            ...
            <!-- 全屏 -->
            <span class="iconfont  preview-bottom"
              :class="fullScreen ? 'dyt-preview-icon-one' : 'dyt-preview-quanping1'" @click="screenAll"></span>
            <!-- 右旋转 -->
