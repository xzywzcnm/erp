    ...
    <div
      ...
      <slot name="input">
        ...
        <div class="ivu-tag ivu-tag-checked" v-for="(item, index) in selectedMultiple" :key="index">
          ...
          <Icon type="ios-close" @click.native.stop="removeTag(index,item.nodeKey)" />
        </div>
        <span
        ...
        />
        <Icon type="md-arrow-dropdown" :class="[prefixCls + '-arrow']" v-if="!remote" />
      </slot>
    </div>
...
import { oneOf, findComponentDownward, deepCopy } from '../d-Table/relation/utils/assist';
