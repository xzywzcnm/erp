...
@font-face {
  ...
  src: url('//at.alicdn.com/t/font_wpboh046qx5hfr.eot?t=1493796069197#iefix') format('embedded-opentype'), /* IE6-IE8 */ url('//at.alicdn.com/t/font_wpboh046qx5hfr.woff?t=1493796069197') format('woff'), /* chrome, firefox */ url('//at.alicdn.com/t/font_wpboh046qx5hfr.ttf?t=1493796069197') format('truetype'), /* chrome, firefox, opera, Safari, Android, iOS 4.2+*/ url('//at.alicdn.com/t/font_wpboh046qx5hfr.svg?t=1493796069197#iconfont') format('svg'); /* iOS 4.1- */
}
...
.icon-iconfontunie047:before {
  content: "\e60b";
}

.icon-iconfontunie048:before {
  content: "\e60c";
}
...
<template >
  <div
    ...
    <template v-if="renderType === 'switch'" >
      <template v-if="row.hasChild" >
        <i
            ...
            :style="{paddingLeft:row.indentSize + 'px'}" > </i > {{ row[column.key] }}
      </template >
    ...
    <template v-if="renderType === 'expand' && !row._disableExpand" >
      <div
          :class="expandCls" @click="toggleExpand" >
        <Icon type="ios-arrow-right" ></Icon >
      </div >
    </template >
    <template v-if="renderType === 'normal'" ><span v-html="row[column.key]" ></span ></template >
    <Cell
        v-if="renderType === 'render'" :row="row" :column="column" :index="index" :render="column.render" ></Cell >
    <template v-if="renderType === 'mixSelection'" >
      <template v-if="cs == null && rs == null" >
        <Checkbox
            :value="checked" @on-change="toggleSelect" :disabled="disabled" ></Checkbox >
      </template >
      <template v-else >
        <Cell
            :row="row" :column="column" :index="index" :render="column.render" ></Cell >
      </template >
    </template >
  </div >
</template >
...
import Icon from './relation/icon/icon.vue';
import Checkbox from './relation/checkbox/checkbox.vue';
