<template >
  <table cellspacing="0" cellpadding="0" border="0" :style="styles" >
    ...
    <thead >
    <tr v-for="(cols, rowIndex) in headRows" :key="`tr-${rowIndex}`">
      <th
          ...
          <template v-else >
            ...
            >
              <span :class="[prefixCls + '-filter']" >
                <i class="ivu-icon ivu-icon-funnel" :class="{on: getColumn(rowIndex, index)._isFiltered}" />
              </span>
              <div
                  slot="content"
                ...
                <div :class="[prefixCls + '-filter-list-item']" >
                  ...
                  >
                    <checkbox v-for="(item, index) in column.filters" :key="`checkbox-${index}`" :label="item.value" >
                      {{ item.label }}
                    </checkbox >
                  </checkbox-group >
                </div >
                <div :class="[prefixCls + '-filter-footer']" >
                  <i-button
                      ...
                      @click.native="handleFilter(getColumn(rowIndex, index)._index)" >筛选
                  </i-button >
                  <i-button type="text" size="small" @click.native="handleReset(getColumn(rowIndex, index)._index)" >重置
                  </i-button >
                </div >
              </div >
              <div slot="content" :class="[prefixCls + '-filter-list']" v-else >
                <ul :class="[prefixCls + '-filter-list-single']" >
                  <li
                      ...
                      @click="handleReset(getColumn(rowIndex, index)._index)" >全部
                  </li >
                  <li
                    :class="itemClasses(getColumn(rowIndex, index), item)"
                    v-for="(item, lIndex) in column.filters"
...
<script>
import CheckboxGroup from './relation/checkbox/checkbox-group.vue';
import Checkbox from './relation/checkbox/checkbox.vue';
import Poptip from './relation/poptip/poptip.vue';
import iButton from './relation/button/button.vue';
