<template >
  <table
      cellspacing="0" cellpadding="0" border="0" :style="styleObject" class="ive-table-body" ref="body" >
    <colgroup >
      <col v-for="(column, index) in columns" :width="setCellWidth(column, index, false)" :key="`col-${index}`" />
    </colgroup >
    ...
    <template v-for="(row, index) in data" >
      <template v-if="!row.pid && row.pid !== 0" >
        <tr
            ...
            @dblclick="dblclickCurrentRow(row._index, row.nodeIndex)" >
          <template v-for="(column, n) in colPos[index]" >
            <td :class="alignCls(column, row)" v-if="column.rowSpan && !column.colSpan" :rowSpan="column.rowSpan" :key="`td1-${n}`" >
              <Cell
                  :fixed="fixed"
                  ...
                  :rs="column.rowSpan" ></Cell >
            </td >
            <td
              :class="alignCls(column, row)"
              v-if="column.rowSpan && column.colSpan"
            ...
            >
              <Cell
                  ...
                  :expanded="rowExpanded(row._index)"
                  :rs="column.rowSpan"
                  :cs="column.colSpan" ></Cell >
            </td >
            <td :class="alignCls(column, row)" v-if="!column.rowSpan && column.colSpan" :colSpan="column.colSpan" :key="`td3-${n}`">
              <Cell
                  :fixed="fixed"
                  :prefix-cls="prefixCls"
                  :row="row"
            ...
            <td :class="alignCls(column, row)" :v="column.hide" v-if=" column.hide != 1" :key="`td4-${n}`">
              <Cell
                  :fixed="fixed"
                  :prefix-cls="prefixCls"
                  :row="row"
      ...
      <template v-else >
        <tr
          ...
          <template v-for="(column,n) in colPos[index]" >
            <!--   {{row}} -->
            <td :class="alignCls(column, row)" v-if="column.rowSpan && !column.colSpan" :rowSpan="column.rowSpan"  :key="`td5-${n}`">
              <Cell
                  :fixed="fixed"
                  :prefix-cls="prefixCls"
                  :row="row"
            ...
            <td :class="alignCls(column, row)" v-if="!column.rowSpan && column.colSpan" :colSpan="column.colSpan" :key="`td7-${n}`">
              <Cell
                  :fixed="fixed"
                  :prefix-cls="prefixCls"
                  :row="row"
            ...
            <td :class="alignCls(column, row)" :v="column.hide" v-if="column.hide != 1"  :key="`td8-${n}`">
              <Cell
                  :fixed="fixed"
                  :prefix-cls="prefixCls"
                  :row="row"
      ...
      <tr v-if="rowExpanded(row._index)" :key="`tr1-${index}`">
        <td
            :colspan="columns.length" :class="prefixCls + '-expanded-cell'" >
          <Expand
              :key="row._rowKey" :row="row" :render="expandRender" :index="row._index" ></Expand >
        </td >
      </tr >
    </template >
    </tbody >
...
export default {
  ...
  methods: {
    ...
    rowClasses (_index) {
      return [
        `${this.prefixCls}-row`, this.rowClsName(_index), {
