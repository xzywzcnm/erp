...
let regular = {
  ...
  nubReg: /^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*)|0)$/, // 币种
  validatePhone: /^(([0-9]|[-])+$)|(^$)/, // 可为空，纯数字
  validatePrice: /^[+]{0,1}(\d+)$|^[+]{0,1}(\d+\.\d{1,2})$/, // 正数或者两位小数的金额
  validateGreater: /^([1-9]\d*(\.\d{1,2})?|([0](\.([0][1-9]|[1-9]\d{0,1}))))$/, // 大于 0 (包含2位小数)
  validateInteger: /^[1-9]\d*$/, // 校验正整数
  validateAmount: /^[0-9]\d*$/, // 校验正整数
  // validateUrl: /^(?:http(s)?:\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+$/, // 校验网址
  validateUrl: /^(https:\/\/|http:\/\/)/, // 校验网址
};
