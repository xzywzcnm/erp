<template>
  <div class="mainBox sizeManagePage">
    <Card shadow>
      <div class="operaBtn" style="display: flex; max-width: 800px;">
        <div style="flex: 50;">
          <Button type="primary" class="mr10" @click="add" v-if="getPermission('pdsSettings_sizeManage_add')">添加</Button>
          <Button type="primary" @click="exportExcel" :loading="exportLoading"
            v-if="getPermission('pdsSettings_sizeManage_export')">导出</Button>
        </div>
      ...
      <div class="mt10" style="max-width: 800px;">
        <Table :columns="columns" :data="tableList" border :loading="tableLoading" :max-height="tableHeight"
          highlight-row></Table>
      </div>
