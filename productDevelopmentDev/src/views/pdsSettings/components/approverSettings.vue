<template>
  <div class="mainBox approver-settings">
    <Card shadow>
      <Form ref="formData" :model="formData" :label-width="176" label-position="left">
        <div v-for="(data, name, key) in formData" :key="key">
          <!-- <FormItem :label="name==='stockDevelopment' ?'备货开发' : '云仓开发',"></FormItem> -->
          <FormItem :label="chooseName(name)" :label-width="165" class="labelWeight"></FormItem>
          <div v-for="(item, index) in data" :key="index + '1'">
            <FormItem :label="item.name + '：'" :prop="name + '.' + index + '.requireVerifyBy'"
              :rules="{ required: item.forman === 1, message: '请选择指定操作人', trigger: 'blur' }"
              :show-message="item.forman === 1">
              <div class="flex-inline">
                <RadioGroup v-model="item.forman" class="radioBlock">
                  <Radio :label="0" :disabled="item.disabled">否</Radio>
                  <Radio :label="1" :disabled="item.disabled">是</Radio>
                </RadioGroup>
                <dyt-select v-model="item.requireVerifyBy" clearable filterable :disabled="item.forman === 0"
                  :transfer="true" class="flexOne">
                  <Option v-for="(item, index) in userInfoList" :value="item.userId" :key="index + '2'">{{ item.userName
                  }}
                  </Option>
                </dyt-select>
              </div>
        ...
        <FormItem>
          <Button type="primary" @click="handleSubmit('formData')" :loading="loading"
            v-if="getPermission('pdsSettings_approverSettings_save')">确定</Button>
        </FormItem>
      </Form>
    </Card>
  </div>
...
export default {
  ...
  methods: {
    init() {
      this.$Spin.show();
      this.axios.get(api.queryProductDefaulReviewer).then((data) => {
    ...
    getUserlist() {
      this.axios.get(api.getAllUserByParm, { params: { module: 'pds' } }).then((data) => {
        if (data.code !== 0) return;
        let list = data.datas || [];
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          // return;
          this.loading = true;
          this.axios.post(api.saveProductDefaulReviewer, total).then((data) => {
            if (data.code !== 0) return;
