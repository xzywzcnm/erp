  ...
  <Modal width="800px" v-model="isVisible" title="关联尺码" :mask-closable="false" class="add-type-contain">
    ...
    <div slot="footer">
      ...
      <Button
        ...
        :title="`${isIdentical ? '关联尺码及排序不变，无需保存！' : ''}`"
      >确定</Button>
    </div>
  </Modal>
</template>
...
export default {
  ...
  computed: {
    // 新数据和旧数据是否相同
    isIdentical () {
      const oldSize = (this.oldTableData || []).map(m => m.sizeId).join(',');
      const newSize = (this.tableList || []).map(m => m.sizeId).join(',');
  ...
  methods: {
    ...
    open () {
      ...
      this.$nextTick(() => {
        ...
        this.oldTableData = this.$common.copy(this.tableList);
      });
    },
    // 表格数据获取（需要保持表格排序）
    getTableList (newVal, oldVa, type) {
      ...
      let rowItem = {};
      newList = (newList.sort((min, big) => {
        return min.sortNo - big.sortNo;
      })).map((row, index) => {
        rowItem = { ...row, sortNo: index + 1 };
        this.$set(this.selectJson, row.sizeId, rowItem);
        return rowItem
      });
      return newList;
    },
    ...
    mobileRow (res, type) {
      if (type == 'up') {
        ...
        this.tableList[res.index].sortNo = res.index - 1;
      } else {
        this.tableList[res.index + 1].sortNo = res.index;
        this.tableList[res.index].sortNo = res.index + 1;
      }
      let rowItem = {};
      this.tableList = (this.tableList.sort((min, big) => {
        return min.sortNo - big.sortNo;
      })).map((row, index) => {
        rowItem = { ...row, sortNo: index + 1 };
        this.$set(this.selectJson, row.sizeId, rowItem);
