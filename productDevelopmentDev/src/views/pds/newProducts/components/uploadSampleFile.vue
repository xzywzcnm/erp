  ...
  methods: {
    ...
    fileUploadBefore (file) {
      ...
      this.axios.post(uploadUrl, newForm).then(res => {
        if (!res || res.code != 0 || this.$common.isEmpty(res.datas)) return;
        if (this.uploadConfig.multiple) {
          this.filePathList.push({ fileName: fileName, fileUrl: res.datas[0] });
        } else {
          this.filePathList = [{ fileName: fileName, fileUrl: res.datas[0] }];
        }
      }).finally(() => {
    ...
    dowFile (file) {
      ...
      let fileUrl = `${window.location.origin}/pds-service/filenode/s${file.fileUrl}`;
      if (process.env.NODE_ENV === 'development') {
        fileUrl = `${window.location.origin}/filenode/s${file.fileUrl}`;
      }
