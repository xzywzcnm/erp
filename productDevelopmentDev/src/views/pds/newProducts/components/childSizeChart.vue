...
export default {
  ...
  computed: {
    ...
    // 是否已存在自用尺码图
    isSelfUsePic () {
      const findPic = this.allPic.find(item => {
    ...
    totalColumns () {
      ...
      let fun = {
        language: _self.defaultLanguage,
        temp: {
          tagSize: {
            minWidth: 40,
            model: 'tagSize',
  ...
  methods: {
    ...
    createIframe () {
      ...
      iframe.src = `${window.location.pathname}#/childSizePicture`;
    ...
    getPicStock () {
      return new Promise((resolve) => {
        const { productId } = this.productInfo;
        this.allPic = [];
    ...
    // 处理商品分类
    categoryData () {
      const productCategoryId = this.productInfo.goodTypeId;
      return new Promise((resolve) => {
      ...
      return new Promise((resolve) => {
        this.$common.promiseAll([
          ...
          this.axios.get(api.queryProductSizeClassificationInfo, {
            ...
            if (res.code !== 0 || !res.datas) return resolve({});
            let { laPaProductSizePartInfoVOList } = res.datas;
            let obj = {};
