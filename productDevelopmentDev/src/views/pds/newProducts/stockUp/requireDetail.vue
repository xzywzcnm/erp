      ...
      <div class="topper topper--flex">
        ...
        <div class="topper__right">
          <template v-if="permissionStatus">
            ...
            <template v-if="dialogObj.btnoperat === 'edit' || editBasic">
              <Button @click="savebasicData()">保存</Button>
              <Button @click="editBasicData(false)" v-if="editBasic">取消</Button>
            </template>
            <template v-if="dialogObj.btnoperat === 'subverify' && !editBasic">
              <Button type="primary" @click="editBasicData(true)"
                v-if="!(productData.productSource === 1 && productData.status === 0)">修改基础资料</Button>
          ...
          <Button @click="closeDialog(1)" v-if="!editBasic">关闭</Button>
        </div>
      </div>
        ...
        <div class="main--flex">
          ...
          <div class="main--flex__right">
            <basic-data ref="basicData" v-if="tag === 'basicData' && !pageLoading" :productData="productData"
              :basiclData="basiclData" :gatherDetail="gatherDetail" v-bind="$attrs" :gatherBasic="gatherBasicBtn"
              :operatList="operatList" @closeDialog="closeDialog" @backBasicsDetail="basicsDetailChange"
...
export default {
  ...
  data() {
    return {
      ...
      trunMan: ['verifyStyle', 'verifyPower', 'verifyPrice', 'verifyPic', 'verifyData', 'finishPic', "finishData"],
  ...
  computed: {
    ...
    goodLink() {
      ...
      if (this.$common.isEmpty(this.newBasicsData.laPaProductInfo)) {
        if (this.$common.isEmpty(this.newBasicsData.goodLink)) return '';
        return this.newBasicsData.goodLink;
      }
      return this.newBasicsData.laPaProductInfo.goodLink || '';
    },
    // 商品分类ID
    goodsCategoryId() {
      if (this.$common.isEmpty(this.newBasicsData)) return '';
