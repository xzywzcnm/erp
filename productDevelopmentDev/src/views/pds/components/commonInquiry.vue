<template>
  <div>
    <div class="demandBoxTit">
      <h3>
        询价<Button
          ...
          >新增询价</Button
        >
        <span style="color: red; font-weight: 100; margin-left: 100px"
          >预期售价： {{ expectedSellPrice }}{{ sellPriceCurrency }}</span
        >
        <span style="color: red; font-weight: 100; margin-left: 20px"
          >目标利润率:{{ profitRate }}%</span
        >
    ...
    <CommonAddSupplier
      v-if="inquiryTab != 'list'"
      ref="CommonAddSupplier"
      :type="supplierType"
      strQuotationId="strQuotationId"
      :list="inquiryDate"
      @goTabList="goTabList"
    ></CommonAddSupplier>
    ...
    return {
      ...
      inquiryTableColumns: [
        ...
        },
        {
          ...
          width: 120,
          render (h, params) {
            if (params.row.listVariQuotationInfo) {
              let priceSection = '';
              if (params.row.listVariQuotationInfo.length > 1) {
                let maxPrice = Math.max(...params.row.listVariQuotationInfo.map(i => i.goodPrice));
                let minPrice = Math.min(...params.row.listVariQuotationInfo.map(i => i.goodPrice));
                priceSection = minPrice + '~' + maxPrice;
              } else {
                priceSection = params.row.listVariQuotationInfo[0].goodPrice;
              }
              return h('span',[
                h('span', priceSection),
                h('span',{
                  ...
                  on: {
                    'click' () {
                      v.listProfitRate = params.row.profitRate;
        ...
        },
        {
          ...
          key: "profitRate",
          render (h, params) {
            let profit = v.expectedSellPrice * params.row.profitRate * 0.01;
            if (isNaN(profit)) {
              profit = 0;
            }
            let merchantExchangeRate = 1;
            v.$store.state.currencyExchangeInfo.forEach(i => {
              if (i.targetCurrencyCode === v.sellPriceCurrency) {
                merchantExchangeRate = i.merchantExchangeRate;
              }
            });
            return h('span', (profit * merchantExchangeRate).toFixed(2) + 'CNY');
          }
        },
        {
          align: 'center',
          ...
          render(h,params) {
            if (params.row.listVariQuotationInfo) {
              let priceSection = '';
              if (params.row.listVariQuotationInfo.length > 1) {
                let maxPrice = Math.max(...params.row.listVariQuotationInfo.map(i => i.goodWeight));
                let minPrice = Math.min(...params.row.listVariQuotationInfo.map(i => i.goodWeight));
                priceSection = minPrice + '~' + maxPrice;
              } else {
                priceSection = params.row.listVariQuotationInfo[0].goodWeight;
              }
              return h('span',[
                h('span', priceSection)
              ]);
            } else {
              return h('span',params.row.productWeight);
            }
          }
        },
        {
    ...
    getList () {
      ...
      v.$axios
        ...
        .then((res) => {
          ...
          if (res.code === 0) {
            if (res.datas.quotationInfoList === null) {
              v.inquiryDate = [];
            } else {
              res.datas.quotationInfoList.forEach((item, index) => {
                item.index = index + 1;
              });
              v.inquiryDate = res.datas.quotationInfoList;
              v.$store.commit("inquiryList", res.datas.quotationInfoList);
            }
          }
          // else {
          // }
