      ...
      <div class="modal-edit-main">
        ...
        >
          ...
          >
            ...
            <template slot-scope="{ index}" slot="suggestPrice">
            ...
            <template slot-scope="{ row }" slot="isMerchantErp">
          ...
          <div class="mt15">
            ...
            >
              <template v-for="(colSlot, colIndex) in ymsSizeSlotKey" slot-scope="{ index }" :slot="colSlot.slotKey">
                <FormItem
...
export default {
    ...
    return {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          render: (h, { row }) => {
            let txt = row.color || '';
            if (this.$common.isEmpty(row.sizeOrModelName)) return h('div', txt);
            return h('div', `${txt}·${row.sizeOrModelName}`);
          }
        },
        {
          title: 'YMS颜色',
  ...
  computed: {
    ...
    attributeValueVOList () {
      if (this.$common.isEmpty(this.commodityData) || this.$common.isEmpty(this.commodityData.attributeValueVOList)) return [];
      return this.commodityData.attributeValueVOList;
    }
  },
  methods: {
    // 重置页面数据
    ...
    initData () {
      ...
      this.$nextTick(() => {
        this.getDetail(this.selectSpuData).finally(() => {
    ...
    getYmsAccountData () {
      return new Promise((resolve) => {
        ...
        this.axios.get(api.ymsMerchantAccount).then(res => {
          if (!res || res.code != 0) return resolve({ success: false, fail: true, data: [] });
          this.ymsAccountData = res.datas || [];
          resolve({ success: true, fail: false, data: res.datas || [] });
        }).catch(() => {
    ...
    getDetail (config) {
      return new Promise((resolve) => {
        this.commodityData = {};
        if (this.$common.isEmpty(config.productId)) resolve({ success: false, fail: true, data: {} });
        this.$axios.get(api.queryLaPaProductGoodsInfo, {
          params: { productId: config.productId }
        }).then(({ code, datas }) => {
          if (code !== 0) return resolve({ success: false, fail: true, data: {} });
          this.commodityData = datas || {};
          resolve({ success: true, fail: false, data: datas || {} });
        }).catch(() => {
          resolve({ success: false, fail: true, data: {} });
        });
      });
    ...
    getProductTree () {
      return new Promise(resolve => {
        if (!this.$common.isEmpty(this.productCategory)) resolve({ success: true, fail: false, data: this.productCategory });
        this.axios.get(api.productCategoryqueryAll).then(res => {
          if (!res || res.code != 0) return resolve({ success: true, fail: false, data: [] });
          this.productCategory = this.getProductCategoryTree(res.datas || [], null);
          resolve({ success: true, fail: false, data: this.productCategory });
        }).catch(() => {
    ...
    getYmsCategory () {
      return new Promise(resolve => {
        ...
        this.axios.post(`${api.getYmsProductCategory}?ymsMerchantId=${this.formData.ymsMerchantId}`).then(res => {
          if (!res || res.code != 0) return resolve({ success: false, fail: true, data: [] });
          this.ymsProductCategory = this.ymsCategoryHand(res.datas || []);
          resolve({ success: true, fail: false, data: res.datas || [] });
        }).catch(() => {
          resolve({ success: false, fail: true, data: [] });
        })
          ...
          this.getYmsCategoryDetails(value).finally(() => {
            ...
            if (!this.$common.isEmpty(this.formData.tableData)) {
              this.formData.tableData.forEach((row) => {
                if (this.ymsSizeList.length > 0 && !this.$common.isEmpty(row.sizeOrModelName)) {
                  const findInfo = this.ymsSizeList.find(f => row.sizeOrModelName == f.cnValue);
                  if (!this.$common.isEmpty(findInfo)) {
                    row.ymsSizeId = findInfo.ymsProductAttributeDictionaryId;
                  }
                }
                if (this.ymsColorList.length > 0 && !this.$common.isEmpty(row.color)) {
                  const findInfo = this.ymsColorList.find(f => row.color == f.cnValue);
                  if (!this.$common.isEmpty(findInfo)) {
    ...
    getYmsCategoryDetails (ymsCategoryId) {
      return new Promise(resolve => {
        ...
        tableData.forEach(row => {
          if (!this.ymsTableSizeData.includes(row.sizeOrModelName)) {
            this.ymsTableSizeData.push(row.sizeOrModelName);
          }
          row.ymsSizeId = '';
          row.ymsColorId = '';
        ...
        this.axios.post(`${api.queryYmsProductCategory}/${ymsCategoryId}?ymsMerchantId=${this.formData.ymsMerchantId}`).then(res => {
          ...
          let itemInfo = {};
          // 处理对应的值
          const getRes = this.$common.copy((res.datas || {}));
    ...
    getAttributeValueList(config) {
      return new Promise(resolve => {
        this.axios.post(api.query_findAttribute, {
          ...
          if (!res|| res.code != 0) return resolve({ state: false, res: {attributeValueIds: this.attributeValueVOList} });
          let newRes = {};
          res.datas.attributeValueIds = this.attributeValueVOList;
          if (this.$common.isEmpty(res.datas)) {
            newRes.attributeValueIds =  this.attributeValueVOList;
          } else {
            newRes = {...res.datas, attributeValueIds: this.attributeValueVOList}
          }
          resolve({ state: true, res: newRes });
        }).catch(() => {
          resolve({ state: false, res: {attributeValueIds: this.attributeValueVOList} });
        })
      })
    },
    // 属性单个匹配判断
    ...
    tableRowDataChange (row, index, key) {
      this.$nextTick(() => {
        ...
        this.formData.tableData.forEach((tRow, tIndex) => {
          if (tIndex != index && this.$common.isEmpty(tRow[key]) && !this.$common.isEmpty(keyObj[key])) {
            if (tRow[keyObj[key]] == nowRow[keyObj[key]]) {
    ...
    getProductDetails (config) {
      return new Promise(resolve => {
        ...
        this.$axios.get(api.getSkuList, {
          params:{
            productId:config.productId
          }
        }).then(res => {
          if (!res || res.code != 0) return resolve({ success: false, fail: true, data: {} });
          this.productGoodsDetails.productGoodsList = (res.datas || []).filter(item => item.choiceStatus == 0);
          resolve({ success: true, fail: false, data: this.productGoodsDetails });
        }).catch(() => {
          resolve({ success: false, fail: true, data: {} });
        })
      })
    ...
    saveData () {
      ...
      this.$refs.modalForm.validate((valid) => {
        ...
        let awaitRes = [
          () => {
            // 调用同步到 YMS 的方法
            return this.syncProductToYms(params);
          }
        ];
        if (!this.$common.isEmpty(this.modalData.callBack)) {
          // 调用同步到 Listing 的方法
          awaitRes.push(this.modalData.callBack);
        }
        this.$common.promiseAll(awaitRes).then((arrRes) => {
          // if (arrRes[1] && arrRes[1].fail && arrRes[0] && arrRes[0].fail) return this.$Message.warning('同步操作失败');
          // if (arrRes[0] && arrRes[0].fail) {
          //   this.$Message.warning('同步Listing操作成功，同步YMS操作失败');
          // } else if (arrRes[1] && arrRes[1].fail) {
          //   this.$Message.warning('同步Listing操作失败，同步YMS操作成功');
          // } else {
          //   this.$Message.success('操作成功！');
          // }
          if (arrRes[1] && arrRes[1].fail && arrRes[0] && arrRes[0].fail) return;
          this.$Message.success('操作成功！');
    ...
    // 调用同步到 YMS 的请求
    syncProductToYms (params) {
      return new Promise((resolve) => {
        this.axios.post(api.syncYmsProductInfo, params).then(res => {
          if (!res || res.code != 0) return resolve({fail: true, msg: res.message || '同步YMS操作失败'});
          resolve({fail: false, msg: '同步YMS操作成功'});
        }).catch(() => {
          resolve({fail: true, msg: '同步YMS操作失败'});
        })
      })
    }
  }
