<template>
  <div class="filterColumns">
    ...
    <transition name="slide-fade">
      <Card class="filterColumnsBox" v-show="filterColumnsIcon != 'ios-arrow-up'" @click.native.stop="hideCardStop" ref="filterCard">
        <Row>
          <Col span="24">
          <p style="border-bottom: 1px solid #ddd">
            自定义列显示<span style="color: #2d8cf0; cursor: pointer; float: right" @click="filterColumsReset">恢复默认</span>
          </p>
          </Col>
          <Col span="12" v-for="(item, index) in columnsInit" :key="index">
          <Checkbox v-model="item.check" @on-change="filterColumsChange">{{
              item.title
            }}</Checkbox>
          </Col>
        </Row>
  ...
  methods: {
    ...
    filterColumsReset () {
      ...
      setTimeout(() => {
        ...
        localStorage.setItem(v.filterName, JSON.stringify(v.columnsInit));
        v.columns.forEach((item) => {
          if (!item.filterHide) {
            arr.push(item);
          }
