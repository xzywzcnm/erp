...
export default {
  ...
  methods: {
    ...
    addTagHand (e) {
      ...
      // 字符串分割
      const newAddItems = this.$common.split(this.inputValue, this.split, true);
      let addItems = [];
      let matchingSplit = [];
      ...
      if (this.defaultProp.value || this.defaultProp.label) {
        ...
        addItems.forEach((item) => {
          if (!keys.includes(item) || this.type === 'textarea') {
            ...
            !this.$common.isEmpty(newTag) && addTags.push(newTag);
          }
        })
      }
      if(!this.preview && this.type === 'textarea' && this.limit > 0) {
        this.vModel = this.defaultProp.value ? addTags : this.$common.copy(addItems);
        setTimeout(() => {
          this.inputValue = this.changeInputVal(matchingSplit);
        }, 300);
      } else {
        this.vModel = this.defaultProp.value ? [...this.vModel, ...addTags] : this.$common.arrRemoveRepeat([...this.vModel, ...addItems]);
        this.inputValue = '';
      }
    },
