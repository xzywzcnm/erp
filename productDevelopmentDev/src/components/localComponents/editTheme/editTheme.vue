<template>
  <Drawer width="300" :closable="false" v-model="model">
    <div slot="header" class="flex-box item-align-center">
      <span class="font-16">{{t('key001')}}</span><Tooltip content="修改主题只保存在当前机器"><Icon class="ml10" type="ios-information-circle-outline" size="14" /></Tooltip>
    </div>
    <div class="flex-box mb10 flex-between"><span>主色调</span><themePicker :colorValue="themeColor"></themePicker></div>
    <div class="flex-box mb10 flex-between"><span>头部背景</span>
      <ColorPicker
        class="theme-picker"
        :recommend="true"
        @on-change="topBackgroundColorChange"
