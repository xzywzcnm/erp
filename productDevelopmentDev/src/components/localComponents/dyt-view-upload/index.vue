      ...
      >
        <template v-if="item.status === 'finished' || viewType">
          ...
          <div class="demo-upload-list-cover">
            ...
            <div v-if="isFileTitle" class="file-name" :title="`${item.name||''}`">{{item.name||''}}</div>
            <div
              v-if="isCheckFile"
              class="file-checkbox"
              :class="{'check-item': item.checked}"
        ...
        <template v-else>
          <Progress v-if="item.showProgress" status="active" :stroke-width="15" text-inside :percent="item.percentage" />
        </template>
      </div>
    </draggable>
    <!-- iviewui 的上传组件 -->
    ...
    >
      <slot>
        <div
          ...
          :class="{'disabled-upload': selectConfig.disabled}" v-if="!viewType"
          :style="`width:${viewWidth};height: ${viewHeight}`"
        >
...
// import dytUpload from '../dyt-upload/index';
...
export default {
  ...
  computed: {
    ...
    selectConfig () {
      return { ...this.defaultConfig, ...this.$attrs }
    }
  },
  created () {},
  mounted () {
