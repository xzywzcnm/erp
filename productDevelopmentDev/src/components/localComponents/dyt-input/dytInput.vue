  ...
  methods: {
    ...
    limitHand () {
      if (!['textarea'].includes(this.$attrs.type)) {
        ...
        if (!dome || !dome.querySelector('.ivu-input-word-count') || !dome.querySelector('.ivu-input')) return;
    ...
    appendColse () {
      ...
      this.$nextTick(() => {
        ...
        if (!this.showClear) {
          ...
          inputDome && inputDome.parentNode && inputDome.parentNode.appendChild(this.iconDome);
        }
        if (this.$slots && (this.$slots.suffix || this.$slots.append)) {
          const domeClass = this.$slots.append ? 'ivu-input-group-append' : 'ivu-input-suffix';
          const demo = refsDome.querySelector(`.${domeClass}`);
          const append = demo ? refsDome.querySelector(`.${domeClass}`).offsetWidth : 0;
          ...
          if (this.$slots.prefix) {
            const drefs = refsDome.querySelector(`.${domeClass}`);
            const prefix = drefs ? refsDome.querySelector('.ivu-input-prefix').offsetWidth : 0;
