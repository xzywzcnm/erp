...
// 在设置区间内获取遇到的第一个空闲端口
const portStr = getFreePort(8088, 50000);

const erpEnv = {
  dev: 'http://172.20.200.30',
  uat: 'http://testing.lapa.com.cn',
  prod: 'https://erp.lapa.com.cn',
  pd: 'http://172.20.200.116:8107',
  jy: 'http://172.20.200.102:8107',
  cx: 'http://172.20.200.100:8107',
  nq: 'http://172.20.200.112:8107',
}
...
const proxyToTargetServe = (target, key, isHtml = false) => {
  return {
    ...
    ws: false,
    onProxyRes: function (proxyRes, req, res) {
      proxyRes.headers["area1"] = isHtml ? 'pds-service' : key;
    },
